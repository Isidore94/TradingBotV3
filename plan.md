...
##### September 22 corrective work — a useful daily AI review

**IMPLEMENTED IN SCRATCH; LIVE VALIDATION OWED, 2026-09-22**, from the trader's request to address the overnight deficiencies and make the on-device daily summary serve the project goals. This repairs existing TJ-4/TJ-12/TJ-13/TJ-16 and M2/WS-TH behavior; it does not promote a wishlist item or a trading rule. Lead branch `codex/overnight-ai-repair`, base `main` `fb95854e`.

1. **AI-R1:** use code-owned read/result links and exact note fragments in model requests; preserve the stored v1 shapes, strict invalid-reply rejection, prior verified files and blind tagging. Tests prove the September 21 duplicate-read and bad-span failure modes before fixing them.
2. **AI-R2:** one visible route from A.I. Summary to the existing Day Review for the last completed exchange session; one existing worker, no model or write from navigation. Use the loaded report card to distinguish a rejected story, an unknown night and a night not run, while retaining facts and ideas.
3. **AI-R3:** deterministic close+35 outcome recovery before evidence/digest readers, then refresh the canonical day facts/pack after measured_report, even if the page was never opened. Save theta picks after deferred quotes, retaining unknowns on failure and never fabricating old prices. The trader approved the two protected file edits with “continue” after the exact-seam request; both are implemented in scratch. A bounded append-open contention repair is conditional on scratch reproduction, never a retry after writing bytes.

R1/R2 completed tester → builder → independent reproduction. R3 independent review reproduced an unreadable-CSV recovery bug: an unknown final row cleared its write-ahead mark. Fix `1d9d909d` keeps that mark, fails the single finalizer and batch sweep closed, and passed red-before-green tests plus reviewer re-reproduction. The prior R3 branch suite had 10,308 passed with the known G7 timing flake green alone; the Sep22 combined suite finished with 10,428 passed, 14 skipped, 72 subtests and the same G7 flake (20/20 isolated, 21/21 file). Frozen source and exe selftests both passed 97/97. Decision 0018 and the order pins cover the new deterministic slots. Preserve all existing live gates; **#148/#165** need a later verified local-model story and exact grounded tags. Additional proof must show the night finalizing saved outcomes with the desk closed, rebuilding its own day pack, displaying the correct completed session and retaining option quotes in new evidence rows. No daytime inference, no live data repair, no detector/score/alert change; historical missing quotes remain unmeasured. The trader authorized the main update on 2026-09-22. Desk restart and live gates remain separate.

##### TJ-17 — Complete the daily and weekly learning loop (AUTHORIZED 2026-09-22)

Trader: **"Go ahead and implement the suggested changes"**, after the 2026-09-22 recap assessment. This advances the assessment's four recommendations into Phase 0.33; no WISHLIST item is promoted. Base `main` `fb95854e`; integration branch `codex/tj17-recap-complete` in an isolated worktree. The desk checkout stays on main. Governing contracts remain sections 5–7, decision 0021, and DESK_INTERNALS TJ-4/TJ-5/TJ-6/TJ-10/TJ-12/TJ-16.

**Existing facts:** one selected-session page, five week cards, month/week strips, strict narration validation, canonical read/baseline readers, Results' entry-environment trade reader and Keep/Dismiss are already built. The assessment reproduced mixed-horizon headlines and a double subtraction of pending calls; the full comparison tables are not rendered and nested measured market internals are not encoded. The live sample at assessment was one saved day pack with five clicked predictions; it is a warm-up sample, not a performance verdict.

**Build order and ownership:**

Build state, 2026-09-22: A and B are green on their targeted tests; C and D are implemented on the same isolated branch and their targeted tests passed before the final read-only and history refinements. A's independent reproduction is complete. C/D later received independent review; its future-session follow-through blocker was fixed in `9f625796` and re-reviewed GO, and numbered live gates #186–189 were added. The combined suite completed with only the known G7 cross-suite flake (20/20 isolated, 21/21 file); the states in the original numbered contract below describe the authorized work at kickoff, not a claim that it is still unbuilt.

1. **TJ-17A — Read integrity (BUILT, targeted tests green).** `prediction_ledger.your_reads` -> report card -> saved pack -> week preserves separate rest-of-day and next-five-session cells and baselines. Closed coverage is not reduced by pending again. Legacy aggregate cards are labelled unseparated and never assigned a guessed horizon; inventory totals carry no cross-horizon rate. The comparison projects an explicit allowlist of measured canonical internals, preserving missingness and zero/False, and never recomputes an indicator.
2. **TJ-17B — Review windows (BUILT, targeted tests green).** A 5/10/20 completed-session learning view with visible read accuracy/baselines, confidence, hour and D1-environment cells, counts and distinct sessions, plus actual trade results using existing owner readers. Day/swing and unknown context stay separate; every table says its dates and supporting rows are reachable. Existing calendar-week story and month strips retain their explicitly named scopes.
3. **TJ-17C — Day decisions and complete facts (BUILT, targeted tests green).** Separate market calls, observed selection outcomes and actual entry/exit review; show original pass/veto action and correct passes without inventing a trade-quality verdict. Show entry/exit words, provenance and measured net result together. Reuse the independently built AI-R1/R2/R3 night-transport, navigation/fallback and deterministic-facts repairs where they meet the requirement. Refresh saved facts after later trade answers and matured calls; stale/missing facts and rejected stories are named, never passed as current.
4. **TJ-17D — One chosen change (BUILT, targeted tests green).** Reuse the ideas card and frozen baselines, add one explicit active process change per exchange week with an explicit followed/not-followed/unknown record, and scoped named readers for market-read accuracy and actual trading/process behavior. Before/after uses non-overlapping dates and the same horizon/context; missing data and thin samples cannot claim improvement or causation. Keeping an idea is not proof it was followed.

**Files expected:** read summaries/comparison modules; `ui/services/day_review_service.py`, `ui/services/weekend_prep_service.py`, their two panels; a shared review-window reader if required; `ai_jobs/improvement_ideas.py`, `ui/widgets/ideas_card.py`; named evidence paths; the existing deterministic facts/night narration seams and page-navigation wiring. No detector, production score, alert, watchlist, Focus, review queue or policy change is authorized. File-scoped ask-first is satisfied for the assessment's named report readers and UI behavior by the approval above; an unrelated sensitive file remains out of scope.

**Verification:** independent red tests -> builder -> independent reproduction review for each packet, a single full suite on the combined tree, ruff, smoke and source selftest; offscreen UI checks and desk-bench timing for page changes. New tests cover actual store/read/render/write paths, not source-string assertions. No model by day; live stores remain untouched by tests. No code is claimed built until its packet passes. Lead owns central documentation reconciliation; worker handoffs supply exact contract and verification changes.

**Live acceptance still owed:** compare each horizon and pending count by hand; 5/10/20 window dates and table totals; open a contributing day/trade; save a late exit answer and see refreshed facts next night; strict verified story or a clearly named fallback; keep one change, record follow-through, restart and verify the baseline and unknown states survive. Existing gates including #155/#161/#164/#169/#172–#175/#181 remain owed where applicable. TJ-8 cleanup still waits for live validation. The trader authorized the main update on 2026-09-22; desk restart and numbered live gates #186–189 remain separate.
