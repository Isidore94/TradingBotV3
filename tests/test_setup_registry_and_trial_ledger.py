...
def test_the_htf_lrsi_grid_is_declared_even_though_it_is_not_on_this_branch():
    """Backfilled from its own branch's grid, so the ledger is complete on merge.

    `HTF_LRSI_RECIPES` lives on `claude/focus-declutter-lrsi-htf` and is not in
    `outcomes` here. The ledger declares it anyway: a look-counter that starts
    when the code merges would record the grid as never having been examined.
    """
    ...
    assert row["declared_cell_count"] == 16
...
def test_neither_module_is_imported_by_production_yet():
    """P7 ships a description, not a dependency. P4.1 is where that changes."""
    import subprocess

    result = subprocess.run(
        ["git", "grep", "-l", "-e", "setup_registry", "-e", "trial_ledger", "--", "scripts/"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    ...
    assert importers <= {
        ...
        "scripts/research_warehouse/trial_ledger.py",
    }, importers
