...
def test_the_multi_source_key_keeps_the_cohorts_apart():
    ...
    # R1 INTEGRATION NOTE. This asserted the DEFAULT key collapses the three
    # rows to one, which was true when P5 was written and stopped being true
    # when P1 merged: P1 widened `_pick_key` with `pick_source_family` to stop a
    # swing like and an M5 pick sharing a row, and that fourth element happens
    # to separate these three as well.
    #
    # So `pick_key_with_source` is now belt-and-braces here rather than the only
    # thing holding the cohorts apart - and it stays, because the two keys are
    # answering different questions: P1's separates CATEGORIES and P5's
    # separates SOURCES within one category, and a future category that carried
    # several sources would collapse again under P1's alone.
    default_keys = {_pick_key(row) for row in rows}
    wide_keys = {pick_key_with_source(row) for row in rows}
    assert len(wide_keys) == len(rows) == 3, "every cohort must keep its own row"
    assert len(default_keys) <= len(wide_keys), (
        "the wider key may never merge rows the default one separates"
    )
