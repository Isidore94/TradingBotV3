"""Tests for the 1st-stdev breakout study family + the recent ("last 30 days")
setup-type stats that surface it.

1st-dev breakouts (fresh cross up through UPPER_1 / down through LOWER_1 that
leaves price inside the 1st-to-2nd-dev zone) are recorded in the isolated study
namespace (no scoring impact) so realized R accumulates and the recent-window
stats can show whether they work before any promotion to a scored family. A
fresh close that already ran past the 2nd band is left to the 2nd-dev study so
the two families stay mutually exclusive.
"""
...
def _long_rows(*, fresh: bool, band: float = 100.0):
    ...
    if fresh:
        # clean lookback: highs well below the 1st band, no touches
        ...
        # chop-at-band: priors keep tagging the 1st band -> not a fresh break
        priors = [_daily(d, 99.5, 100.8, 98.5, 100.2) for d in dates]
        today = _daily("2026-06-29", 100.0, 102.0, 99.0, 101.0)
    return priors + [today]


def _long_rows_past_second_band(band: float = 100.0):
    # Clean lookback below UPPER_1, but today's close blows past UPPER_2 (120).
    dates = [f"2026-06-2{i}" for i in range(2, 7)]
    priors = [_daily(d, 98.0, 98.5, 97.0, 98.0) for d in dates]
    today = _daily("2026-06-29", 99.0, 122.0, 98.8, 121.0)  # closed above the 2nd band
    return priors + [today]
...
def _ai_state(symbol, side, rows, band):
    # Zone width of 20 pts: LONG -> [UPPER_1, UPPER_2]=[band, band+20];
    #                       SHORT -> [LOWER_2, LOWER_1]=[band-20, band].
    if side == "LONG":
        bands = {"UPPER_1": band, "UPPER_2": band + 20}
    else:
        bands = {"LOWER_1": band, "LOWER_2": band - 20}
    return {
        "symbols": {
            symbol: {
                "side": side,
...
class FirstDevBreakoutDetectionTests(unittest.TestCase):
    def test_fresh_long_breakout_emits_study_row(self):
        rows = _long_rows(fresh=True)
        study = m.enrich_priority_rows_with_first_dev_breakouts(
            _priority_rows("NVDA", "LONG", rows),
            {},
            ai_state=_ai_state("NVDA", "LONG", rows, 100.0),
        )
        self.assertEqual(len(study), 1)
        self.assertEqual(study[0]["setup_family"], m.FIRST_DEV_BREAKOUT_STUDY_FAMILY)
        self.assertEqual(study[0]["priority_bucket"], m.FIRST_DEV_BREAKOUT_STUDY_BUCKET)
        self.assertEqual(study[0]["side"], "LONG")
        self.assertFalse(study[0]["is_favorite_setup"])
        self.assertIn("1STDEV_BREAKOUT", study[0]["setup_tags"])
    ...
    def test_break_past_second_band_is_left_to_second_dev_study(self):
        rows = _long_rows_past_second_band()
        study = m.enrich_priority_rows_with_first_dev_breakouts(
            _priority_rows("GAP", "LONG", rows),
            {},
            ai_state=_ai_state("GAP", "LONG", rows, 100.0),
        )
        self.assertEqual(study, [])
...
def _closed_1stdev(symbol, closed_r, scan, side="LONG"):
    return {
        "symbol": symbol,
        "side": side,
        "anchor_date": scan,
        "scan_date": scan,
        "setup_family": m.FIRST_DEV_BREAKOUT_STUDY_FAMILY,
        "study_kind": m.FIRST_DEV_BREAKOUT_STUDY_FAMILY,
        "priority_bucket": m.FIRST_DEV_BREAKOUT_STUDY_BUCKET,
        "scenarios": {
            "s1": {
                "tradeable": True,
                "status": "TARGET_HIT" if closed_r > 0 else "STOPPED",
                "total_r": closed_r,
                "stop_reference_label": "VWAP" if side == "LONG" else "VWAP",
            }
        },
    }
...
class RecentSetupTypeStatsTests(unittest.TestCase):
    def test_recent_family_rows_include_1stdev_breakout(self):
        ref = date(2026, 6, 29)
        study = {
    ...
    def test_build_recent_setup_type_stat_rows_tags_namespace(self):
        base = date.today()
        study = {
            f"study:s{i}": _closed_1stdev(f"S{i}", 0.8, scan=(base - timedelta(days=i + 2)).isoformat())
            for i in range(5)
        }
        payload = {"setups": {}, "study_setups": study}
        rows = m.build_recent_setup_type_stat_rows(payload)
        breakout = [r for r in rows if r["setup_family"] == m.FIRST_DEV_BREAKOUT_STUDY_FAMILY]
