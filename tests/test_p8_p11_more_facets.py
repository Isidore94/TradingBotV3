...
def test_the_new_scan_columns_append_after_every_older_column():
    assert tuple(sp.SCAN_ROW_COLUMNS[-len(sp.D1_HISTORY_COLUMNS):]) == sp.D1_HISTORY_COLUMNS
    assert sp.SCAN_ROW_COLUMNS[: -len(sp.D1_HISTORY_COLUMNS)][-3:] == sp.STAMP_COLUMNS
...
def test_the_scan_writes_the_p11_columns(scan_runs):
    ...
    header = list(row)
    assert header[-len(sp.D1_HISTORY_COLUMNS):] == list(sp.D1_HISTORY_COLUMNS)
