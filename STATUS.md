...
- **Live on `main`:** everything through the 2026-09-24 wishlist merge (tracker readers to SQLite P0-2 2d, gate #221; night AI chain P1-3: Ollama probe, 150-min weeknight budget, Saturday-only ticker briefs, summary slice cap, Flex retry, gates #217-#220; pure setup-permutation module P1-4 4a, shadow only; reviewers GO). Older merges are in `git log`.
...
- **Last full suite:** 2026-09-24 wishlist integration: 11,519 passed + 2 load flakes (34/34 alone, also on base). Ruff clean, smoke 7/7, selftest 101/101 source and frozen.
...
- **Next action:** read the live gates on the next session and night (`docs/GATES.md` #217-#221). Next wishlist step: P1-4 4a stamping keys on scan rows needs the trader's OK for scan-file edits.
