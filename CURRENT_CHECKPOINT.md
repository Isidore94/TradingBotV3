...
| | |
|---|---|
| Agent settings | **2026-09-15, trader-directed: the project Astra model pin is removed.** User defaults and explicit session choice select the lead; helper settings remain. |
| Latest work | **2026-09-20: TJ-13B (`e54c8203`) and TJ-9Q (`86c64f96`) are LIVE on the desk since 07:14 PDT** (`main` fast-forwarded to `acaa13c0` at 07:13, desk started clean - restart log). TJ-9Q ships with its import switch OFF, so nothing in the live journal has moved; the `--apply` is the trader's own act and is owed, and the 27B has still never run. **TJ-10 is MERGED into `lead/p033-integration2`** (`57b44ca9`, integration fix `3e52d94f`) and is **not live**: the read grader measures whether a market read was right, with no model anywhere - a CLICK is the read, an extracted stance is always labelled as one, an `unmeasured` verdict never supersedes a `pending`, a clicked grade is never stored without its context, and the four congruence lines are printed, never pushed. |
| Working branch | **`lead/p033-integration2` on top of `main` `739ce5a4`**, ahead of it by the TJ-10 merge `57b44ca9` and the integration fix `3e52d94f`. Nothing is pushed. |
| Unmerged / open | **TJ-14B is in its THIRD review after two NO-GO fix rounds** (Mentor question registry, tip `cfe137d1`). **TJ-4 and TJ-16 testers are starting off the integration tip** - both were unblocked by the TJ-10 merge (TJ-16's context snapshot shipped with it). Unbuilt: TJ-12, TJ-5, TJ-6, TJ-7, TJ-8, plus the follow-up **TJ-14C** - join a quick like's answered `opportunity_events` row into the like cohort, which wakes TJ-14B's dormant `quick_like_followup` kind (plan.md 12.4 / 12.5). Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` is ~53.0 KB against its ~45 KB rule (TJ-10's bullet was part-paid by trims in the same section), `CHANGELOG.md` Recent changes holds 09-07..09-20, `memory/projects/tradingbotv3.md` is over its 15,000-char consolidation line. Two known suite flakes, unowned: `test_ws_10a_scan_freshness` and `test_g7_speed_pass`, each green standalone. |
| Next action | **Finish TJ-14B's third review, then a desk-down fast-forward of `main` to the integration tip and a restart**, never while a `run_ai_jobs` process is running. A full suite is only a baseline with the nightly AI lock FREE - inside the 22:00-06:00 PDT window about 42 tests that call the real `runner.run_slots` FAIL rather than skip (`docs/AGENT_TEAM.md`, "The nightly AI lock"). TJ-4 and TJ-16 run in parallel off the integration tip. Gates owed, one line each: **#155 (TJ-10)**, which cannot be judged until a `day_review/bars/<date>.parquet` session tape EXISTS - the live home has no `day_review\bars\` folder yet; **#158**'s TJ-13B clause (the TJ-13A half was read 09-20 from the ledger); **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**, which needs #146/#152's bars back-fill first; **#159** (TJ-14A's first clause) and **#160** (TJ-15's first nightly `miss_contrast`). |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one, then run it again with `--apply --i-am-the-trader` and read those same two lines; keep the printed backup a week; start the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means some rows moved, the import rule stayed off and nothing is broken - run it again. To undo: copy the backup back. Both commands and every step: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait a few minutes and type it again. Nothing to check first, and no Task Manager. (3) **Open Day Review once and pick Friday 2026-09-18** so its 5-minute tape back-fills: the desk that ran Friday predated the bars writer, so `C:\TradingBotData\day_review\bars\` does not exist yet and every rest-of-day grade reads unmeasured until it does. Then, as before: after 2026-09-20 15:35 PDT restarts are yours again, and the click-gates (#154, #155, #159, #161 and every `staged-pass`) are read on real sessions. From Monday the 09:00 Mentor card will insist you label Friday's trades. (4) **The "flat" band is my number, not yours.** A day that moves less than a quarter of SPY's normal daily range counts as flat, so calling "up" on a day that went nowhere is not wrong. I picked a quarter because the packet gave no number. Change it whenever you like - but never from one day's result. |
| Last verified baseline | **Full suite on the merge `57b44ca9`: 9,536 passed, 14 skipped, 72 subtests, 2 failed - the known flake `test_ws_10a_scan_freshness` and `test_fixture_contract` (the TJ-10 text fixture sat where the market-data provenance contract applies; moved in `3e52d94f` and re-run green with the AI lock free, 2026-09-20)**. Before it: **9,396 passed, 14 skipped, 72 subtests, 1 failed - the known unowned flake `test_ws_10a_scan_freshness` - on `86c64f96` (everything live + TJ-13B + TJ-9Q), nightly AI lock free, 2026-09-20 06:57 PDT; ruff clean, smoke 7/7, source selftest 92/92**. What IS verified: TJ-13B's fix round with the real lock free (340 AI-jobs tests green) and TJ-9Q's two reviews by reproduction on byte-exact copies of the live journal. |
| Frozen exe | **Rebuilt and verified 2026-09-16: source 87/87 and frozen 87/87**, with the required `(frozen)` stamp. The production desk runs from SOURCE. |
| Desk | **UP since 2026-09-20 07:14 PDT on `acaa13c0`** (TJ-13B + TJ-9Q), started clean after the 06:11 close; `last good` is `acaa13c0` (restart log). The next fast-forward takes TJ-10; IB is not logged in (TWS is the trader's). |
