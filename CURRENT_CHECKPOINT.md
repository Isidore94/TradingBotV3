...
| Field | Current value |
|---|---|
| Roadmap phase | **Phase 0.5 R4 addition — recovered Alert Center contract, code paused at fenced-file scope check** (trader redirect 2026-08-16). R3's existing §4.3.5 decision remains paused; R1 + R1.1 + R2 + R7 + R8 are built and their live gates remain owed |
| **Active packet** | **R4 "NOT TODAY" ARMED-ALERT SURVIVAL** (`docs/DESK_CHART_UNIFICATION_PLAN.md` §6.1). Historical packet `docs/ALERT_CENTER_QUALITY_PACKET.md` was recovered byte-for-byte from `671ee57`, read fully, classified as historical, and reconciled: R2 already absorbed auto-pick provenance/scoped removal; R4 now owns the still-live armed-alert rule, labeled feed Focus action, and display-only repetition/open-burst outcomes. No alert code has been edited yet |
...
| Scope | Trader-approved fenced edit plan: in `alert_center_panel.py`, stop `_ignore_alert_symbol` deleting `_chart_watches`; exempt `CHART_WATCH_TAG` from `add_alert`'s ignored-symbol return; add deterministic tests. Anything beyond those exact edits is ask-first. Documentation recovery/reconciliation was explicitly required before implementation |
| State | **Documentation recovery complete; deterministic baseline unchanged at 3375 passed / 19 subtests, exit 0.** Source recon found two additional live suppressors not named in the exact approval: `_poll_d1_level_watches` and `_poll_d1_event_watches` both skip ignored symbols, so those trader-armed watches cannot actually fire after "Not today" even if `add_alert` is exempted. No fenced source edit has been made. Main desk checkout and live runtime remain untouched |
| Next action | Trader decision required: authorize removing the ignored-symbol deferrals from both D1 watch pollers as part of this repair, or constrain the implementation to the two named edits while accepting that D1 level/event watches remain deferred for the day. After that decision, write the regression first, make only the authorized changes, run the full gate, reconcile docs, commit/push, and then retire the recovered origin branch |
