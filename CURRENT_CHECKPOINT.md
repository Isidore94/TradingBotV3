...
| **Active packet** | **R3 SWING QUALITY AND FEEDBACK — CHARACTERIZATION FIXTURES BUILT, SHADOW CLASSIFIER NEXT** (`docs/SWING_QUALITY_AND_FEEDBACK_PLAN.md`). `tests/fixtures/r3_swing_quality_v1.json` carries threshold mutations, trending/gap/missing/forming-completed cases and the live-membership baseline. The demotion classifier must ship shadow-only: emit `would_demote` evidence and change no live row membership, ordering, visibility or tier. The full shadow week remains owed. R4–R6 follow only after R3's deterministic build and governance close-out |
...
| Next action | Commit and push the green fixture-only milestone, then build the shadow-only classifier and additive report/table evidence before the pre-close honesty bundle and dislike-reason loop. Monday's one-merge sequence remains owed and unchanged |
