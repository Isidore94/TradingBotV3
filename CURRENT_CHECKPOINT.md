...
| Field | Current value |
|---|---|
| Roadmap phase | **Phase 0.5 R4 addition — recovered §6.1 armed-alert survival BUILT and green** (trader redirect 2026-08-16). R3's existing §4.3.5 decision remains paused; R1 + R1.1 + R2 + R7 + R8 are built and their live gates remain owed |
| **Active packet** | **R4 "NOT TODAY" ARMED-ALERT SURVIVAL — DETERMINISTICALLY COMPLETE, LIVE PROOF OWED** (`docs/DESK_CHART_UNIFICATION_PLAN.md` §6.1). Historical packet `docs/ALERT_CENTER_QUALITY_PACKET.md` was recovered byte-for-byte from `671ee57`, read fully, classified as historical, and reconciled. The four explicitly approved Alert Center edits are built; R4 now also retains the still-unbuilt labeled feed Focus action and display-only repetition/open-burst outcomes |
...
| Scope | Trader-approved fenced edit plan now names exactly four changes in `alert_center_panel.py`: stop `_ignore_alert_symbol` deleting `_chart_watches`; exempt `CHART_WATCH_TAG` from `add_alert`'s ignored-symbol return; remove the ignored-symbol deferrals from `_poll_d1_level_watches` and `_poll_d1_event_watches`; add seam tests in both directions. Anything beyond those edits is ask-first |
| State | **R4 §6.1 green: 3377 passed / 19 subtests, exit 0; focused Alert Center/arm/watch gate 80 passed; smoke 7/7, all exit 0.** Ignored trader-armed chart watches survive; trader-armed D1 level/event watches evaluate, feed and sound; the ignored automatic Focus path remains silent. Historical packet blob identity was verified between `671ee57` and pushed recovery commit `7241520`, then remote `origin/claude/wishlist-integration-analysis-2ixvy0` was deleted. Main desk checkout and live runtime remain untouched. Frozen selftest 49/49 remains the last applicable frozen gate; no rebuild trigger was touched |
| Next action | Commit/push the isolated §6.1 behavior change, then return to R3's existing §4.3.5 seam/defer decision. The §6.1 real-session two-direction proof remains owed; do not claim it from deterministic tests |
