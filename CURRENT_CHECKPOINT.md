...
| # | Gate | Owed by |
|---|---|---|
| 56 | **The lake is de-duplicated and rebuilt (S1 repair)** - `research_warehouse.cli dedupe --dataset bar_m5 --apply` run once with no build running; the manifest shows two COMPACT lines with `rows_dropped` 10,198,313 (`month=2026-08`) and 332,603 (`month=2026-09`); then `bar_derived`, `feature_snapshot_intraday` and the outcome datasets for those months are REBUILT, because they were computed from the duplicated rows | 2026-09-03 evening entry |
