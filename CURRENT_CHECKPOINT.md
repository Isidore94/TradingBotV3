...
| Latest work | **TJ-10 (`57b44ca9`, fix `3e52d94f`) and TJ-14B (`161e905c`) are LIVE since 2026-09-20 11:48 PDT**, on TJ-13B + TJ-9Q (07:14); `main` `16ec284e` is what the desk runs. TJ-9Q's import switch ships OFF, so nothing in the live journal has moved; the `--apply` is owed and is the trader's act; the 27B has still never run. **TJ-16 (`c4a760e5`) and TJ-4 (`d929e34f`) are MERGED on the integration branch and NOT live** until the next fast-forward. TJ-16: the prediction ledger beside three naive baselines on the SAME stamps, the deterministic `prediction_contrast` slot, and the `observation_tags` tagger whose payload holds no outcome and whose verifier re-checks its own bounds. TJ-4: the PURE hash-stable day pack, the overnight day story whose every verdict must EQUAL the measured read row and whose bounds come FROM THE PACK, the rolling D1 view, and a night that SWEEPS a daytime Redo's markers. |
| Working branch | **`lead/p033-integration2` tip `d929e34f`, on top of `main` `16ec284e`**, ahead by the TJ-16 merge `c4a760e5`, the seventh slot-order pin `4f131b4c` and the TJ-4 merge `d929e34f`. Nothing is pushed. |
| Unmerged / open | Not started: **TJ-12** (UNBLOCKED by TJ-4; tester starting now), then TJ-5 → TJ-6 → TJ-7, TJ-8, plus **TJ-14C** - give `quick_like_followup` a reader, then wake it (plan.md 12.4). Free now: TJ-14B's `grader_gap` kind can be woken (TJ-10's field is on this branch). **Slot-order pins: SEVEN files** (plan.md 12.3); only `day_review_narration` and `observation_tags` may sit between `ai_summary` and `ticker_briefs`. Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` ~56 KB against its ~45 KB rule; `CHANGELOG.md` Recent changes holds 09-07..09-20; `memory/projects/tradingbotv3.md` over its 15,000-char line. Two known unowned flakes, green standalone: `test_ws_10a_scan_freshness`, `test_g7_speed_pass`. |
| Next action | **Run the full suite on the integration tip `d929e34f`.** `main` moves only when the TRADER says so and only with the desk down - the lead's restart grant ended 15:35. Never while a `run_ai_jobs` process runs; a baseline needs the nightly AI lock FREE (inside 22:00-06:00 PDT ~42 tests calling the real `runner.run_slots` FAIL rather than skip - `docs/AGENT_TEAM.md`). Gates owed, one line each: **#155 (TJ-10)**, now judgeable - the `day_review/bars/` tape exists; **#158**'s TJ-13B clause; **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**; **#148 (TJ-4)** - the morning-after story; printed verdicts equal to the ledger's by `read_id`; a DAY Redo that leaves a marker, starts no process, and is narrated and cleared next night; **#159** (TJ-14A + TJ-14B); **#160** (TJ-15's first `miss_contrast`); **#163** (TJ-14B's pull tally, the three reserved pulls, the one-click overnight question); **#164-#167 (TJ-16)** - the contrast pack's hour/environment tables off real clicks, the tagger's first grounded night, `tag:` features the night after, `entries_written_after` present and 0 on a live-answered session. |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one, run it again with `--apply --i-am-the-trader`, keep the printed backup a week, then start the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means rows moved, the rule stayed off and nothing is broken - run it again; to undo, copy the backup back. Commands: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait and type it again. (3) **Click a prediction on the Mentor card** - What I expect, a direction and How sure. No journal row carries one yet, so the first day story has nothing to grade and the were-you-right list stays empty until you do. From Monday the 09:00 card also insists you label Friday's trades, and may add up to three small questions. (4) **The "flat" band is my number, not yours.** A day that moves less than a quarter of SPY's normal daily range counts as flat, so calling "up" on a day that went nowhere is not wrong. I picked a quarter because the packet gave none; change it whenever you like, but never from one day's result. Done 16:06 PDT: the `day_review\bars\` tape back-filled 19 sessions through 2026-09-18, so #155 and #147 can now be read. |
| Last verified baseline | **9,622 passed, 14 skipped, 72 subtests, 0 failed on the integration tip `161e905c`** (live + TJ-10 + TJ-14B), AI lock free, 11:44 PDT; ruff clean, smoke 7/7, selftest **93/93**. **TJ-16 on top (`eca27e6f`): 9,670 passed, 1 failed = the known flake `test_g7_speed_pass` (green alone), 12:58 PDT.** **TJ-4 on top (`d929e34f`): 9,807 passed, 14 skipped, 72 subtests, 0 failed, 16:52 PDT; ruff clean, smoke 7/7**; source selftest on `d929e34f` is **95/95** (+`day_review_pack`, +`ai_jobs.day_review_narration`). |
| Frozen exe | **Rebuilt and verified 2026-09-16: source 87/87 and frozen 87/87** with the `(frozen)` stamp. The desk runs from SOURCE. |
