...
| | |
|---|---|
| Latest work | **2026-09-08 ~23:30 PT: SN5 / SN6 - the M5 scanner BREATHES and scans the trader's picks FIRST** (trader: *"Anything we can get done from wishlist.md that's quick and cheap?"* then *"Go"*): `BounceBot._breathe` waits `SYMBOL_BREATH_SECONDS` (0.02) on the stop event after each symbol in the fast lane and both sweep loops; `BounceBot._fast_lane_order` puts the trader's own Focus names before the auto-adopted ones (`focus_picks.load_auto_pick_symbols`, one marker reader shared with the store), set unchanged. Pacing and order only; built by the lead directly on `main` under the trader's usage cap; gate #92 owed. SN1-SN4 stay in WISHLIST. **Earlier: ** **2026-09-08 ~22:00 PT: the point system is GRADED and can correct itself on the trader's word** - `scripts/setup_points_evidence.py`: an append-only log of every ranked row's parts, terciles-by-total joined to the tracker's own outcomes at 5 sessions (lift = top third minus bottom third, refused under the n floor), a per-part lift, and a proposed multiplier file the desk applies only when `... > Points: learned weights` is on (default OFF); the grade sentence on the setups status row; gate #91 owed. **Earlier ~21:00 PT: the setups table can be ranked by a POINT system** (trader: *"ranked instead by a point system ... its setup ... nearby S/R ... RS/RW ... a recent bounce"*) - `scripts/setup_points.py` (pure, four named parts from fields already on the row), a `Points` column with the parts in its tooltip, a **Points** checkbox on the setups strip (`rank_setups_by_points`, default OFF, reorders and never hides); built by the lead directly on `main` under the trader's usage cap, 7291 passed, exit 0; gate #90 owed at the next restart (dated entry below). **Earlier: ** **2026-09-07 ~18:35 PT: the Desk's Strength window is ONE flat scrolling page** (trader: *"the strength tab is unusable there's like 2 tabs and they get no space each"*) - `ui/widgets/strength_page.py` replaces the two collapsible sections; Focus strength, the auto RS/RW board, the RRS snapshot with its scopes stacked, then the M5 Strength Board, every block sized to its content; built by the lead on `claude/strength-page`, 7280 passed, merged to `main`; gate #89 owed at the next restart (dated entry below). **Earlier, ~15:10 PT: Phase 0.22's G lane is COMPLETE on `main` - G7 (the speed pass: first load on first show for five Research children, the Setup Tracker refresh on a worker re-fitting only changed tables, Market Journal charts on demand, the bench's own overhead fixed; reviewer GO; advisories fixed; gate #88) merged last, after G0, G1, G2a, G2b, G3, G3b, G4, G4b and G5.** **2026-09-07 later: G7 fix round (same branch) - four small items, all proven RED first: a `refresh()` asked for inside the worker's teardown window was DROPPED (now restarted on the worker's own `finished` signal), every `refresh()` leaked a `QThread` (now `deleteLater`'d on `finished`), `PriceAlertsPanel._save_table` could write an empty table over the store before the first show (now refuses), and one stale comment reworded. 21/21 green in `tests/test_g7_speed_pass.py`.** **~16:20 PT: G7 (the speed pass - the bench stops taxing its own settle, five Research children read on their FIRST SHOW so Research's first paint loads ONE child not nine, the Setup Tracker's refresh runs on a `ReadWorker` and re-fits only the tables whose export changed, the Market Journal's four charts are built on the first capture) BUILT and green on `claude/g7-speed-pass`; live gate #88 owed. Re-measured over a staged copy at 3456x2160: `research.construct` sync p95 3,025 -> 533 ms, `setup_tracker.refresh` 577 -> 0.2 ms, `market_journal.construct` p50 58 -> 10 ms, the layout-fit table byte-identical; the deferred first loads show up against the tab that asks for them. Fifteen tester tests written RED first, all eighteen green. In the same pass `CLAUDE.md` 51.7 -> 48.5 KB, sixteen bullets' story moved verbatim to `docs/DESK_INTERNALS.md`.** **Earlier 2026-09-07 (each with its own dated entry below): G5 Research > Results BUILT, reviewed NO-GO and FIXED (gate #87); G2b MERGED (#85); G4b BUILT (#86); G2a MERGED.** **~03:00 PT: the Setup Tracker review is COMPLETE on `main` at `c73f5325`** - ST6 and ST7 merged on top of ST1-ST5 and the G lane's G4/G0/G3/G1/G3b; ST6 is the ONE deterministic evidence snapshot plus the Working-lately strip and the priority switch, ST7 makes the trader's three scoring decisions the tracker's DEFAULTS (decision 0019). |
| Working branch | `main` (the Strength page commit fast-forwarded on top of `7991a9d2`), checked out in the desk checkout, pushed to `origin/main`. The ST lane, the G lane and the Strength page are all merged; no packet branch is open. |
| Unmerged / open | **G lane: nothing unmerged.** INCIDENT 2026-09-07 ~15:20 PT, resolved: the G7 fix-round builder's worktree removal failed (`Permission denied`), the leftover directory kept its `.git` file, and git commands run `-C` that directory fell through to the DESK CHECKOUT - the reflog shows `checkout: moving from main to claude/g7-speed-pass` while the desk was running. The tree was the intended final state (identical to `lead/merge-g`), so nothing on the desk changed; `main` was re-pointed to `d1e0b279` and checked out with identical trees, the leftover directory deleted, the rule recorded in `docs/AGENT_TEAM.md`'s spirit: `rm -rf` a failed worktree directory before any `git -C` on it, and check `git branch --show-current` in the desk checkout before every fast-forward. Follow-ons queued (not authorized as packets yet): the Day-trade Tracker's `reload_from_disk()` still ~450 ms on the Qt thread at its FIRST click (G7 moved when, not where); the Journal packet (the Trades splitter opens 39/61 against its declared 3:2; the blank space above the table); the two order-dependent Qt flakes (`test_chart_snapshot` backfill, `test_qt_alert_center` auto-refresh); CLAUDE.md at 48.5 KB against ~45 (the next docs pass). **G7 is BUILT and green on `claude/g7-speed-pass`, awaiting review and merge** (the bench's settle probe, five Research children reading on their first show, the Setup Tracker's refresh on a `ReadWorker` with a per-table memo, the Market Journal's charts on demand; no detector, score, alert or evidence writer, and the layout-fit table is byte-identical before and after). **G5 is BUILT, reviewed and fix-rounded on `claude/g5-research-results`**, and `claude/g4b-setup-tracker-detail` is BUILT and green; both await review and merge. G lane still queued: the Journal packet the G2a tester surfaced (the Trades splitter opens 39/61 against its declared 3:2; the blank space above the table). Open from before: `claude/s1-quick-verbs`, the 2026-09-03 lake incident call, the lock-file sweep packet, the `test_chart_snapshot` cooldown flake, and the order-dependent `test_qt_journal_panel::test_migration_failure_stays_visible_instead_of_claiming_no_accounts` flake. **One packet owed:** 100 of 1,980 cached daily-bar files end in a FORMING candle on seven sessions and the replay reads them. **`CLAUDE.md`'s trim is STARTED, not finished** (51.7 -> 48.5 KB, still ~3 KB over the rule). **One G7 advisory:** the Day-trade Tracker's `reload_from_disk()` is still 448 ms synchronous on the Qt thread - G7 moved when it runs, not where. |
| Next action | **Nothing owed from the trader tonight: the desk was restarted 20:45 PT on `becd1923`.** Gate #92 is read on the next live day (2026-09-09) from `thread_cpu.jsonl` / `ui_stalls.jsonl`; gates #89-#91 are on screen now. **Superseded:** **Restart the desk when convenient: SN5/SN6 (gate #92), the Points column (gate #90) and the Strength page (gate #89) all land then.** **Restart the desk when convenient: the Points column and checkbox (gate #90) and the Strength page (gate #89) both land then.** Then, if the trader wants it, the AWAY digest's swing order by points (ask first: it ranks by the Wilson bound today). **Restart the desk to put the Strength page on screen (gate #89) - the running desk (pid 22256) is on `f787f10f`, one commit behind.** **The desk is LIVE on `main` `f787f10f` since 17:47 PT 2026-09-07 (see Desk), so ST1-ST7 and the whole G lane are on screen; gates #80-#88 are now checkable by the trader.** Then Tuesday 2026-09-08 (Monday is Labor Day): the first persisted tracker write at 13:00 PT restates the tracker under the three new defaults (gate #84: the `Setup tracker policies:` log line, every record stamped, `n_pending` ~902, the first D1 scan still writing NONZERO score deltas), and with it gates #70-#76, #78's surviving clause, #79 and #83 (the strip on the first DESK session reads `no evidence` for swing_trade_r until that write, then `awaiting persistence (1 of 2)`); #77 is the artifact clause only. **Tuesday morning: the first `journal_enrichment` row that is not `refused` (gate #63).** Then the two new packets above. |
| Trader actions owed | **None tonight - the lead restarted the desk at 20:45 PT 2026-09-08 (SN5/SN6, the Points column and the Strength page are all live).** Look at gates #89-#92 on the running desk when convenient. **Earlier:** **Restart the desk when convenient to see the flat Strength window (gate #89).** **Previous restart done 17:47 PT 2026-09-07 (lead, on the trader's "restart the desk").** Look at gates #85-#88 on the running desk when convenient. The three scoring decisions are TAKEN and built (decision 0019: pending stays pending - trader; a re-entry after a closed first attempt is its own trade - trader; `gap_aware_v2` + `prior_session_v2` - lead's discretion, both pure correctness fixes; rollback is one switch by name). Expect the Setup Tracker's numbers to move on Tuesday's write: 2,249 -> 2,712 episodes, 61.6% -> 72.0% favorable on the recent family rows - a restatement, not a defect. Still: the 26 provisional tags await review in Weekend Prep; record planned risk on new trades (0 of 172 have it); the 2026-09-03 lake incident call. |
| Last verified baseline | **`main` after the SN5/SN6 commit, 2026-09-08 ~23:30 PT, desk checkout: `pytest tests/ -q` with NOTHING DESELECTED: 7312 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0 (a first run failed one old fast-lane stub test in test_bounce_learning.py; the stub was given the two new helpers and the rerun is green); ruff clean; smoke 7/7.** Previous: **`main` after the points evidence commit, 2026-09-08 ~22:00 PT, desk checkout: `pytest tests/ -q` with NOTHING DESELECTED: 7299 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0; ruff clean; smoke 7/7.** Previous: `main` `4cd16a24`, 7291 passed, 3 skipped, 72 subtests, exit 0. Previous: **`claude/strength-page` (the tree merged to `main`), 2026-09-07 ~18:30 PT, in its worktree: `pytest tests/ -q` with NOTHING DESELECTED: 7280 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0, 7 min 34 s; ruff clean; smoke 7/7; selftest 74/74.** Previous: **`lead/merge-g` at `cbd38239` (G7 - the speed pass - with its advisory fix round, fast-forwarded on top of G5's `a00f296d`), 2026-09-07 ~15:10 PT, in the scratch merge worktree, nightly AI lock FREE (probed): `pytest tests/ -q` with NOTHING DESELECTED: 7276 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0, 7 min 28 s; smoke 7/7; selftest 74/74; ruff clean.** Previous: `main` `a00f296d`, 7255 passed. |
...
| Desk | **RESTARTED 20:45 PT 2026-09-08 on `main` at `becd1923` (SN5/SN6 live; trampoline pid 22916, python pid 17240) on the trader's "You restart the desk"**: selftest 74/74 on `main` first; the old desk (trampoline 19696 / python 17048, started 15:14:47 PT by the trader on `98d0ec8b`, so gates #89-#91 were ALREADY on screen since then - the earlier "pid 22256 / f787f10f" note below was stale) closed through `CloseMainWindow` in ~1 s with both writer leases released; `trading_desk.cmd` is not used from a non-interactive shell (see 09-05/09-07), so the lead started `.venv\Scripts\python.exe launch_gui.py` directly with stdout/stderr under the session scratchpad ("desk slot held (mutex: held, file lock: held)"); IB connected at 20:45:56, quiet hours, scanning paused, after-close jobs started, 714 MB at 30 s. The IB `VOL-NYSE` / `AD-NYSE` `no data` errors and the `$VOLD` contract line are identical to the 15:15 start - not new. **Previous:** **Running on `f787f10f`, ONE commit behind `main` (the Strength page lands at the next restart).** **RESTARTED 17:47 PT 2026-09-07 on `main` at `f787f10f` (pid 22256, trampoline 19528) on the trader's "get us to the latest update branch and then restart the desk"**: selftest 74/74 on `main` first; the old desk (pid 15080, `6d9f4b43` from 09:01, the Labor Day session) closed through `CloseMainWindow` in 3 s with both writer leases released; `trading_desk.cmd` from this non-interactive shell again left no process, so the lead started `.venv\Scripts\python.exe launch_gui.py` directly (stdout/stderr under the session scratchpad, "desk slot held"); IB connected at 17:48:03, quiet hours, Auto Pilot ON from saved state. **First-3-minute GUI stall record: 7 stalls / 3.4 s blocked (longest 1.8 s at `app.py:1344`) against this morning's 37 / 70.2 s on `6d9f4b43` (27 s of it startup at `app.py:1339`, 34 s BounceBot's session-open connect at `legacy.py:5465`, which quiet hours skip) - gate #88's "the window appears sooner" half is consistent with this; the trader's on-screen check is still the gate.** Working set 738 MB at 70 s. Labor Day 2026-09-07 assessment (lead, chat): the 2x RAM was the SCAN CHILD holding the 1.1 GB tracker (first-scan catch-up 804 s, the 13:00 write's `tracker update+calibrate` 1,517 s - the FIRST persisted tracker write since 2026-08-18, every 13:00 write between was skipped by the purity gate the M3 fix removed), the desk itself peaked at 1.8 GB; the network was Yahoo bars in and two ~125 MB `bronze_setup_tracker` snapshots out to `\MINI-PC` (192.168.0.223, a separate box); and the holiday made `TechnicalIntegrityMonitor._ensure_session` flip between the clock's calendar date and the bars' Friday date on every alternation, streaming the 770 MB events log hundreds of times (532 GB of reads; the same flip exists 06:00-06:35 PT every morning, small) - a defect noted, no packet written. Previous restart: 21:18 PT 2026-09-05 on `e1a2de15` (pid 9036): the old desk (pid 29260, `a6fb1a8d`) closed cleanly through `CloseMainWindow` (both writer leases released in the log), and two launches through `trading_desk.cmd` from a hidden `cmd /c` left no process, so the lead started `.venv\Scripts\python.exe launch_gui.py` directly with stdout/stderr captured under the session scratchpad (`desk_stdout.log`: "desk slot held"); `trading_bot.log` shows Auto Pilot ON from saved state, weekend quiet hours, IB 502 (TWS off on a Saturday, expected), 668 MB at 60 s. N1-N3 and M1-M5 are LIVE on this desk. The heartbeat file is not refreshed on a weekend (still pid 600 / 09-04 14:11), as on the previous restart. |
