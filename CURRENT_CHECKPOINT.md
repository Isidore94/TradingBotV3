...
| | |
|---|---|
| Latest work | **2026-09-07: G5 (Research > Results - the landing page the trader may read: four populations never pooled, a pure view that computes no new statistic, the ST6 snapshot read through `cells_from_payload`) BUILT and green on `claude/g5-research-results`, `origin/main` `6d9f4b43` merged in; live gate #87 owed. Tester's nine red tests re-proven failing before the first edit (6 failed, 3 errors), eleven added.** **2026-09-07 ~08:40 PT: G2a (the Journal's Trades table and Weekend Prep's Tag Week tables name their text column - width-rule calls only) MERGED on top of `7e018c99`, lead-verified (four tests fail with the two panels reverted); G4b and G5 testers started on `7e018c99`.** **2026-09-07 ~03:00 PT (Labor Day): the Setup Tracker review is COMPLETE on `main` at `c73f5325` - ST6 and ST7 merged on top of ST1-ST5 and the G lane's G4/G0/G3/G1/G3b.** ST6 (V4): ONE deterministic evidence snapshot (`working_lately.build_snapshot`, built off the Qt thread by `working_lately_service`, persisted small with a deduplicated `leader_change_events.jsonl`, `LEADER_PERSISTENCE_SNAPSHOTS` 2), the Working-lately strip at the top of the M5 column, the priority switch (a VIEW over the arrival-ordered lists, never a stored sort, never a withheld row), the AWAY Recap's snapshot line and held x ran column; reviewer GO after one four-blocker round. ST7: the trader's three decisions (2026-09-06 21:15 PT) are the tracker's DEFAULTS - `first_actionable_v2`, `gap_aware_v2`, `prior_session_v2` (decision 0019); the v1 names stay reproducible by name; history is restated by design at the next persisted write; reviewer GO on code after one text round. Before that (2026-09-06): ST1-ST5 merged at `68762909` (see that entry), the Q4 spot-audit and three scoring questions decided by the morning session, and the layout lane's G packets. |
| Working branch | `claude/g5-research-results` (G5, built, unmerged, with `origin/main` `6d9f4b43` merged in). `main` after the G2a merge is the trunk; the ST lane is complete. |
| Unmerged / open | **G5 BUILT on `claude/g5-research-results`, awaiting review and merge** (adds `scripts/research_results.py`, `ui/panels/research_results_panel.py`, the Results tab in `research_panel.py` and ONE line in `app.py`; touches no detector, score, alert or evidence writer). G lane still queued: G2b (Setup Tracker tabs, the Desk Setups table, the AWAY tables), G4b (`claude/g4b-setup-tracker-detail`, tester running), G7 speed pass, then the Journal packet the G2a tester surfaced (the Trades splitter opens 39/61 against its declared 3:2). Open from before: `claude/s1-quick-verbs`, the 2026-09-03 lake incident call, the lock-file sweep packet, the `test_chart_snapshot` cooldown flake, and a NEW one seen in this packet's full run: `test_qt_journal_panel.py::test_migration_failure_stays_visible_instead_of_claiming_no_accounts` failed once in a whole-suite run (`account_button` read `No accounts`) and passes in isolation, in its own file and in a second whole-suite run - a race between the migration worker and the header's account read, unrelated to G5. **Two packets owed:** 100 of 1,980 cached daily-bar files end in a FORMING candle; CLAUDE.md is over its ~45 KB rule. |
| Next action | **Review and merge G5, then restart the desk (trader's call)** - ST1-ST7 and the G lane are on `main` and none of it is live until then. Gate #87 is the trader's first read after that restart: Research opens on Results, the four selections show four different populations, "not enough evidence" reads as a sentence. Then Tuesday 2026-09-08 (Monday is Labor Day): the first persisted tracker write at 13:00 PT restates the tracker under the three new defaults (gate #84), and with it gates #70-#76, #78's surviving clause, #79 and #83. **Tuesday morning: the first `journal_enrichment` row that is not `refused` (gate #63).** |
...
| Last verified baseline | **`claude/g5-research-results` at `71ec5442` (G5 built, `origin/main` `6d9f4b43` merged in), 2026-09-07 in the builder's worktree, nightly AI lock FREE (probed before and during): `pytest tests/ -q` with NOTHING DESELECTED: **7214 passed, 3 skipped, 72 subtests passed, ZERO failures and ZERO errors, pytest's own exit code 0** in 9:33; ruff `All checks passed`; smoke 7/7; `launch_gui.py --selftest` 74/74.** Two earlier whole-suite runs on this branch each found ONE real cross-test defect and both are fixed in the tip: (1) the tester's panel test leaves `research_results_selection` = `mine/day/all` in the session's `local_settings.json`, so every later `ResearchPanel` construction (`test_st6_service_and_surfaces`, `test_warehouse_readout_off_qt`) read the journal on its worker and CREATED the journal database, which is the state `test_qt_journal_panel::test_migration_failure_stays_visible_instead_of_claiming_no_accounts` exists to find the absence of - contained by one autouse fixture in `tests/conftest.py`; (2) that fixture's first version read the settings file in TEARDOWN and errored inside `test_ai_evidence_coverage::test_a_huge_file_is_never_read_whole`, which forbids `Path.read_text` for the length of its own body - it now compares a `stat()` stamp first and swallows every failure. Previous: `lead/merge-g` at the G2a merge on top of `main` `7e018c99`, 7192 passed, exit 0. |
...
9 passed with the build, and the builder ADDED eleven more
