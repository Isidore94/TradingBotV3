...
| Latest work | **2026-09-20: TJ-13B (`e54c8203`) and TJ-9Q (`86c64f96`) are MERGED into `lead/p033-integration2`** - `main` has NOT been fast-forwarded to them, so neither is live. TJ-13B measures the large local model before anything uses it: `--probe-model` is a COMMAND, a model LOAD, holding the AI-jobs lock by default and writing ONE `manual_test` row - and the 27B has still never run. TJ-9Q makes a Questrade fill say what it is and a sold put a SALE, with the import seams shipped OFF and `scripts/journal_reclassify.py` the only way stored rows move; the `--apply` is the trader's own act and is owed. Earlier: **TJ-15 + TJ-14A went live at 05:45 PDT** and the desk closed again at 06:11 (restart log). |
| Working branch | **`lead/p033-integration2` on top of `main` `9fa10e4b`**, ahead of it by the TJ-13B merge `e54c8203` and the TJ-9Q merge `86c64f96` over the wave-2 merges already on `main`. Nothing is pushed. |
| Unmerged / open | **In RE-REVIEW after a NO-GO fix round: TJ-10** (read grader, tip `14bcc894`) **and TJ-14B** (Mentor question registry, tip `05ec3d06`). Unbuilt: TJ-4, TJ-16, TJ-12, TJ-5, TJ-6, TJ-7, TJ-8, plus the NEW follow-up **TJ-14C** - join a quick like's answered `opportunity_events` row into the like cohort, which wakes TJ-14B's dormant `quick_like_followup` kind (plan.md 12.4 / 12.5). Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` is ~52.6 KB against its ~45 KB rule (both 09-20 doc commits are size-neutral or smaller), `CHANGELOG.md` Recent changes holds 09-07..09-20, `memory/projects/tradingbotv3.md` is over its 15,000-char consolidation line. Two known suite flakes, unowned: `test_ws_10a_scan_freshness` and `test_g7_speed_pass`, each green standalone. |
| Next action | **A clean full-suite baseline on the integration tip with the nightly AI lock FREE** - inside the 22:00-06:00 PDT window about 42 tests that call the real `runner.run_slots` FAIL rather than skip, so a run there is neither a baseline nor proof of a red branch (`docs/AGENT_TEAM.md`, "The nightly AI lock") - **then a desk-down fast-forward of `main` and a restart**, never while a `run_ai_jobs` process is running. Then wave 3: the TJ-10 and TJ-14B re-reviews. Gates owed, one line each: **#158**'s TJ-13B clause (the TJ-13A half was read 09-20 from the ledger); **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**, which needs #146/#152's bars back-fill first; **#159** (TJ-14A's first clause) and **#160** (TJ-15's first nightly `miss_contrast`). |
| Trader actions owed | **Three things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one - `refused:` and the SWITCH line - then run it again with `--apply --i-am-the-trader` and read those SAME two lines on that output; keep the printed backup a week; start the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means some rows were fixed, the new import rule stayed off and nothing is broken - look at the named position and run it again. To undo: copy the backup back. Both commands and every step: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts
un_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait a few minutes and type it again. Nothing to check first, and no Task Manager. (3) **Open Day Review once and pick Friday 2026-09-18** so its 5-minute tape back-fills: the desk that ran Friday predated the bars writer, so `C:\TradingBotData\day_reviewars\` does not exist yet and every rest-of-day grade reads unmeasured until it does. Then, as before: after 2026-09-20 15:35 PDT restarts are yours again, and the click-gates (#154, #155, #159, #161 and every `staged-pass`) are read on real sessions. From Monday the 09:00 Mentor card will insist you label Friday's trades. |
| Last verified baseline | **9,396 passed, 14 skipped, 72 subtests, 1 failed - the known unowned flake `test_ws_10a_scan_freshness` - on `86c64f96` (everything live + TJ-13B + TJ-9Q), nightly AI lock free, 2026-09-20 06:57 PDT; ruff clean, smoke 7/7, source selftest 92/92** - the integration tip `86c64f96` (TJ-13B + TJ-9Q over wave 2) has no full-suite count yet; it is owed with the nightly AI lock FREE. What IS verified: TJ-13B's fix round with the real lock free (340 AI-jobs tests green) and TJ-9Q's two reviews by reproduction on byte-exact copies of the live journal. Before this tip: 9,264 passed, 14 skipped, 72 subtests, 2 failed - exactly the two known unowned flakes - on `1f260ffa` (wave 1 + TJ-11F + TJ-3 + TJ-15 + TJ-14A), 2026-09-19 22:03 PDT, the run begun at 21:49 so no AI-lock test stood down; ruff clean, smoke 7/7. |
...
| Desk | **DOWN.** It ran clean on `38c270eb` (wave 1 + TJ-11F + TJ-3 + TJ-15 + TJ-14A) from 05:45 PDT until the window closed at 06:11:59 through its normal close path - most likely a person (restart log). The lead restarts it with the next fast-forward; IB is not logged in (TWS is the trader's). |
