...
| Desk | **DOWN since 2026-09-20 15:45:45 PDT** - a normal window close the lead did NOT make (the second unexplained one today, after 06:11); the restart grant ended 15:35, so **starting it is the TRADER's act** (`trading_desk.cmd`). `main` was fast-forwarded to the TJ-16 tip while it was down, so the next start runs TJ-10 + TJ-14B + TJ-16; `last good` is `16ec284e`. |
...
- 2026-09-20 15:45:45 the desk CLOSED through its normal shutdown path (`Away report writer lease released on shutdown`, no traceback) - NOT the lead, who was paused by the trader from 12:58 to 16:00; the second such close today. The grant ended 15:35, so the lead did not restart it. 16:1x (desk DOWN) `main` fast-forwarded `16ec284e` -> the TJ-16 docs tip; source selftest run; NO start - the next start is the trader's.
