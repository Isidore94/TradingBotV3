...
| | |
|---|---|
| Latest work | **2026-09-13 12:01 PT: the WISHLIST sweep is 21 of 26 packets built and verified on `claude/wishlist-sweep-2026-09-12` (gates #94-#114).** The per-item table is the dated entry "2026-09-12 - WISHLIST SWEEP" below - the resume point. Remaining: DR and WL (red tests pushed, builders not started), 10I and 10C (packets written), RP (lands last), then the docs pass. Deferred by lead decision: SN1, 5E, item 7's legacy.py stamp, item 9's hide option. |
...
| Next action | **Take over the sweep in a fresh session:** (1) `git status` clean on `claude/wishlist-sweep-2026-09-12` at ba22c0ff = `origin` = `lead/sweep-merge`; (2) recreate the lead merge worktree (`git worktree add <scratch>/wt-sweep lead/sweep-merge`); (3) spawn builders (Opus, own worktrees) on the tester tips - DR `origin/claude/ws-dr-daily-recap` 3e1c1973, WL `origin/claude/ws-wl-watchlist-tab` d9f44da2 - each told to `git checkout -b claude/<slug>-build origin/claude/<slug>` then merge the sweep branch, push after every green step, and hand off with the CHANGELOG paragraph and gate text (packets in `.claude/packets/WS-DR.md`, `WS-WL.md`, rules in `WS0_COMMON.md`); (4) then testers for `WS-10I.md` and `WS-10C.md`, builders after; (5) `WS-RP.md` last; (6) the docs pass from `.claude/packets/docs_pass_notes.md`; the lead's paste helper is `.claude/packets/sweep_docs.py` (run from the merge worktree: `inventory` / `recent` / `gate` / `status` / `landed`). Merge rule: merge into `lead/sweep-merge`, full suite with the AI lock probed and pytest's own exit line read from the log, then `git -C C:/Users/Aaron/TradingBotV3 merge --ff-only lead/sweep-merge` and push. Run at most three full suites at once (the writer-lock dir serialises them). Known order-dependent tests: `test_g7_speed_pass` (one), `test_ws_sx_star_x` (four when run alone), `test_ws_10a_scan_freshness::..._never_on_paint`, `test_ai_jobs_runner::test_a_weekend_firing...` - green alone. |
...
| Last verified baseline | **2026-09-13 12:00 PT (sweep batch 13, `lead/sweep-merge` = desk checkout ba22c0ff): full suite 7747 passed, 5 skipped, 72 subtests, zero failures, exit 0 (500 s); ruff clean.** Smoke 7/7 and selftest 80/80 were each builder's own proof (selftest 74 -> 80 across FC1/5D/TM); the lead did not rerun them on the merged tree. Every batch since 576dd672 was fast-forwarded only on a green suite except batch 4 (one slate pin, fixed in batch 5) and batch 10 (one order-dependent test, green alone). |
