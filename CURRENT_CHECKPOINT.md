...
| | |
|---|---|
| Agent settings | **2026-09-15, trader-directed: the project Astra model pin is removed.** Session choice selects the lead; helper settings remain. |
| Latest work | **TJ-10 (`57b44ca9`, fix `3e52d94f`) and TJ-14B (`161e905c`) are LIVE since 2026-09-20 11:48 PDT** (`main` at `cdad9533`), on top of TJ-13B and TJ-9Q (live since 07:14). TJ-9Q's import switch ships OFF, so nothing in the live journal has moved; the `--apply` is the trader's own act and is owed, and the 27B has still never run. **TJ-16 MERGED on the integration branch 2026-09-20** (`c4a760e5`): the prediction ledger beside three naive baselines measured on the SAME stamps, the deterministic `prediction_contrast` slot directly after `miss_contrast`, and the Stage 2 `observation_tags` tagger whose payload structurally holds no outcome and whose verifier re-checks the reply's own bounds - a schema is a grammar hint, never a guard. Readers only, no page yet, and **NOT live on `main` until the lead's next fast-forward**. |
| Working branch | **`lead/p033-integration2` tip `4f131b4c`, on top of `main` `739ce5a4`**, ahead by the TJ-10 merge `57b44ca9`, the fix `3e52d94f`, the TJ-14B merge `161e905c`, the TJ-16 merge `c4a760e5` and the seventh slot-order pin `4f131b4c`. Nothing is pushed. |
| Unmerged / open | **TJ-4 is in REVIEW round 3** off the integration tip. Not started: TJ-12, TJ-5, TJ-6, TJ-7, TJ-8, plus **TJ-14C** - give `quick_like_followup` a reader, then wake it (plan.md 12.4). Free now: TJ-14B's `grader_gap` kind can be woken, TJ-10's field being on this branch. **The slot-order pins are SEVEN files now** (plan.md 12.3): `tests/test_opt_in_evidence_scopes.py` is the seventh, and only `day_review_narration` and `observation_tags` may sit between `ai_summary` and `ticker_briefs`. Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` is ~54.4 KB against its ~45 KB rule, `CHANGELOG.md` Recent changes holds 09-07..09-20, `memory/projects/tradingbotv3.md` is over its 15,000-char line. Two known unowned flakes, each green standalone: `test_ws_10a_scan_freshness`, `test_g7_speed_pass`. |
| Next action | **Run the full suite on the integration tip `4f131b4c`, then a desk-down fast-forward of `main` to it and a restart** - never while a `run_ai_jobs` process is running. A full suite is only a baseline with the nightly AI lock FREE: inside 22:00-06:00 PDT about 42 tests calling the real `runner.run_slots` FAIL rather than skip (`docs/AGENT_TEAM.md`, "The nightly AI lock"). Gates owed, one line each: **#155 (TJ-10)**, unjudgeable until a `day_review/bars/<date>.parquet` tape EXISTS; **#158**'s TJ-13B clause; **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**, after #146/#152's bars back-fill; **#159** (TJ-14A + TJ-14B); **#160** (TJ-15's first nightly `miss_contrast`); **#163** (TJ-14B's pull tally, the three reserved pulls, the one-click overnight question); **#164-#167 (TJ-16)** - the contrast pack's hour and environment tables off real clicks, the tagger's first grounded night, `tag:` features the night after it, and `entries_written_after` present and 0 on a live-answered session. |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one, then run it again with `--apply --i-am-the-trader` and read those same two lines; keep the printed backup a week; start the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means rows moved, the import rule stayed off and nothing is broken - run it again. To undo: copy the backup back. Both commands: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait a few minutes and type it again. (3) **Open Day Review once and pick Friday 2026-09-18** so its 5-minute tape back-fills: Friday's desk predated the bars writer, so `C:\TradingBotData\day_review\bars\` does not exist yet and every rest-of-day grade reads unmeasured until it does. After 15:35 PDT restarts are yours again, and the click-gates (#154, #155, #159, #161, #163, #164 and every `staged-pass`) are read on real sessions. From Monday the 09:00 Mentor card will insist you label Friday's trades, and it may add up to three small questions. (4) **The "flat" band is my number, not yours.** A day that moves less than a quarter of SPY's normal daily range counts as flat, so calling "up" on a day that went nowhere is not wrong. I picked a quarter because the packet gave none. Change it whenever you like - but never from one day's result. |
| Last verified baseline | **9,622 passed, 14 skipped, 72 subtests, 0 failed on the integration tip `161e905c`** (everything live + TJ-10 + TJ-14B), nightly AI lock free, 2026-09-20 11:44 PDT; ruff clean, smoke 7/7, source selftest **93/93**. **TJ-16 on top (`eca27e6f`): 9,670 passed, 1 failed = the known flake `test_g7_speed_pass` (3 of 3 green alone), 12:58 PDT.** Before it: 9,396 passed + the one known flake (`test_ws_10a_scan_freshness`) on `86c64f96`. |
| Frozen exe | **Rebuilt and verified 2026-09-16: source 87/87 and frozen 87/87** with the `(frozen)` stamp. The desk runs from SOURCE. |
| Desk | **DOWN since 2026-09-20 15:45:45 PDT** - a normal window close the lead did NOT make (the second unexplained one today, after 06:11); the restart grant ended 15:35, so **starting it is the TRADER's act** (`trading_desk.cmd`). `main` is still `16ec284e` (TJ-10 + TJ-14B), so that is what the next start runs; **TJ-16 waits on `lead/p033-integration2`** until the trader says to fast-forward `main` (desk down first). |
