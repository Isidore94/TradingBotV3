...
| Next action | **Build PCT-2 (trendline break) from the spec's section 7: recon, tester, builder, reviewer, then merge and run every gate. After that, ask the trader what Daily Recap clean-up means before writing its packet.** Trader: restart the desk only when this plan branch is declared ready, then run gates #123-#129 plus the older combined live checks. Lead notes: `tests/test_ws_10a_scan_freshness.py::...never_on_paint` is a known order flake; the checkpoint and CHANGELOG are far over their size rules and an archive pass is owed. |
