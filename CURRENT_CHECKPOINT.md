...
| 2 FC1 forming candles in the daily-bar cache | `WS-FC1` / `claude/ws-fc1-forming-candles` | PACKET | Build the writer guard + manifest counts + `python -m master_avwap_lib.daily_bar_cache repair` (dry run default). `--apply` on the live cache stays the TRADER's action. legacy.py writer seam only - the trader's FC1 prompt is the recorded yes. |
...
| 5D watchlist intent events | `WS-5D` / `claude/ws-5d-watchlist-intent` | PACKET | Append-only dated add/remove event at the trader-edit seam; machine writers labelled; an external diff labelled `observed_external`. |
...
**Landed so far:** packets written for wave 1 (`.claude/packets/WS0_COMMON.md`, `WS-FC1`, `WS-EF1`, `WS-SX`, `WS-5A`, `WS-PT4`, `WS-AI1`, `WS-5D`, `WS-ENV`) and for SN4/SN3/SN2; testers spawned for FC1, EF1, SX, AI1 and builders for 5A, PT4 at ~15:30 PT 2026-09-12. Check `origin` for branches `claude/ws-*` before re-spawning anything.
