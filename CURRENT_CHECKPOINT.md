...
| Last verified baseline | **`claude/g5-research-results` at the G5 build with `origin/main` `6d9f4b43` merged in, 2026-09-07, in the builder's worktree, nightly AI lock FREE (probed): `pytest tests/ -q` with NOTHING DESELECTED: 7222 passed, 3 skipped, 72 subtests passed, exit 0; ruff clean; smoke 7/7; selftest 74/74.** The first run of the same suite on the same tree gave one failure - `test_qt_journal_panel.py::test_migration_failure_stays_visible_instead_of_claiming_no_accounts`, green in isolation, green in its own file, green on the re-run - recorded as a flake above, not as a baseline. Previous: `lead/merge-g` at the G2a merge on top of `main` `7e018c99`, 7192 passed, exit 0. |
