...
| # | Gate | Owed by |
|---|---|---|
| 65 | **The narration comes back, bounded (N3)** - the next overnight `setup_research` row in `ai_job_ledger.jsonl` reads `narrated K of N eligible cell(s)` (expect K around 64 and N around 619, and K rising only if the head shrinks), with a `.narration.json` beside **ONE** pack for the date, and the pack `.md` carrying "Narration covers K of N eligible cells ... N-K omitted for size" under a `## Narration` heading. **A row still reading `narration absent` is the news**, and its message now names the HEAD's size, so it says whether the head or the model's window is the problem. **This REPLACES gate #40's narration clause**; #40's other halves (one pack per date, non-empty `recipe_ids`, `built_by_commit`) are unchanged and were met on 2026-09-05 | 2026-09-05 N3 entry |
...
| 61 | **MET 01:26 PT 2026-09-05**: `band-coverage --month 2026-09 --recipe swing_house_v1` = 105 of 105 occurrences with all three required bands (100%), knowledge split PRINTED: `reconstructed` 105, `observed` 0 (as BD-100 predicts - the bridged bars are newer than the 14 hand anchors), `legacy` 0; August 2,401 of 2,437 (98.5%) `reconstructed`, 36 `none`; `path_kind` populated on the recomputed rows (`noTgt` 36 = the 36 anchorless rows on `plain_no_target`, every banded row on the target path). Every step ran from `scripts/` with the desk up and idle; the nightly AI runner was not touched. Original clause: **The band repair chain (packet Q2)** - after the next nightly build, on the trader's go: `rebuild-daily-features --from 2026-08-01 --to <today>` DRY RUN lists the August and September sessions; `--apply` then `recompute-outcomes --apply` then `band-coverage --month 2026-08` / `--month 2026-09` shows `swing_house_v1`'s required-band coverage above 90% for September with the observed / reconstructed / legacy split PRINTED, and `path_kind` populated on the recomputed rows. **The gate is the split being printed, not `observed` being non-zero**: the anchor choice keeps the NEWEST bar on or before the session regardless of knowledge, so once the bridge lands anchors with newer bars they displace the 14 hand-imported ones and `observed` can legitimately read 0. Rides with gate #59 and is measured by the same nightly | 2026-09-04 (~16:00 PT) Q2 entry |
| 60 | **Held is MEASURED held (Q1)** - one desk session where the Daytrade Tracker's Held columns render with the Measured column populated (`n_measured / n`), the status line names the window and its missing sessions, and `held_run_score.load_episodes()` on the live file reports the four measurement counts beside the merge-day 5,222 / 1,960 / 979 (0 pending after the close); `REASON_BREAK_TIME_UNKNOWN` fired on 0 live episodes at merge - the milestone rows bracket the window - so a non-zero count is news, not a defect | 2026-09-04 evening Q1 entry |
| 59 | **MET 01:24 PT 2026-09-05** (lead-run overnight on the trader's "run the lake rebuild"): the bridge was run STANDALONE from the scanner caches (no scan - weekend) and appended 3,678 rows for 1,847 symbols (`source=scanner_earnings_cache`, CSV 15 -> 3,693 lines); a by-hand `cli build --session-date 2026-09-04` ingested them (bronze `earnings_avwap_anchors` 3,678, `anchor_instance` 3,681 rows / 1,847 symbols, build 3 min); `rebuild-daily-features --from 2026-08-01 --to 2026-09-04 --apply` rewrote 25 sessions x 1,560 rows (18 files retired, 0 carried, 7 symbols/session `ANCHOR_BAR_NOT_IN_HISTORY`); `recompute-outcomes --apply` did 32/32 buckets in 41 min (6,897 occurrences, 36,849 rows rewritten, 103,899 unchanged, 424,806 `INSUFFICIENT_PATH_DATA` skips). **`swing_house_v1` August: TARGETED 376 / STOPPED 291 / OPEN 227 / AMBIGUOUS 29 - 56% of the 667 resolved, where 0/257 stood; September 2 / 0 / 42 / 2.** The control (fixed 1R/2R) reads 253 / 290 on the same occurrences. 1,514 of 2,437 August occurrences are `NOT_SIMULATED` (no outcome row at all) and 36 have no anchor - both named by `band-coverage`, neither counted as a win or a loss. Original clause: **The anchors CSV feeds the warehouse (earnings-anchor bridge)** - after the next scan, `trading_bot.log` carries `Earnings-anchor bridge: N cached anchor(s) ...` and `earnings_avwap_anchors.csv` holds a row per scanned symbol (~1,100+ tickers, ~2,200 rows, `source=scanner_earnings_cache`); after the next nightly build, `anchor_instance` holds ~2,200 rows (current + previous per symbol) and `feature_snapshot_daily` carries non-null `avwape_*` bands for those symbols' sessions; then `recompute-outcomes` (BD-98, `force`) re-grades `swing_house_v1` and the 0/257 reads as a real rate | 2026-09-04 14:30 entry |
...
| 40 | **The narration fits (R3)** - **the narration clause is SUPERSEDED by gate #65 (N3, 2026-09-05)**: sending every eligible cell stopped being possible at 619 cells / 658,292 chars, so "the view is still too large" is no longer the question and the view now selects. What remains owed under #40: one overnight `setup_research` run that publishes **exactly ONE pack** for the date with a `.narration.json` beside it, carrying `built_by_commit` and a non-empty `recipe_ids`. **That half was MET on 2026-09-05** (one pack, `built_by_commit 7f2273d3`, 102 recipe ids) | 2026-09-02 R3 entry, superseded by the 2026-09-05 N3 entry |
...
**Branch `claude/n3-research-narration-bounded`, off `main` (`e7b12ebe`), tester's red
tests at `9d41e454`.** Trader: *"Go ahead and build these fixes out"*. Live gate #65,
which replaces gate #40's narration clause.

**The problem, from the ledger.** `setup_research` has published no narration for four
nights: 09-02 273,622 chars / 47 cells (the server sheared at 32k), 09-03 82,192 vs
78,119 / 69, 09-04 143,636 / 128, 09-05 **658,292 vs 78,119 / 619 cells** - gate #59's
lake recompute landed 141,299 recipe outcomes overnight, up from 23,802. R3 had already
cut the view down from the pack and stated the repeated prose once; what remained is
that it carried EVERY eligible cell, and the grid grows. **"Raise the budget" is not a
fix** - no 64k-context model reaches 658k chars.

**Select, then fill.** The head (everything that is not a cell - gate, coverage,
evidence shape, excluded families, hoisted conventions) is encoded first: **11,084
chars** on the live 09-04 pack. Eligible policy cells are then added in order until the
next would cross the budget, and the after-like ELIGIBLE cells (P10 C3, untouched)
follow under the same rule from their own top-level `n_episodes`.

**The order is a SIZE rule, and gate #43 is why.** `stats.n` descending, then
`recipe_id`, `family`, `side`. `stats.n` is the outcome-row count the eligibility floor
itself gates on; the tie-breaks are identifiers, so the order is total. **No `mean_r`,
`win_rate`, `profit_factor`, `expectancy`, bootstrap bound or trimmed mean is in that
key** - gate #43 is a refusal, and ranking the model's input by result would hand it
the flattering half of a frozen grid and call it a selection. All 619 live cells carry
BOTH `stats.n` and `stats.n_episodes` and they are **equal on every one**; the
after-like grid is FLAT (no `stats` wrapper), so it has its own key. **BD-101.**

**Coverage is stated in three places** so no reader of any one of them mistakes 64
cells for the finding: `narrated` {K, of, `selected_by`, after-like K, of} in the view
and in the `.narration.json`; a `## Narration` line in the pack `.md`; `narrated K of N
eligible cell(s)` on the ledger reason. The markdown's line is computed BEFORE the file
is written - the cut needs no model - so **one pack and one `.md` per date still holds
(gate #40) and nothing published is rewritten**. A pack that is not narrated prints no
line rather than "0 of 0". The refusal narrows to "head + the FIRST cell does not fit"
and adds the head's size to the message.

**Dry run over a COPY of the live 2026-09-04 pack** (scratchpad copy; the DAS is
read-only to an agent; no model call): 658,292 -> **77,791 chars**, head 11,084,
...
**Verification.** Tester's seven in `tests/test_n3_narration_bounded.py` (pinned to a
fixture written by the pre-N3 nightly at `7f2273d3`) plus six builder-added in
`tests/test_n3_narration_bounded_edges.py`. **Fail-before-fix proven**: with
`scripts/ai_jobs/setup_research.py` stashed, 12 of 13 fail; the one that passes asserts
an ABSENCE. One pre-existing R3 test now opens the budget explicitly - its question is
what KIND of thing may be in the view, and it was otherwise measuring whichever budget
the running machine resolves (11,066 under the harness, 78,119 on the desk); no
assertion was weakened. **No packaging trigger**: no dependency, no asset, no new
top-level package, no dynamic import - `ai_jobs` is already collected.
