...
| Field | Current value |
|---|---|
| Roadmap phase | **Phase 0.5 R7 pre-flight fix pass — BUILT and green 2026-08-16**; further R3 work stayed paused for this explicit trader redirect. R1 + R1.1 + R2 + R7 + R8 are built and their live gates remain owed |
| **Active packet** | **R7 JOURNAL PRE-FLIGHT — ALL FIVE SPOT-CHECK FINDINGS CLOSED** (`docs/JOURNAL_RELIABILITY_AND_UX_PLAN.md`). Legacy Questrade order-id uids re-key before collapse with report counts; id-less fill hashes use only stable discriminators; GUI preparation reads the persisted schema; nightly refuses an existing pre-v3 DB until trader-present GUI preparation; execution assembly sorts by normalized identity; activities failure propagates FAILED. No live journal or broker path ran |
...
| Scope | Trader redirect 2026-08-16: *"PRE-FLIGHT FIXES ... do these before Packet R3"* — journal migration/identity/store/runner and the Qt preparation gate only. The file-scoped detector/scoring/alert ask-first rule did not apply. Existing R3 commits were preserved; no later packet work started |
| State | **R7 pre-flight green: 3375 passed / 19 subtests, exit 0**; focused journal gate **145 passed**, exit 0. Optional `python -m ruff` remains unavailable in the shared venv (`No module named ruff`); pytest is the required per-commit gate. Production journal code, live stores and brokers remain untouched. Prior smoke 7/7 and clean-cache frozen selftest 49/49 `(frozen)` remain the last applicable non-pytest gates; no frozen trigger was touched |
| Next action | Commit and push this R7 pre-flight fix pass. Then R3 returns to its existing §4.3.5 choice: explicitly defer same-slot volume normalization or authorize/specify a cached intraday same-slot source. Do not start R4 until that R3 governance decision is recorded. Monday's one-merge sequence remains owed and unchanged |
