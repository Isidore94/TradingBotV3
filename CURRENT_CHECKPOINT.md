...
| Trader actions owed | **Restart the desk** (running pid 29260 on `a6fb1a8d`; M1-M5 and N1-N3 reach it only at the next start). **Run `python -m ai_jobs.digest approve-audit --pack <d> --pack <d> --pack <d>`** after spot-auditing three packs, or enrichment stays silent (Q4). **Decide** the two scoring questions under Next action, and the 28-of-41 third population M3 named (baseline scenarios neither open nor closed). |
