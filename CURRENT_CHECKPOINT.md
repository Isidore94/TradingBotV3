...
| Latest work | **TJ-10 (`57b44ca9`, fix `3e52d94f`) and TJ-14B (`161e905c`) are LIVE since 2026-09-20 11:48 PDT**, on TJ-13B + TJ-9Q (07:14); `main` `16ec284e` is what the desk runs. TJ-9Q's import switch ships OFF, so nothing in the live journal has moved; its `--apply` and the 27B probe are the trader's (below). **MERGED on the integration branch and NOT live** until the next fast-forward: TJ-16 (`c4a760e5`), TJ-4 (`d929e34f`), the NaN bar fix (`ed62bd0a`), TJ-12 (`843a3f02`) and **TJ-5 (`1b9d77e0`)** - Week Review first in Weekend Prep: five day cards on ONE payload that computes nothing, the week pooled in ONE place, a Saturday-only story narrating only the week's own packs. |
| Working branch | **`lead/p033-integration2` tip `1b9d77e0`, on top of `main` `16ec284e`**, ahead by TJ-16, the seventh order pin `4f131b4c`, TJ-4, the NaN fix, TJ-12 and TJ-5 (hashes in Latest work). Nothing is pushed. |
| Unmerged / open | Not started: **TJ-6** (UNBLOCKED by TJ-5; tester starting now) → TJ-7, then TJ-8; plus two unscheduled follow-ups - **TJ-14C** (a reader for `quick_like_followup`, then wake it) and **TJ-12F** (bounded readers for the Focus-add and armed-alert lanes, then both in `DESK_ORIGIN_LANES_READ`; until then the Process line and the prompt NAME them unread). Free now: TJ-14B's `grader_gap` kind can be woken. **Slot-order pins: EIGHT files** (plan.md 12.3); only `day_review_narration`, `observation_tags` and `week_review_narration` may sit between `ai_summary` and `ticker_briefs`. Machine rule: ONE suite at a time. Doc debts, none blocking: `CLAUDE.md` ~57 KB vs ~45 KB; Recent changes and the dated entries below are both over their rule; `memory/projects/tradingbotv3.md` over 15,000 chars. Two known unowned flakes, green standalone: `test_ws_10a_scan_freshness`, `test_g7_speed_pass`. |
| Next action | **Run the full suite on the integration tip `1b9d77e0`.** `main` moves only on the TRADER's word, desk down - the lead's restart grant ended 15:35. Never while `run_ai_jobs` runs: a baseline needs the AI lock FREE (22:00-06:00 PDT ~42 real-`run_slots` tests FAIL rather than skip - `docs/AGENT_TEAM.md`). Gates owed, the clause behind each in its dated entry: **#155** (TJ-10, judgeable now - the bars tape exists), **#158**'s TJ-13B clause, **#162** (TJ-9Q's `--apply`), **#147** (TJ-3), **#148** (TJ-4's morning-after story, verdicts equal to the ledger's by `read_id`), **#159** (TJ-14A + TJ-14B), **#160** (TJ-15's first `miss_contrast`), **#163** (TJ-14B's pulls and the one-click question), **#164-#167** (TJ-16's contrast tables off real clicks, the tagger's first grounded night, `tag:` features after), **#157 re-worded + #168-#169** (TJ-12's six lines clicking through, `How fresh` naming ONLY a slot whose LAST row was not `ok` and `unknown` past the tail, the origin wake asked once with its caveat, AWAY silent), **#149, #170, #171** (TJ-5: Saturday's five cards and ONE model call - MEDIUM-local, the ledger saying why, until the probe row exists - unreadable until three of the week's sessions are packed; the strip pooling to the day cards' sum on the desk's week; one `week_review_narration` row on Friday's session, none on a weeknight). |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 PT: close the desk, run the LOOK-first command, read page one, run it again with `--apply --i-am-the-trader`, keep the backup a week, restart and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means rows moved and nothing is broken - run it again; to undo, copy the backup back. Commands: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses - wait and type it again. (3) **Click a prediction on the Mentor card** - What I expect, a direction and How sure. No journal row carries one yet, so the were-you-right list stays empty until you do. From Monday the 09:00 card also insists you label Friday's trades and may add up to three small questions. (4) **The "flat" band is my number, not yours** - a day moving less than a quarter of SPY's normal daily range counts as flat; change it whenever you like, never from one day's result. Done 16:06 PDT: the `day_review\bars\` tape back-filled 19 sessions to 2026-09-18, so #155 and #147 can be read - but **after the next restart, re-fetch those 19 bar files** (written by the old code, missing the names it dropped). |
| Last verified baseline | **9,913 passed, 14 skipped, 72 subtests, 0 failed on `843a3f02`** (live + TJ-16 + TJ-4 + TJ-12), AI lock free, 18:25 PDT; ruff clean, smoke 7/7, source selftest **96/96**. **TJ-5 on top (`1b9d77e0`): 9,974 passed, 14 skipped, 72 subtests, 0 failed, 19:30 PDT; smoke 7/7, selftest 97/97.** The reviewer's GO at `f7fac9cb`: 43 TJ-5 tests, 798 neighbours, 217 slot/slate, ruff clean, selftest **97/97** (verified on this tip); the lead's trial-merge run found the sparkline's second height setter, fixed `e93ffe49`. |
| Frozen exe | **Rebuilt and verified 2026-09-16: source and frozen both 87/87** with the `(frozen)` stamp. The desk runs from SOURCE. |
| Desk | **UP since 2026-09-20 16:03 PDT, started by the TRADER** on `main` `16ec284e` (TJ-10 + TJ-14B), after it closed itself at 15:45:45 (the second unexplained close that day, after 06:11). **The lead's restart grant is OVER**: `main` moves only when the trader says so, desk down. **TJ-16 and TJ-4 wait on `lead/p033-integration2`**, so neither is live. |
...
### 2026-09-19 — Phase 0.33 build: the lead's restart log (plan.md 12.6a window)
...
| 157 | **The report card (TJ-12, PLANNED)** - Day Review opens on five measured lines whose numbers match the tables under them; a missing input says so. | trader, after TJ-12 |
...
| 154 | **Forced 09:00 trade labels (TJ-9, MERGED 2026-09-19, OWED)** - the 09:00 card lists yesterday's trades with a suggested setup, Save waits for every answer, a skipped section returns at 10:00, the Journal shows them confirmed; plus a first-morning check that a `journal not ready` card becomes the questions once the fills land. | trader, first session after the restart (Monday 2026-09-21) |
...
- **Verification.** 8,779 passed, 14 skipped, 72 subtests, 0 failed, pytest exit 0; ruff clean, smoke
...
variant/opportunity and adapts one existing M5 entry-bearing population without multiplying old exit
recipes. No-trigger and missing-data rows remain denominators; shared-triggered MFE deltas and
all-opportunity coverage remain separate; populations do not pool; duplicate scans collapse at the
attempt/cluster level; and outlier sensitivity, floors, declared windows and trial maturity prevent a
winner headline. The ledger is read only: declaration helpers neither register nor run a trial, and
failed/abandoned history remains visible. The tester's nine red tests failed before this module existed
...
**Trader, 2026-09-15 (chat):** a new tracker setup - strong names making new highs watched for a
pullback on M15 (150-SMA) / M30 (75-SMA): below the SMA then a reclaim with an LRSI reversal, a
reclaim then a later M30/M15 LRSI reversal while holding the SMA, or a retest of the SMA; rename the
H1 retester the Pullback alert and fold these in (or, if too cumbersome, just add the claim name);
setups for compression breaks and trendline breaks; a compression MEASURE because the compression
veto is the most common one. Answers to the lead's four questions: chart arm + auto-arm on claimed
D1 picks and swing Focus; LRSI reversal = "cross up through 80, ideally below 50 2-4 bars before";
measure + chip first; every named ask-first file may be edited additively for this work.

**Done by the lead:** three Sonnet recon passes (H1 retester map, claim vocabulary and families,
compression vetoes with live counts - `compressed` 189 + v1 25 = 214 of 598 coded vetoes, all D1,
LONG 97 / SHORT 92, 136 uncoded) and two premise checks; the spec + resume brief
`docs/PULLBACK_COMPRESSION_TRENDLINE_PLAN.md` (trader's words, lead decisions the trader may
overrule, file:line premises, packets PCT-1 / PCT-3 / PCT-2 with red-first tests and gate text,
resume checklist); plan.md Phase 0.29 row + stub; docs/README.md; three `[stated]` lines in
`memory/people/trader.md`. Found and left alone: the scan ALREADY measures anchor compression on
every priority row and penalises the score (`legacy.py:4927-5026`, `:26989-26999`) - the trader's
vetoes say the measure disagrees with their eye, so PCT-3 copies the hidden numbers through, shows a
chip and ships a read-only calibration CLI before any threshold is touched. Another session's
uncommitted "Show vetoed" edits sat in the desk checkout all afternoon; nothing here touched them.

**Process:** testers (Opus) spawned for PCT-1 and PCT-3 in parallel (disjoint files), builders next,
reviewer by reproduction, then PCT-2. Integration into the plan branch in a scratch worktree; the
combined branch and `main` untouched until the trader says. Live gates are written in the spec and
numbered when a packet lands.

**Built and accepted:** PCT-3 passed three review rounds and supplies the compression measure/chip,
calibration CLI and `compression_break`; PCT-1 passed its second review over 117 copied live-shaped
names and supplies the Pullback alert, auto-arm and claim names; PCT-2 passed its third review and
supplies the frozen completed-close trendline-break watch plus one additive feed row. The merged
plan branch at `fd9997d2` passed 8,362 tests / 0 failed / 6 skipped / 72 subtests in 615.62 s with
the AI lock free, ruff clean, smoke 7/7 and source selftest 83/83. Live gates #124-#130 remain.

### 2026-09-15 — The day-trade watchlists are wiped after the close (trader-directed, lead on `claude/desk-combined-2026-09-14`)

**Trader, 2026-09-15 (chat):** after asking which watchlists are the intraday ones and whether
`longs.txt` / `shorts.txt` are wiped at the end of the day (they were not): *"i want all names wiped
at the end of the day. its a daytrade watchlist not a permanent one."*

**Built by the lead alone, no packet, on the desk's own branch.** `scripts/daytrade_watchlist_reset.py`
- `reset_due(now, written_at)` is the pure rule (the file's mtime at or before the last completed
session's 16:00 ET close means due; written after it means tomorrow's), `apply_reset` empties a due
list through `autopilot_core.write_watchlist_file` FIRST and appends one WS-5D `remove` row per name
with the new source `session_reset` after (a refused write records nothing; a failed append costs the
rows, never the wipe), `python -m daytrade_watchlist_reset` is a dry run unless `--apply`.
`AutopilotService._maybe_reset_daytrade_watchlists` runs on every tick right after `_roll_day_state`,
BEFORE the weekend short-circuit and the open scan, in every Auto mode; after a wipe it forgets
`autopilot_written` and logs one line. Switch `daytrade_watchlists_reset` (default ON). Decision 0020
amends plan.md sec 5's watchlist invariant for these two lists only (emptied WHOLE at a session
boundary; a machine still never removes one user-entered name by its own judgement; swing lists
unchanged). Untouched: the swing lists, the auto lists (their own day-roll clear), the Focus store and
its injection membership, BounceBot (re-reads both files every cycle; no ask-first file edited).

**Lead decisions the trader may overrule:** the M5 Focus picks are NOT reset with the lists; a name
typed after the close is tomorrow's and survives to the next close; the wipe fires at the first tick
after 13:00 Pacific rather than in the evening.

**Live state at build time (read-only dry run 08:20 PT):** the desk was running today's session on
the OLD code; `longs.txt` (65 names) and `shorts.txt` (75) were written at 08:10 / 08:12 PT, after
Monday's close, so the CLI reported both "written after the last close". On the new code they are
wiped at the first tick after today's 13:00 Pacific close if the desk is restarted before then, or
at the first tick of the restart that follows it.

**Verification:** `tests/test_daytrade_watchlist_reset.py` 26 passed; the two Auto Pilot tick tests
stub the new step; full suite 8206 passed, 1 failed, 6 skipped, 72 subtests in 877 s (exit 1: the failure is the known order flake tests/test_ws_wl_watchlist_tab.py::test_selection_survives_a_refresh, which passes alone); ruff clean; smoke 7/7; source selftest 81/81.
Docs: CLAUDE.md/AGENTS.md (Inputs rule + invariant), plan.md (sec 5, Phase 0.28 row and stub),
CHANGELOG (inventory + Recent changes), DESK_INTERNALS "DTR", decision 0020, docs/README.md,
memory (`memory/people/trader.md` [stated] line, MEMORY.md keywords). Gate #123. The checkpoint
and CHANGELOG remain over their size rules; the archive pass is still owed.
...
**Recon first (two Sonnet passes), two premises corrected:** the like handlers (`_after_like`, `_advance_after_like`, `_advance_review_queue`, `_retire_after_veto`) live in `scripts/ui/panels/alert_center_panel.py`, not in `alert_chart_review.py`; and `AlertChartReview.set_alert` never passed a timeframe, so `CaptureRail._timeframe` was `"D1"` for every chart in the review pane - the stale-timeframe risk the trader named was real. Also verified: the setups table merges rows from no source but the scan report; `pick_feedback.decisions_today` keys likes by symbol + date (no side); HC is an overlay at feed-write time and NO tier-outcome row carries `high_conviction` (live: 8,258 `favorite_setup`, 16,299 `near_favorite_zone`, 1,809 blank); the like cohort (`like_cohort_*.csv`, 163 picks / 130 outcomes) already grades likes forward from the claim day's close.

| Packet | Branch @ tip | Red tests (tester) | Build (builder) | Review (reproduction) |
|---|---|---|---|---|
| D1C-A claimed picks | `claude/d1c-claimed-picks-build` @ 270dd839 (tester d9b2861f) | `tests/test_d1c_claimed_picks_{store,route,queue,panel}.py`: 73 red (ModuleNotFoundError / TypeError on the named seams) | `scripts/claimed_picks.py` + `CLAIMED_PICKS_FILE`; `claim_horizon`; `claimPlaced` -> `_place_claimed_d1` -> `_retire_claimed_review`; the D1 gate + mtime cache + skip count; `claimed_setup_rows.merge_claims`, `SetupRow.bucket_keys`, five chips, `RANKED_BUCKETS` + `claimed_like`, `Drop my claim`; docs. Ten builder tests. | Round 1 NO-GO: blank timeframe left the previous chart's value standing (typed D1 look after an M5 chart stamped `M5` + a sidecar); real `"5m"` alerts upper-cased to `"5M"`. Fixed at 270dd839 (`bounce.capture_timeframe`, passed always). Round 2 GO: five of six new tests proven red; the blocker reproduction re-run on the production doors reads `D1` / no sidecar and `M5` / sidecar respectively. |
| D1C-L desk sides | `claude/d1c-desk-sides-build` @ efc61a32 (tester 7762bc9b) | `tests/test_d1c_desk_sides.py`: 7 red; three place-pinning expectations re-pointed in `test_qt_swing_favorites.py` | `d1_column` (setups over the strip, own key, 6:1), `m5_column` a one-child splitter, `show_watchlist` / rescue / floors on the column, tabs-mode visibility after `addTab`; two builder tests | GO with advisories (control-set lines, 6:1, a dead guard) - all folded into efc61a32 or this docs commit; offscreen renders at 1640x980 and 3456x2160 checked, `qt_m5_column_split_sizes_v1` never written |
| D1C-B claim grading | `claude/d1c-claim-grading-build` @ ceb869a9 (tester 6cfd2684, based on A @ 784cb27e) | `tests/test_d1c_claim_grading_{reader,tab,cli}.py`: 41 red, every number reproduced independently before it was written | `scripts/claimed_pick_evidence.py` (`load_inputs`, pure `build_comparison`, `render_text`, `python -m`), the `My claims` tab on Research > Setup Tracker on the existing ReadWorker; six builder tests; `WINDOW_RECENT` because R4b reserves `*LATELY*` | Round 1 NO-GO on one blocker: the overlap counts joined ALL liked rows against only the ELIGIBLE horizon-5 tracker rows, so a claim from the newest three sessions could never be `also FAV/Near that day` (live copies: FAV 18 vs 22, Near 17 vs 31, all 14 misses in the last three sessions). Fixed at ba234f1a - `tracker_sightings` over the RAW tier rows (the scan's fact), populations still through `read_eligible_rows`; the tier CSV parsed once per version through the panel's `_load_csv_rows_cached`; `also_hc` surfaced; `lately` by `end=` only; the display label in the tab; the claims-joined line; nine of ten new tests proven red. Round 2 GO: FAV 22 / Near 31 with every other number unchanged (liked 56/29, FAV lately 557/214, Near 2276/1259, all 887/408 and 3358/1844); second read on unchanged copies parses zero CSVs (0.369 s -> 0.169 s). One advisory left open: the claims-joined line's trailing clause says the per-row claim fields are SHOWN, but no surface renders them yet (invisible until `claimed_picks.jsonl` has rows). |

**Lead decisions (the trader may overrule):** base is the LOCAL sweep branch, not `main` (the desk runs it); a claim places in the setups table ONLY - no Focus, no watchlist (the "Add to Swing Focus" row verb stays explicit); a claim is active until `Drop my claim` or `focus_picks.FADE_TRADING_DAYS` trading days (due on the eleventh session by the packet's "more than" rule); the horizon is the alert's (`is_d1` / the M5-review flag) then the setup's registry group - which has NO day-trade group, so the fallback answers d1 or unknown; the D1 gate keys (symbol, side) for `is_d1` scan alerts only; a veto or dislike never retracts a claim (P5: verdicts stay separate); five chips FAV / HC / Near / Liked / All; the grading surface is a Setup Tracker tab + CLI, not Research > Results (which reads ONE snapshot); HC is reported unmeasured because the tracker never stamps it; the tester's `recorded == ["like_advance"]` was amended to `count == 1` + no reject-family action because advancing records the next chart's `shown` impression on the pre-packet route too.
...
Trader instruction: "Go ahead and make those repairs but first confirm your sub agent rules as per the loca .md files". The lead read AGENTS.md, docs/AGENT_TEAM.md, .codex/config.toml and the Codex role files, then confirmed Luna medium / Terra high / Astra integration ownership. The repair code already existed and had independent red/green acceptance, so no duplicate build chain was spawned. A scratch documentation commit above 6753f9fd records acceptance and is fast-forwarded into the clean, stopped local sweep checkout. No main merge, push, live-store repair or restart. Full-suite baseline is unchanged because only these control documents changed. The trader's test week, WS-RP and final main-merge decision remain open.

### 2026-09-13 - REVIEW REPAIRS (Astra's sweep review answered; branches `claude/rv-*`, integration `lead/rv-integration`)

**Trader, 2026-09-13 (evening):** *"Start fixing the findings from Astra's independent WISHLIST sweep review. Complete the five scoped packets below through failing tests, implementation, independent review and pushed fix branches ... This prompt authorizes these five repairs, including the narrow H1 watch-history, arm-time, armed-poll and phone-delivery seams in alert_center_panel.py, chart_watch.py, h1_history.py and price_alert_service.py ... Anything wider remains outside scope."* Later: *"Do not merge to main or restart the desk."*

Astra's review (artifact `sweep-review-final.txt`, reviewed tip 85781c8f, compared with `main` d31a3923; RP excluded) returned NO-GO: B1 the yfinance H1 fallback stopped refreshing once it held 45 bars; B2 a newly armed H1 watch fired on a bounce that had finished before the arm; B3 the armed phone push ran `send_push` on the Qt thread with a 10 s timeout; B4 gate 107 asked the trader to open the retired Chart Review page; and the frozen-tip suite ended 7926 passed / 1 failed on `test_migration_failure_stays_visible_instead_of_claiming_no_accounts`, attributed to a shared test database. Fix base: the current sweep tip c4df3ac8 (docs-only above the reviewed tip; the in-flight RP build touches none of these files).

| Packet | Branch @ tip | Red tests (tester) | Fix (builder) | Independent review |
|---|---|---|---|---|
| B1 `RV-H1-HISTORY` | `claude/rv-h1-history` @ d843e18f (tester ee8972c0) | `tests/test_rv_h1_history_refresh.py`, `..._staleness.py`: 6 red / 5 guards; the review's `fallback_bars 45 source yfinance refresh_requests 0` reproduced at the panel seam | need on the PRIMARY series; session-bucket cadence (`last_completed_h1_bucket`); `h1_bucket_end` for the closing bar; `last_refresh_failed` + the `stale` note; one fixture corrected; builder suite 7940 exit 0 | GO - 83 focused green; restoring only `alert_center_panel.py` reddened the review's test, only `h1_history.py` reddened seven; advisories: weekend refetches (~7/day, was 24), fixture moved, Pacific-hardcoded tests (pre-existing), 0.04 ms session lookup on the Qt thread |
| B2 `RV-H1-ARM-TIME` | `claude/rv-h1-arm-time` @ ac68df9d (tester 0a40ff04, rebased on B1) | `tests/test_rv_h1_arm_time_fence.py`: 8 red / 6 guards; the review's `armed_at 13:30 ... new_alerts 1 watches_left 0` reproduced, both boundary edges, pre-arm invalidation, restart, re-arm, an aware arm three hours WEST as a strip detector | `h1_event_is_post_arm` (event bar END strictly after `armed_at`), `_comparable_moments` (attach, never strip), `H1_PRE_ARM_REASON`, applied in `evaluate_h1_bars`; frozen rule untouched; six WS-10C fixtures pinned before the golden bounce; builder suite 7954 exit 0 | GO - 97 focused green; the eight red on the restored `chart_watch.py`; four mutations proved the guards bite; a `_naive` strip was caught by the aware-arm test; the pins proven neutral (old code 72/72); advisory 1 (fail-open on an undatable event) taken by the lead as fd457d0c |
| B3 `RV-H1-PHONE-WORKER` | `claude/rv-h1-phone-worker` @ f53d2230 (tester fb7467c0, rebased on B2) | `tests/test_rv_h1_phone_worker_delivery.py`: 9 red / 1 guard; the review's `send_push_on_qt_thread [True] call_seconds 0.202` reproduced; feed row while the send is blocked; duplicate ids; failure; raise; bounded shutdown; AWAY/EVENING/OFF | id joins `_announced_watch_ids` BEFORE dispatch; one owned daemon thread per fire (`armed-watch-push`); `{ok, queued}` return; outcome marshalled as `_notify` does; `ARMED_PUSH_SHUTDOWN_WAIT_SECONDS = 2.0`; AUTO_MODES amendment corrected; builder suite 7964 exit 0 | GO - 95 focused green; the nine red on the restored service; hand reproduction outside pytest: dispatch 0.6 ms (base 202 ms), the send on `armed-watch-push`, three same-id calls = one send, zero live threads after `shutdown()`; the shutdown-thread emit ruled SAFE (a direct call into the price-alert panel's label refresh on the GUI thread); advisories: the worker's own queued `statusChanged` is unproven without an event loop, `_last_push_error` is one shared field (two fires in a tick race for it, the log line names the watch), the `price-alerts` poll thread is still unjoined (pre-existing), a `thread.start()` failure would mark the episode announced |
| B4 `RV-LIVE-GATE-107` | `claude/rv-live-gate-107` @ cd81fb8b | docs only | gate 107 rewritten: doors that exist (centre chart, Watchlist Chart / Chart only, Ctrl+L), Load older 2 -> 10 in four presses, the `D1 back to` strip noted as absent from the centre chart, the retired page not brought back | lead-verified against `PAGE_SPECS`, `watchlist_tab.py`, `symbol_snapshot_dialog.py` and `tests/test_ws_ch_chart_history.py`; no reviewer (docs-only rule) |
| 5 `RV-JOURNAL-TEST-ORDER` | `claude/rv-journal-test-order` @ 1599eab7 (tester e03b9a46) | a prepared-store predecessor placed directly above the target reproduces `'No accounts' != 'No accounts'` deterministically; the target passes alone | the target owns `tmp_path / "unprepared.sqlite3"` and asserts the Prepare button is visible before the click; every original assertion byte-identical; no production file; builder suite 7928 exit 0 | GO - alone / predecessor+target / whole file 48 green; fail-before-fix reproduced; the added assertion proven non-tautological; advisories: literal tempdir prefix, deliberate session-wide state, implicit close |

**Lead decisions (the trader may overrule):** the fix base is the live sweep tip, not the reviewed tip; the three H1 packets are a chain (history -> arm-time -> phone-worker) so the combined tree is the last link; testers and reviewers ran focused files only and each builder ran the full suite once (machine cap three); the arm-time reviewer's fail-open advisory became a lead commit with its own red-then-green test because "missing data is uncertainty, never confirmation" is a section 5 invariant; the weekend-refetch advisory is recorded as a follow-up, not widened into the packet. Process: the usage limit reset at 20:40 PT killed the first four agents mid-task; the three testers were resumed in place with their context and the journal builder re-spawned - nothing was lost. Memory lines proposed by the agents are integrated in `memory/projects/tradingbotv3.md` under 2026-09-13.
...
|---|---|---|---|
| 1 GUI leftovers (Journal splitter 39/61 + blank space; Day-trade Tracker first click; CLAUDE.md trim) | `WS-J1` / `claude/ws-j1-journal-splitter` | MERGED (lead branch) 9ae8d6d0; gate #105 | Splitter + blank space as one small packet. The CLAUDE.md trim is a docs pass at the END of the sweep (the lead). The DT first-click read stays as it is (G7 recorded it as the accepted cost). |
| 2 FC1 forming candles in the daily-bar cache | `WS-FC1` / `claude/ws-fc1-forming-candles` | MERGED (sweep) 0134ff12; gate #98; `--apply` and two ask-first seams (Parquet mirror unfiltered; `fetch_daily_bars` return carries the forming bar) are the trader's | Build the writer guard + manifest counts + `python -m master_avwap_lib.daily_bar_cache repair` (dry run default). `--apply` on the live cache stays the TRADER's action. legacy.py writer seam only - the trader's FC1 prompt is the recorded yes. |
| 3 EF1 exit frameworks by family | `WS-EF1` / `claude/ws-ef1-exit-by-family` | MERGED (sweep) a55cf320; gate #97 | Second export + family picker on the Exit frameworks tab. legacy.py export seam only - the EF1 prompt is the recorded yes. |
| 4 SN1-SN4 (SN5/SN6 built 2026-09-08) | `WS-SN4` / `claude/ws-sn4-feed-diff`; `WS-SN3` / `claude/ws-sn3-one-rrs-pass`; `WS-SN2` / `claude/ws-sn2-incremental-bars`; Phase 0.31 | SN4/SN3/SN2 MERGED; SN1 BUILT in Phase 0.31, gate #134 | BounceBot now has one below-normal child with restart and shutdown ownership; detector behavior is unchanged. |
| 4 (points) block 4 - AWAY digest ranks swing picks by points | `WS-PT4` / `claude/ws-pt4-digest-points` | MERGED (sweep) c2215b19; gate #95 | Decision: the digest ranks by points ONLY when the existing Points switch is ON; OFF keeps today's Wilson order. Weight tuning stays the trader's. |
| 5A weekend verdict (confirmed defect) | `WS-5A` / `claude/ws-5a-weekend-verdict` | MERGED (sweep) df475e45; gate #96; floor kept at 5 (builder ruling, trader may raise) | Typed numeric contract, horizon/side explicit, percent never labelled R, "weakest veto reason" becomes "rejected names with the highest side-adjusted return". |
| 5B symmetric actual-trade comparison | `WS-5B` / `claude/ws-5b-preference-symmetric` | MERGED (lead branch) a0a29f04; gate #112 | Extend `preference_trade_outcomes` with rejections + `like_mode`; wave 2. |
...
| 5D watchlist intent events | `WS-5D` / `claude/ws-5d-watchlist-intent` | MERGED (sweep) e8770233; gate #94 | Append-only dated add/remove event at the trader-edit seam; machine writers labelled; an external diff labelled `observed_external`. |
| 5E identity / journal coverage | Phase 0.31 | BUILT; gate #135 | One versioned identity now splits side and timeframe; legacy rows remain readable and the restatement is counted. |
...
| 6 Theta pick tracker | `WS-TH` / `claude/ws-th-theta-tracker` | MERGED (lead branch) ee676ae5; gate #106 | Decisions: one row per (symbol, scan_date); graded at the sold put's expiry AND marked at 5/10/20 sessions; the readout's cohort is FIRST appearance with a repeat count. Written from the runner after the scan (never legacy.py's save pass). |
| 7 D1 market environments | `WS-ENV` / `claude/ws-env-d1-environment` | MERGED (sweep) bb4b8dc0; gate #102; the legacy.py outcome-row stamp stays owed (ask-first) | One pure rule (`scripts/indicators/d1_environment.py`, versioned), append-only daily store, runner hook, reader join by SCAN DATE, a "By environment" block on Research > Results. The legacy.py stamp on the outcome row is NOT built (ask owed) - the dated store gives the same point-in-time label. |
| 8 Setups star / X reflect the day's decisions | `WS-SX` / `claude/ws-sx-star-x` | MERGED (sweep) d81bc929; gate #99 | Both marks when both facts hold. Presentation only. |
| 9 Longs below AVWAPE / shorts above it | `WS-WS` / `claude/ws-ws-wrong-side` | MERGED (lead branch) 7b8628d5; gate #108 (rule reads the band zone - scan rows carry no AVWAPE price); hide / previous anchor / other surfaces stay the trader's | Decision (display only, hides nothing): a `wrong side` badge on the setups table and a `[wrong side]` tag in the AWAY digest line, current anchor only. Hiding or a detector change stays the trader's decision. |
| 10A D1 discovery freshness | `WS-10A` / `claude/ws-10a-scan-freshness` | MERGED (lead branch) c258f7a8; gate #109 | Build the truthful freshness surface (last successful scan / latest input bar / latest shown result, stale label) + a replay CLI over recorded manifests. The live trace is a gate. |
| 10B TC2000 picks reach the M5 watchlist | `WS-10B` / `claude/ws-10b-board-to-scan` | MERGED (sweep) a1b9fccd; gate #101 (AWAY staging was the broken link and is built) | Verify/repair + a visible "watching / staged / not adopted because" surface. |
| 10C H1/H4 retester | `WS-10C` / `claude/ws-10c-h1-retester` | MERGED (lead branch) adcaeb19; gate #116; lead rulings: pure copy of the H1 aggregation, `armed_at` naive in the store, `PERSISTENT_WATCH_KINDS`, the arm-bar test enables the bar first, a yfinance H1 fallback for ARMED names because the desk caches ~35 of the 45 warm-up bars (the trader may overrule: a wider M5 window in bounce_bot_lib is ask-first) | Step 1 only (H1 15-EMA bounce watch, opt-in arm button, versioned rule sheet). Wave 3. |
| 10D Market Journal story + thesis | `WS-10D` plus Phase 0.31 | BUILT; gates #114, #137-#138 | Deterministic story/thesis/rollups plus subject-session repair and bounded source-linked narration. |
| 10E journal tags from setup notes | `WS-10E` / `claude/ws-10e-note-tags` | MERGED (lead branch) f5fabaa3; gate #111; the CLAUDE.md four-lanes line lands in the docs pass | After WS-AI1. Wave 2. |
| 10F Daily Recap | `WS-DR` / `claude/ws-dr-daily-recap` | MERGED (lead branch) f9b07df4; gate #115; lead fix: the tester's board-door recorder keeps only symbol-bearing alerts; ask owed: the one-line add_alert skip for a symbol-less status row (alert file) | Session reader over durable stores first, then the four views. Wave 2/3. |
| 10G one Watchlist tab | `WS-WL` / `claude/ws-wl-watchlist-tab` | MERGED (lead branch) 21e8038f; gate #117; lead fix: the manual-removal test expects the Focus injection to remain for its own owner; open for the trader: rename the old Watchlists tab, open the desk with the setups column shown | Wave 3. |
...
| 10J Trade Mentor | `WS-TM` plus Phase 0.31 | BUILT; gates #110 and #139 | Raw answer is saved first; local AI fills a reviewable draft and shows one grounded coaching question. The popup is larger and remembers its size. |
...
**Landed so far:** packets written for wave 1 (`.claude/packets/WS0_COMMON.md`, `WS-FC1`, `WS-EF1`, `WS-SX`, `WS-5A`, `WS-PT4`, `WS-AI1`, `WS-5D`, `WS-ENV`) and for SN4/SN3/SN2; testers spawned for FC1, EF1, SX, AI1 and builders for 5A, PT4 at ~15:30 PT 2026-09-12. Check `origin` for branches `claude/ws-*` before re-spawning anything. FC1 tester finding: the live count is now 66 of 1,988 cache files (ongoing, dates rolled to 2026-09-11) and `market_calendar` has no early-close model. AI1 tester findings outside its packet: `operations_audit._ai_jobs_check` counts `degraded` while the ledger writes `degraded_no_narrative`, and reads `ts` while the ledger writes `started_at` (both fixed inside AI1); `tests/test_g7_speed_pass.py::test_showing_the_research_tab_loads_only_the_child_whose_tab_is_open` is an order-dependent flake on the sweep branch (passes alone). Wave-2 packets written ~16:10 PT: `WS-TH`, `WS-10B`, `WS-CH`, `WS-WS`, `WS-J1`, `WS-5B`, `WS-TM`, `WS-10A`. First merge batch (~19:00 PT): 5D, PT4, 5A, EF1 merged into the sweep branch, gates #94-#97; FC1 built, ENV/SX/AI1 in build. Usage cap hit ~18:30 PT (resets 18:50): nine agents killed; SX and AI1 builds were committed locally and pushed by the lead as `*-build` branches, builders resumed; ENV/10B/TH/SN4 respawned; batch 1 fast-forwarded into the desk checkout at 204f4640 (suite 7369 green). Batches 2-3 (FC1, SX) fast-forwarded into the desk checkout at 35b858b7 (suite 7403 green); AI1 merged on the lead branch ~21:40 PT, suite running. Batch 4 (AI1) fast-forwarded to the desk at f38ac3d6 BEFORE its suite tail was read: one red pin (`test_p6_preference_to_trade::test_the_scope_behaviour_is_unchanged`, the third nightly-slate pin) - fixed by the lead in the ENV merge commit on the lead branch; batch 5 (ENV, 10B) suite running ~22:40 PT. Batch 5 (ENV, 10B) fast-forwarded to the desk at 8aa61388 (suite 7469 green after the known `test_g7_speed_pass` order flake passed alone). Second usage-cap kill ~23:30 PT 2026-09-12 (reset 00:30): SN4 (b593008a), SN3 (f842ce30), TH (1e49636d), CH (48e5b7ad), WS (3e8368a6 red tests), TM (4a267134), 10A (982fc137) had pushed or were pushed by the lead as `*-build` branches; all eight builders (incl. J1) resumed 05:35 PT 2026-09-13; 10E and 5B testers respawned. **SESSION CLOSED ~06:05 PT 2026-09-13 on the trader's word ("finish up nothing new now").** Every in-flight builder's worktree was committed (WIP commits where edits were uncommitted) and pushed; agent contexts do not survive, so the next session spawns FRESH builders on the branch tips named in the table (tell each: review the WIP commit, make the packet's red tests pass, gates, handoff). Lead branch `lead/sweep-merge` (pushed to origin) holds batches 1-5 + SN4 (gate #103); the batch-6 suite log is `<scratchpad>/suite_batch6.log` of session b0a76b0c - if it is not readable, rerun the suite on `lead/sweep-merge` before fast-forwarding the desk checkout to it. The desk checkout is at f16cb0ff (batches 1-5, all suite-verified). Remaining after the in-flight rows: SN2, DR, WL, 10D, 10I, 10C, RP (packets written), then the docs pass (CLAUDE.md trim, plan.md, WISHLIST `SWEEP:` lines). Batch 6 (SN4) fast-forwarded to the desk at 1920d693 (suite 7483 green). Batch 7 (SN3, J1) merged on the lead branch ~07:00 PT 2026-09-13, suite running; 5B and 10E builders spawned. Writer-lock contention: the machine-global lock dir (73,500 files) serialises concurrent suites - run few at once. Batch 7 (SN3, J1) fast-forwarded to the desk at e0933967 (suite 7498 green). Batch 8 (TH, CH) merged on the lead branch ~07:40 PT, suite running. Batch 8 (TH, CH) fast-forwarded to the desk at b3739191 (suite 7551 green). Batch 9 (WS, 10A) merged on the lead branch ~08:20 PT, suite running. Known order-dependent tests: `test_ws_sx_star_x.py` fails 4 when run ALONE (green in the suite); `test_g7_speed_pass` and `test_ai_jobs_runner::test_a_weekend_firing...` flake under some orders. Batch 9 (WS, 10A) fast-forwarded to the desk at ebcbf477 (suite 7636 green). Batch 10 (TM) merged on the lead branch ~08:45 PT, suite running; 5B and 10E builders still in flight. Batch 10 (TM) fast-forwarded to the desk at cc20ecb9 (7675 passed, ONE order-dependent red: `test_ws_10a_scan_freshness::test_the_manifest_is_read_on_the_refresh_path_and_never_on_paint` passes alone and after the TM file - recorded as a flake to own, not a regression). Batch 11 (10E, 5B) merged on the lead branch ~09:15 PT, suite running. Every spawned builder has handed off; nothing else is in flight. Remaining: SN2, DR, WL, 10D, 10I, 10C, RP (packets written), then the docs pass (`<scratchpad>/docs_pass_notes.md` of session b0a76b0c lists the CLAUDE.md lines). Batch 11 (10E, 5B) fast-forwarded to the desk at c7bf035f (suite 7706 green) ~09:30 PT 2026-09-13; testers spawned for SN2, DR, WL, 10D (wave 3). Third usage-cap kill ~09:45 PT 2026-09-13 (reset 10:30) took the four wave-3 testers before they wrote anything; respawned 10:50; SN2 red at 11:00, builder running. Planned pause 12:15 PT. FOLLOW-UP PACKET OWED (found by the 10D tester, not fixed): `EvidenceLedger.append` overwrites the Market Journal entry's `session_date` with the New-York date of the append, so an evening Pacific note (21:00 PT = 00:00 NY next day) is filed under the NEXT session and `entries_for(session)` misses it; 10D selects by `market_journal.session_date_for(created_at)` to route around it. SN2 merged on the lead branch ~11:50 PT (batch 12); DR (3e1c1973) and WL (d9f44da2) red with builders deferred; 10D builder still running at the 12:15 pause. Batch 12 (SN2) fast-forwarded to the desk at de3b0031 (suite 7719 green). Batch 13 (10D) fast-forwarded to the desk ~12:05 PT (suite green). Session paused ~12:13 PT on the trader's word: nothing in flight; next session spawns builders for DR (3e1c1973) and WL (d9f44da2), then 10I, 10C, RP, then the docs pass. Takeover session 15:00 PT 2026-09-13: testers for 10I and 10C done; four builders (DR, WL, 10I, 10C) spawned, all killed by the 15:40 PT cap and resumed 15:48 PT in their own worktrees (`.claude/worktrees/agent-*`); the 10C builder holds its full suite until a slot frees. Batch 15 (10C, WL, 10I) merged on the lead branch ~16:45 PT 2026-09-13 with the lead fixes recorded on the rows; suite running. Remaining: RP (tester then builder), then the last docs lines for RP. RP tester done 17:15 PT (44 red on `claude/ws-rp-shared-report` off `lead/sweep-merge`); RP builder spawned ~17:20 PT; Astra's review prompt handed to the trader in chat (branch `claude/wishlist-sweep-2026-09-12`).
...
The trader requested Astra as planner, orchestrator and final decision-maker, with
Luna/Terra doing recon, implementation, tests and review. Existing Codex role files
had no model keys and still described Claude models. Added project defaults and
explicit role models; scoped the common runbook by tool and kept AGENTS/CLAUDE
identical. Existing WISHLIST assessment edits remain untouched; no wishlist work
was promoted. The glance block was shortened to remove superseded status chains.
Verification: all five TOMLs parse with the expected models/efforts; root files are
byte-identical; git diff --check passes; Codex CLI 0.153.0 reports config.load OK with
no startup warnings; rendered prompt includes the routing rule after moving it near
the top (previous bottom placement was truncated). Independent Terra review: GO, no
remaining findings. Doctor overall reports the unrelated noninteractive TERM=dumb
condition; its configuration check passes. No app tests or live writes were needed; the 7312-test baseline and all live and
promotion gates are unchanged. No new product phase or desk restart is required.

### 2026-09-08 ~23:30 PT - SN5 / SN6: the scanner breathes and scans the trader's picks first (lead, on `main`)

- **Ask:** the trader, on 9% of the week's usage, asked for anything quick and cheap from
  WISHLIST; the lead named SN5 and SN6 from the "keep the desk snappy" prompt and the trader
  said "Go" - the yes for `bounce_bot_lib/legacy.py`'s two seams (loop pacing, fast-lane order).
- **Built:** `SYMBOL_BREATH_SECONDS` + `BounceBot._breathe` (stop-event wait after each symbol
  in the fast lane and both sweep loops); `BounceBot._fast_lane_order` (trader picks, then auto
  picks, alphabetical within each); `human_focus_auto_symbols` loaded beside the focus map
  through the new read-only `focus_picks.load_auto_pick_symbols()`, whose marker rule is the
  store's own `_read_todays_auto_pick_markers` (refactored out of `_load_auto_picks`). Nothing
  detected, scored, stored or alerted changes.
- **Verified:** `tests/test_sn5_sn6_scanner_breath_and_fast_lane_order.py` 13 tests; with the
  two source files reverted to HEAD 12 fail and the clock pin passes (the revert proof). Full
  suite 7312 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0 (a first run failed one old fast-lane stub test in test_bounce_learning.py; the stub was given the two new helpers and the rerun is green); ruff clean; smoke 7/7. Built by the lead alone, no agents, under the usage cap.
- **Docs:** CHANGELOG (Recent changes + inventory), plan.md (Phase 0.24 row and stub),
  DESK_INTERNALS ("SN - the scanner's hold on the interpreter"), this file. WISHLIST untouched
  (the trader's notepad). CLAUDE.md untouched (over budget).
- **Owed:** gate #92 (above); a desk restart. SN1-SN4 are NOT authorized.

### 2026-09-08 ~21:00 PT - The setups table ranked by a point system (lead, on `main`)

Trader, 2026-09-08 ~20:00 PT: the four-input point ranking for the Master AVWAP setups
(setup family record, nearby S/R, RS/RW in the trade's direction, a recent bounce), "put it
in wishlist.md then start working on it block by block". The trader has under 10% of the
plan's usage left, so the lead built it directly (no tester/builder/reviewer round) as ONE
presentation packet on the ST6 pattern: a switch that reorders and never hides, `legacy.py`
untouched, every input a field the scan already writes.

- Built: `scripts/setup_points.py`, the `points` column, the `Points` checkbox, eleven tests
  (`tests/test_setup_points.py`), two golden pins widened (`test_g2b_named_columns`'s compact
  widths, `test_setup_group_context`'s column tail). CHANGELOG inventory + Recent changes,
  plan.md Phase 0.23, WISHLIST blocks 1-3 removed (the trader's uncommitted notepad; not staged).
- Verified: full suite 7291 passed, 3 skipped, 72 subtests, exit 0; ruff clean; smoke 7/7.
- Owed: gate #90 (above). The AWAY digest's order is NOT changed (ask first). No PF exists
  on any surface; the setup part is the Wilson bound + expected R. Weights are named
  constants at the top of the module for the trader to tune after a few desk days.
- **~22:00 PT, the evidence loop** (trader: *"ensure that we track how this system
  performs ... a way for the system to correct itself"*): `scripts/setup_points_evidence.py`
  - log, grade (terciles by total against the tracker's own 5-session outcomes through the
  ONE reader, lift refused under the n floor, per-part lift), proposal (`1 + 2 x lift`
  clamped [0.5, 1.5], only when both halves hold the floor) - applied ONLY when the trader
  turns on `Points: learned weights` (default OFF). Grade sentence on the setups status row
  and in the Points tooltip. Eight tests. Gate #91 owed (restart + 5 sessions). Decision,
  recorded here for the trader to overrule: the correction is a proposal the trader
  enables, never a silent move - a weight that moves itself on thin evidence would be the
  "ranking by result" invariant broken one step removed.

### 2026-09-07 ~18:35 PT - The Strength window is ONE flat page (branch `claude/strength-page`, merged to `main`)

**Trader (evening):** *"For the main trading desk, the strength tab is unusable there's like 2 tabs and they get no space each. Create a solution that removes the tabs and just collates all the data to be more easily readable."* The "2 tabs" are the two `CollapsibleSection` headers ("RS/RW Board" open, "M5 Strength Board (TC2000)" closed) that shared the strength column's height by stretch factor with the Focus board above them.

**Built (lead, directly - a hosting-only change; `alert_center_panel.py` is an ask-first file and the trader's message is the instruction for this window):**

- `ui/widgets/strength_page.py` (new): `StrengthPage`, one `QScrollArea` with the four reads one under another - `FocusStrengthBoard`, `EntryAssistBoard`, `RrsSnapshotWidget` (scopes STACKED), `StrengthBoardPanel` under a page-owned heading. Every text board is sized to its document (`fit_height_to_document`), every table to its rows (`FIT_ROWS_CAP` 30, the sideways scrollbar's height reserved), so nothing on the page scrolls on its own. Vertical scrollbar always on (an as-needed bar reflows and can loop). Floor 170 px, the column's old floor.
- `AlertCenterPanel`: `strength_column`, `rrs_board_section`, `strength_board_section`, `rrs_board_tab` GONE; `strength_page` is the splitter's second widget; `attach_strength_board` hands the panel to the page. `CollapsibleSection` has no caller left.
- Width fixes measured with real fonts on an offscreen render (`QT_QPA_FONTDIR`): the entry board's title and the RRS environment label WRAP, "Copy All RS/RW" became "Copy RS/RW" with tooltips, the strength panel's status label is `Ignored` horizontally, the side-table title wraps, every data column is `ResizeToContents`. Page body minimum 228 px at scale 1.0 against a ~475 px page; rendered PNG read by the lead: one column, readable top to bottom.
- Measured while building: a never-shown `QTextEdit` document reads `size() == (0, 0)` until its first relayout - the fit sets the text width itself first (`docs/DESK_INTERNALS.md` "The Strength window is one flat page, long form").
- Tests: `tests/test_qt_strength_board_in_the_desk.py` section 5 rewritten, seven tests RED first (`ModuleNotFoundError`); two other files follow the widget. Docs: CLAUDE.md/AGENTS.md "Charts and boards", DESK_INTERNALS, CHANGELOG (Recent changes + two inventory sentences superseded).

**Verification (worktree, this branch, the finished tree):** `pytest tests/ -q` with nothing deselected: **7280 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0, 7 min 34 s**; ruff clean; smoke 7/7; selftest 74/74 (both against a scratch `TRADINGBOTV3_DATA_DIR`).

**Gate #89 (trader, next desk restart):** the Strength window on the Trading Desk is ONE column that scrolls with ONE bar on its right: Strength (Focus + field), then "Auto RS/RW Board", then the environment title with **Copy RS / Copy RW** and *vs SPY / vs Sector / vs Industry* one under another, then **M5 Strength Board (TC2000)** with Longs over Shorts. No arrow headers, nothing to expand, no box inside the column with its own up/down scrollbar (a table longer than 30 rows may scroll itself; the Strength tables may scroll sideways). The charts are no narrower than before.

**Not changed:** any detector, score, alert, queue, fold, evidence writer, the adoption gate, the one `StrengthBoardService`, any click route.
...
**FIX ROUND (same day, same branch): four small items, all proven RED against the
reviewed tip `0a06cea0` before their fix, four new tests in `tests/test_g7_speed_pass.py`
(21 of 21 green).** (1) A `refresh()` asked for inside the worker's teardown window was
DROPPED: `_read_until_nothing_is_pending`'s loop releases `_refresh_lock` with
`_refresh_pending` False the instant it decides to stop, but the `ReadWorker` thread has
not actually finished at that point, so a `refresh()` landing there saw `isRunning()`
still True, set the flag, and nobody was ever going to check it again - the page went on
showing rows ranked at the previous `min_closed`. Fixed on the worker's own `finished`
signal (fired only once `isRunning()` is reliably False), which now also drops the
panel's `_read_worker` reference before a possible restart or `deleteLater()` - a stray
old reference was crashing the NEXT `refresh()` with `RuntimeError: libshiboken: Internal
C++ object (ReadWorker) already deleted` once item 2 below started collecting workers,
which is how the fix for one item exposed the shape the other needed. (2) Every
`refresh()` built a new `ReadWorker` and none was ever deleted; `finished` now also
carries `worker.deleteLater()`, proven by five refreshes leaving at most one `QThread`
child instead of five. (3) `PriceAlertsPanel._save_table` would write the table's rows -
EMPTY before the first `showEvent` load (G7.1) - over the real store if a save fired
before the page was ever shown; it now refuses and logs when `not self._loaded_once`.
(4) The stale comment on `_counted_win_rate_rows` ("runs on the Qt thread with the rest
of `refresh()`") reworded to name the worker path G7.2 actually put it on
(`_read_tracker_exports`, called from `_read_until_nothing_is_pending`).
...
nobody is touching. `built_at` and every event `ts` are market-local and AWARE. Gate #83 now
...
BUILT bullets.
### 2026-09-07 - G5 BUILT: Research > Results, the landing page the trader may read (branch `claude/g5-research-results`)

Phase 0.22, the G lane. Trader, 2026-09-06 (decision 0016 answer 7 AMENDED that day, and
decision 3 of that day): Research gains a **Results** page the trader may read, Bot setups /
My trades and Swing / Day trading kept as FOUR separate populations and never pooled; it is
the FULL readout, the Desk's Working-lately line stays primary, and both read ONE evidence
snapshot; it opens on Bot setups x Swing x Recent 20 sessions with the last choice remembered.

**Tester first.** Nine RED tests at `a79c7f20` off `main` `7e018c99`
(`tests/test_g5_research_results.py`, `tests/test_g5_research_results_panel.py`), re-run on the
un-fixed tree in the builder's worktree before the first edit: **6 failed, 3 errors**. They are
9 passed with the build, and the builder ADDED **thirteen** more (the first draft of this
entry and the CHANGELOG both said eleven; the file holds thirteen, and the fix round below
takes the added count to thirty-one)
(`tests/test_g5_research_results_extra.py`) - the degenerate inputs a banding rule breaks on,
an OPEN trade in no bucket, a blank timestamp reading `unknown timing`, the exposure sentence
naming an instrument and never a direction, and `ResearchPanel`'s forward of a pushed snapshot.

**What landed.** `scripts/research_results.py` is a PURE view (frozen dataclasses; `band_cells`
RAISES on a mixed-kind cell list the way `working_lately.pool_cells` refuses across its axes).
It computes **no new statistic, threshold or eligibility rule**: `meets_floor` / `n_floor` and
`EvidenceCell.concentrated` are the cell's own, the ordering is `working_lately.rank_basis`,
the window is `evidence_stats.lately_window`, and every number on a bot row is the cell's OWN
OBJECT (an identity test over counts above CPython's small-int cache proves it, and a
source-level guard proves `rate * n` never appears). Stronger lately is the eligible cells in
the snapshot's own order, top three; Weaker is the three lowest by statistic **among the ones
Stronger did not take**, which is what makes "never both" structural rather than lucky; Not
enough evidence carries the reason. Cells are read through
`working_lately.cells_from_payload` because `snapshot["cells"]` is COMPACTED. My trades splits
CLOSED trades by holding period through `journal_trade_shape.is_date_only`, shows the broker
rows as `unknown timing` under BOTH horizons and assigns them to neither, and refuses a
leaderboard where nothing is confirmed. `ui/panels/research_results_panel.py` renders it on a
`ReadWorker` with G4's identity-aware pane; `research_panel.py` puts Results first and current
and grows a `set_working_lately_snapshot` seam; `app.py` gains ONE line.

**Packet deviation, measured rather than argued.** The packet asked for `EvidenceCell.line()`
on each band card. Three of those twelve-clause lines in a third-width card left the shortlist
**26 pixels tall at 1920x1080** - exactly G0/G1's `overflow` defect. The card now prints a
one-line head spelled the way the Desk's Working-lately line spells it, keeps the full
`EvidenceCell.line()` as its tooltip, and every field of that line is also a column of the
shortlist and a paragraph of the detail pane. Re-measured at three sizes: the table gets 295 /
...
**Review fix round (same day, same branch, `origin/main` `b231323e` merged in).** Reviewer
NO-GO with three blockers and eight advisories; **eighteen tests written RED first** and proven
failing on the reviewed tip `85d11227` (18 failed, 13 passed) before the first edit.

- **BLOCKER 1 - the window control was inert.** `_window_of` labelled a window and nothing
  filtered by it, so the reviewer counted a trade closed in **2019** under "2026-09-01 to
  2026-09-04". `research_results.in_window` now filters MY TRADES' closed trades by
  `closed_at`, inclusive at both ends, and reports what it turned away
  (`stats["n_outside_window"]`) rather than dropping it in silence; a trade whose `closed_at`
  the journal never carried is counted OUT of a bounded window, because "not known to be
  inside" is not "inside". For BOT the snapshot owns its window: the three buttons are
  DISABLED with a tooltip saying so, and `ResultsView` gains `window_applies` /
  `window_sentence`, the latter read off the cells' own `window_sessions` through the
  snapshot's `as_of` - the freshness line and the status line print that, never a date range
  the numbers never saw. **A test the tester wrote clicks the window button while the page is
  on Bot** (item G5.3.8, the detail pane comes down), so the three button groups moved from
  `idClicked` to `idToggled` filtered on `checked`: still ONE reaction per change, and the
  state of a button the page disabled is still drivable. Nothing was weakened to arrange that.
- **BLOCKER 2 - `an unnamed file @ None`, three times, on every real snapshot.**
  `working_lately.build_snapshot` writes `path: ""` and `mtime: null` for each source it
  computes and fills in `rows`. `_source_text` prints the ROW COUNT and adds a path or an
  mtime only when one is present; a snapshot with no source at all says `NO_SOURCES`. The
  literal `None` can no longer reach the screen.
- **BLOCKER 3 - a card said "27 shown" over six rendered lines.** `N of M shown`, N being the
  lines the card printed (`CARD_LINES`) and M the band's own total, so the trader knows the
  shortlist below holds the rest.
- **Eight advisories taken.** The window button label reads `evidence_stats.LATELY_SESSIONS`
  (with a source-level guard, because 20 == 20 by coincidence today); money names its currency
  (`_currency_label`, off `resolve_pnl_key`'s own choice) and a refused total prints that
  refusal's own reason and the currencies instead of "net unmeasured"; the `untagged` fallback
  bucket never appears as a row under the "Confirmed tag" header - coverage is the sentence's
  business; a study row is muted the way an ineligible one is; an empty section leaves no
  dangling heading; the section text is ONE short verdict line per kind (`verdict_short` -
  state and reason, still verbatim) with the leader, the policy line and the population
  sentence in the tooltip, and every running-text label is capped at **G3's** reading measure
  (`READER_MEASURE_CHARS` 100, left-aligned, the constant asserted equal to
  `market_journal_panel`'s) - measured at 1,922 characters of running text on the fixture
  snapshot before the fix.
- **Test hygiene (advisory 7).** The suite-wide autouse selection fixture is GONE from
  `tests/conftest.py`, where it stood over 1,800 tests that never touch the key. The two files
  that write it clean up after themselves: `_forget_the_saved_selection()` in the panel file's
  `page` fixture teardown (the one cleanup edit the lead allowed in the tester's file - no
  assertion moved) and an autouse fixture in the builder's extras.
- **Rendered, not argued.** The page was drawn at 3456x2160 on the REAL `windows` platform
  from an UNSHOWN widget (`Qt.WA_DontShowOnScreen` + `show()`, the theme applied at scale 1.0,
  no window on the trader's desk and pid 9036 untouched), against the fixture snapshot carrying
  the LIVE `sources` shape. It shows: the three window buttons greyed with their tooltip; the
  line *The snapshot owns this window: 20 sessions ending 2026-09-04* under the controls and
  again at the foot; the freshness line reading `swing_trade_r <- 10 row(s); swing_favorable
  <- 3 row(s); daytrade_held_run <- 4 row(s)` with no `None` and no "unnamed file"; the cards
  reading `5 of 5 shown` / `3 of 3 shown` / `3 of 3 shown`; the two study rows (`studyfam`,
  `fav_study`) muted with the below-floor, concentrated and unmeasured ones; and the section
  text 655 characters in a 600 px left-aligned wrapped column, where it was 1,922 characters
  across the whole width.
- **A doc defect fixed with it**: `CLAUDE.md`'s "The Research tab is not a trader surface" was
  still absolute after the amendment that created this page. It now names the exception, the
  window rule and the `DESK_INTERNALS` entry; `AGENTS.md` re-copied byte-identical.
- No packaging trigger: no new dependency, no non-`.py` asset, no new top-level `scripts/`
  package, no dynamic import. Gate **#87** stands, with the fix round's clauses added to it.

### 2026-09-07 - G2b BUILT: the Setup Tracker tabs, the Desk Setups table and the AWAY tables name their column (branch `claude/g2b-tracker-desk-away-columns`, unmerged)

Phase 0.22, the half of G2 that waited for ST6 to stop rewriting these three panels. Tester
first: nine tests committed RED at `4d99f1e7` on `7e018c99` and re-run red by the builder
before the first edit (six in `tests/test_g2b_named_columns.py`, three added to
`tests/test_table_width_rule.py`), plus three green-by-design guards - a render golden over
every tab's cells and row order, a golden of every compact setups width, and the AWAY Focus
table's already-correct measured answer, kept as a regression guard with the reason in its
docstring. Every populated fixture carries a DECOY text column longer than the one the packet
names, so the measured path picks the decoy and the assertion can only pass by naming. Builder
added four more (`tests/test_g2b_key_level_delegate.py`), proven red with all four source
files reverted. `origin/main` `6d9f4b43` (the G2a merge) merged in clean.

**What was wrong.** `DataTable.set_width_rule` had ZERO production callers, so every table on
these three surfaces ran the measured path, and measured is content-dependent: a populated
Catch Rate gave the width to `sample_caught_winners` while the MISSED winners the tab exists
for clipped; an EMPTY Controls or Studies tab measured its HEADERS, so `Win % (low)` stretched
and `Family` sat at its floor; Human Picks handed `cohort` most of a 4K window and pushed its
ten measurements off the right; and the Desk Setups table gave the slack to `Key Level` - the
longest cell on the table - so `Setup Tags` stayed pinned at its measured width.

**Built.** (1) `apply_width_rule(..., stretch_last: bool = True)`; with it False and no text
column named, NOTHING stretches - it suppresses the measured auto-pick as well as
`stretchLastSection`, because leaving the classify path on would hand `cohort` the slack by
the back door, and that is the whole complaint. A NAMED text column still stretches. Default
leaves every existing caller byte-identical. (2) `_make_table(columns, *, text_key=None,
elide_keys=(), stretch_last=True)` declares all fourteen tracker tables, every index resolved
BY KEY through a new `_column_index` that RAISES on an unknown name; the two `fit_columns()`
call sites are unchanged, so the attribute leaderboard's worker slot is covered too. (3) The
Desk Setups FULL profile names `setup_tags` and installs `_KeyLevelElideDelegate` on
`key_level`; the COMPACT profile removes that column delegate on the way in and is otherwise
untouched. (4) AWAY Recap middle-elides `Line`, `Trigger` and `Cell / held x ran` and names
`Symbol` on the Focus table.

**DEVIATION from the packet, and why.** G2b.3 asked for `elide_columns=(key_level,)` on the
setups table. `apply_width_rule` gives an elide column a per-column `MiddleElideDelegate`, and
**a per-column delegate REPLACES the view's own delegate** - so that call would have left
`key_level` painted by Qt's default while every other column of the same row kept
`SetupTableDelegate`'s alternating background, favorite tint, selection fill and hairline
separator. One column drawn differently from its own row is worse than the clipping this
packet fixes. `_KeyLevelElideDelegate` inherits BOTH classes: the setups delegate wins `paint`
and `sizeHint`, `MiddleElideDelegate` supplies the full-value tooltip, and the ONE override is
`_text`, because that helper hard-codes `ElideRight` - the end elision §12 forbids for an
identifier whose tail is its anchor and retest date. The four added tests pin exactly this:
the delegate is a `SetupTableDelegate` too, its `sizeHint` is still the setups row height,
painting a long key level really does reach `elide_middle` with the WHOLE value and leaves a
real tail, and compact gets the column back.

Layout lane: no number, sort key, read or write moved; the rule still runs once per fill,
after the fill. Live gate **#85**.
### 2026-09-07 - G4b BUILT: the Setup Tracker's detail pane clears when its context changes (branch `claude/g4b-setup-tracker-detail`)

Phase 0.22, the second half of G4, deferred in that packet while ST2/ST6 were rewriting
`setup_tracker_panel.py`. ST6 landed on `main`, so the hold lifted. Tester-first on `main`
`7e018c99`: six tests at `9879aa4a`, five proven RED and one golden green BY DESIGN - it pins all
FOURTEEN tabs' rendered header and rows through the sort proxy, so a re-show inserted before the
`set_rows` calls or a row dict mutated while scanning turns it red. Builder made the five pass
without weakening any, added a sixth, and merged `origin/main` `6d9f4b43` in.

**What was true.** `SetupDetailView.shown_identity` and its overriding `clear()` shipped with G4.1
and nothing on this panel called them. `self.tabs` (fourteen tabs) had no `currentChanged` handler
and `refresh()` replaced every model's rows with `set_rows` without touching the pane. The pane on
this page carries a stop price, a 1R and two targets, so the last row the trader clicked stayed on
screen beside a table that no longer held it - a price plan read against the wrong symbol.

- **G4b.1.** `self.tabs.currentChanged` -> `_on_context_tab_changed` -> `clear()`: empty, hidden,
  identity forgotten. The same verb G4 gave the Day-trade Tracker.
- **G4b.2.** `_reshow_or_clear_detail()` at the END of `refresh()`, after every `set_rows` and
  `fit_columns`. **The visibility question comes FIRST** - the trader reaches the hidden state by
  moving tabs, and a scan that re-showed on a match alone would pop an explanation open under
  someone who had closed it (the tester's finding, driven by
  `test_a_refresh_leaves_a_hidden_pane_hidden_even_when_its_row_is_present`). Then one linear scan
  of the CURRENT tab's model - `row_at`, no dict copied per row - for the matching identity,
  re-shown through that tab's own `show_*` call and from the NEW row dict, so the pane prints the
  revised number rather than the cached one; no match, `clear()`. The tab's model and show-kind come
  from `_detail_tables` by asking which tab widget owns the table (`isAncestorOf`), never by tab
  position, so adding or reordering a tab cannot silently re-point the re-show.
- **The identity is widened where this page collides.** A Setup Types row has no `dimension` and no
  `symbol`, so two rows of one (side, family) in different zones share the whole identity.
  `DETAIL_WIDENING_KEYS` (`favorite_zone`, `priority_bucket`) separates them, normalised with
  `str(... or "")` so a row carrying neither compares equal on both sides and widening can never
  turn a real match into a miss. **A pair differing only in `retest_label` still collides** and
  falls to the first such row in the model's own order - stated here rather than fixed, because the
  packet named two columns and inventing a third grain is a measurement decision, not a layout one.
- **`_on_family_row_clicked` REMOVED**: defined since the panel was written and never connected to
  anything; with the pane now carrying a lifecycle, a dead second show path is a trap.
  `SetupDetailView.show_family` is untouched (a G4 test drives it).
- **Out of scope, stated.** The Attributes tab is filled later by its own worker
  (`_on_attributes_loaded`, 19.7 MB off the Qt thread) and is not one of the eight clickable tables,
  so no click can open the pane on it and the re-show never looks there; `refresh()` reached from
  the min-closed spinbox through its 250 ms coalescer takes the same path as the button.

**Layout lane, honoured.** No number, sort, column or read moved: the golden is identical before the
change, after it, and after a full click / tab-switch / refresh cycle.

**Fail-before-fix, proven.** `scripts/ui/panels/setup_tracker_panel.py` restored from the tester's
`9879aa4a` and the file re-run: 5 failed, 1 passed (the golden). Restored: 7 passed. The
builder-added widening test was proven the same way with `DETAIL_WIDENING_KEYS = ()` - it fails with
"the pane is showing the pre-refresh number", because the identity-only scan finds the
better-scoring twin first.

Live gate **#86** (G2b holds #85). No packaging trigger: no new dependency, no non-`.py` asset, no
new top-level `scripts/` package, no dynamic import.
...
Small follow-up packet, cut from G3's tip `af650c86` in its own worktree. Three items,
each with a test proven RED first (restored the pre-fix files with `git show HEAD`, ran,
all three new tests failed; restored the fix, ran, 15/15 green in
`tests/test_g3_market_journal_reader.py`). (1) `_written_line` (`:133-151`, confirmed
against the packet's pointers) printed the zone the stored `created_at` CARRIES, and
every live row stores it in UTC (`market_journal.py`'s
`moment.astimezone(timezone.utc)`), so every note read `written 13:36 UTC` for one typed
at 06:36 Pacific. Now `astimezone`s the AWARE side to `pass_bars.desk_zone()` and prints
`HH:MM UTC±HH:MM`; a NAIVE stamp is never guessed at and still says
`(no zone recorded)`. (2) `_on_entry_selected` (`:717`) filled the reader by ROW INDEX
(`_entry_for_row`) while the list's `Qt.UserRole` and the charts both key on `entry_id` -
today the two line up because every row is an entry, but a future header row would
desync them silently. Now keyed by `_selected_entry_id()` through the new
`_entry_for_id`; proven with a dummy `QListWidgetItem` (no `entry_id`) inserted at row 0
and the handler exercised directly at the new (wrong-for-index) row - the reader still
shows the selected entry's words. (3) `setMaximumWidth(reader_measure)` (`:349-351`) ran
once, in `__init__`; extracted into `refresh_reader_measure()`, called from
`MainWindow._apply_scaled_metrics` (`scripts/ui/app.py`, one added line - the seam that
method exists for). Proven by forcing a small then a large font on the reader widgets
and checking the cap actually changes when the method is called.

**No store, `entry_id`, `created_at`, `session_date`, `written_after_the_session`,
layout dimension or write path touched.** Full suite 7078 passed / 3 skipped (up from
G3's 7063 - the other lane's ST-packets landed on `main` since), lock free (probed),
ruff clean, smoke 7/7, selftest 74/74 - no packaging trigger. **Known fragility, not
fixed here (out of scope for this packet):** the pre-existing
`test_the_meta_line_keeps_the_zone_the_stored_stamp_carries` fixture hands
`_written_line` a `created_at` already carrying `-07:00`, which happened to be
byte-identical to `pass_bars.desk_zone()`'s answer on this machine today (Pacific,
PDT, September); before this packet that test was zone-independent (it only echoed the
input), and it is not anymore - it will need a monkeypatched `desk_zone()` (the pattern
this packet's own new tests use) before PST season or a differently-configured machine,
...
splitter then opened at one third in the fix round - 1,151 / 2,301 px at 3456 × 2160, measured by the reviewer). Nothing new on the Qt
...
**Fix round (2026-09-06, tip `12418084`).** The lead read the 3456 × 2160 screenshot and
the reader was present but not READABLE, which is the packet's point, so two measurements
were repaired. (1) **The thought gets a measure:** `thought_view` and `thought_meta` are
capped at `_reader_measure` — `QFontMetrics.averageCharWidth() × 100` or `theme.px(1200)`,
whichever is SMALLER, floored at `theme.px(240)` — and left-aligned inside the pane with
the slack on the right, so the text that was set across the full 2,779 px as one
400-character line now runs 600 px and **96 characters** (measured against the theme's own
6.12 px mean character), while the pane keeps its width for the charts below. (2) **The
entries column opens at a third of the page:** stretch shares a RESIZE and never the first
layout, so the size hints opened it at 655 px and clipped the new 90-character excerpt after
about 35 — `lower.setSizes(LOWER_SPLIT_SHARES)` opens it at 1,132 px, still draggable. Both
proven red first on the stashed panel (viewport 2,777 px against a 1,250 px bound; entries
655 against 1,000); 11 tests in the G3 file now, and the other three Market Journal suites
untouched and green. Still `theme.qss` by object name, still no `setStyleSheet`, still no
store, identity, timestamp or write change.
...
### 2026-09-06 - G4 BUILT: the explanation pane clears when its context changes (branch `claude/g4-stale-research-detail`)

**Item:** packet `.claude/packets/G4.md` (plan.md Phase 0.22, the Desk Reshape Plan), items
**G4.1 and G4.2 only**. G4.3 and the tester's tests 5-6 stay DEFERRED to packet G4b:
`setup_tracker_panel.py` is being rewritten by ST2/ST6 and is untouched on this branch.

**Branch note:** the packet named `lead/merge-st` `e7c51409` as the base; the tester actually
branched from `76eba3ae` and the lead directed a merge of `main` `68762909` instead, which is
what this branch carries (merge `10f6fb95`, no conflicts). The tester's five red tests are
`98668de3`; the fix is `b255fb31`.

**What was wrong.** The GUI review that morning found the Day-trade Tracker still showing the
`lrsi_cross50` explanation with the **Combos** tab open and no combo selected.
`ResearchExplanationView.show_row` set HTML and `setVisible(True)`, and nothing ever took the
pane back down: neither `tabs` nor `decisions_tabs` had a `currentChanged` handler, and
`_on_refresh_finished` / `_on_held_run_loaded` replaced every model row without touching it.
The last row clicked therefore survived every tab switch and every re-aggregation, beside a
table that no longer held it - and the numbers on the right get read as the numbers on the
left, which is why this is correctness rather than polish.

**What was built.**
- **G4.1** - `ResearchExplanationView.show_row(kind, row, *, identity=None)` records
  `shown_identity`, and `clear()` is an **override**: `QTextEdit.clear()` already existed here
  (the tester found this and said so - the packet had expected an `AttributeError`) and only
  empties the document, which would leave an empty pane standing. The override empties, HIDES
  and forgets. `SetupDetailView` gains the same pair; its identity is
  `(kind, side, family, symbol-or-blank, dimension-or-blank)` computed in `_render` from the
  row it just drew, and its `clear()` also drops `_current` so a late levels callback cannot
  re-open a cleared pane. **Nothing calls `SetupDetailView`'s pair yet** - that caller is G4b.
- **G4.2** - `_explanation_identity(kind, row)` is `(kind, dimension, direction, segment)`,
  read from the ROW DICT and never from the display text. `direction` is in it because the
  learning store keys a segment `direction|segment` and `long vwap` / `short vwap` are two
  measurements; the packet wrote the identity as `(kind, dimension, segment key)` and this is
  that key spelled the store's way. Both tab strips clear the pane on `currentChanged` - the
  `decisions_tabs` connection is belt-and-braces (the outer strip fires first in the live GUI,
  so it is only reached for a move BETWEEN the My Decisions sub-tabs) and the comment beside it
  says so. Both data-revision slots call `_reshow_or_clear_explanation`, which looks the
  identity up in the model that now holds the tab's rows and re-shows from the **new** row dict
  - re-showing the cached one would reproduce the defect wearing a number instead of a name -
  or clears when the revision dropped the segment.

**Fail-before-fix, proven twice.** With the three production files restored to `98668de3`
(`git checkout HEAD -- ...`), **6 failed in 1.45 s**: the four panel tests on `.isHidden()` and
on the pre-refresh number, the tester's widget test on *"clear() emptied the pane and left the
empty pane on screen"*, and the added `SetupDetailView` test on `AttributeError: 'SetupDetailView'
object has no attribute 'shown_identity'`. Restored: 10 passed alongside
`tests/test_research_explanations.py`. No tester assertion was weakened, skipped or rewritten;
`tests/test_g4_setup_detail_view_identity.py` is the only test ADDED.

**Baseline on this branch (after merging `main` `68762909`), nightly AI lock FREE (probed):**
`pytest tests/ -q` with nothing deselected - **7069 passed, 3 skipped, 72 subtests passed,
zero failures, exit 0, 8 min 2 s**; `ruff check .` clean; `smoke_check.py` 7/7; source
`launch_gui.py --selftest` 74/74. **No packaging trigger**: no dependency, no non-`.py` asset,
no new top-level `scripts/` package, no dynamic import. Live gate **#83** above; the desk
restart that makes it observable is the trader's call.
### 2026-09-06 (afternoon; reviewed and fixed that evening) - G0 BUILT: a repeatable desk workload bench and the layout-fit check (`claude/g0-measure-first`)

The first step of Phase 0.22's build order. It builds a measuring tool and takes one
baseline. **No panel changed**; nothing on the desk imports the module and no timer starts
it. Files: `scripts/ui/desk_bench.py` (new), `tests/test_desk_bench.py` (new, 32 tests on
round 1, **38 after the fix round**), `docs/GUI_FLUIDITY_MEASUREMENT_RUNBOOK.md` section 7,
plus the ledgers.

**Fail-before-fix, proven twice.** With the module moved aside the whole test file is a
collection error. With the live-store guard sabotaged to `return False`, nine tests fail -
**and that run staged six synthetic files into `C:\TradingBotData\scratch` and the same
folder on the DAS.** Both subtrees were new, both were deleted, and no live file was read
into or overwritten (the live listing matches its pre-run state exactly). The lesson is in
the code: `_refuse_to_open_for_writing` is a SECOND guard comparing its own literals,
checked before the destination is created and again before every file is opened, so the
next agent who proves guard one bites cannot write into the store while doing it.

**The baseline** (`%LOCALAPPDATA%\TradingBotV3\diagnostics\desk_bench_baseline_2026-09-06.json`,
offscreen, 3456x2160 / 3840x2160 / 2560x1440, `--repeat 3`, over a 383.6 MB staged copy of
the live store; 5 min 47 s):

| what | number |
|---|---|
| chrome, measured from the widgets | top bar 49 + status bar 33 + margins 8 = **90 px**, so 2160 leaves **2,070 px** of page and 1440 leaves 1,350 |
| ops over 250 ms sync p95 | **three ops, at three sizes**: `research.construct` 5,142 / 4,312 / 4,400 ms; `setup_tracker.refresh` 1,320 / 1,098 / 1,060 ms; `market_journal.construct` 299 ms at the target size only |
| the slowest thing on the desk that is not a `sync` number | `weekend.refresh_everything` returns in **1.7 ms** and settles in **12.7 s p50**, hitting the 20 s deadline once - the V2 design working exactly as written, and still 12 s of a page filling in |
| worst single `processEvents()` in the whole run | **806.6 ms**, in `research.construct` at 2560x1440. This row first read 683 ms, which was the worst iteration inside the Weekend wait rather than the worst in the run (reviewer, advisory 6) |
| pages flagged `overflow` | `weekend_prep` (needs **3,072 px**) and `weekend_prep.focus_review` (needs **2,858 px**), at **every** size including 3840x2160 - the overflow is vertical and the extra width does not help. 2,340 px of the focus page's requirement is nine table floors of 260 px (`TABLE_TEN_ROWS_PX`) stacked in one vertical layout. **This is the proof the check sees the defect G1 fixes.** Every other page fits at all three sizes |

**Two things recorded rather than fixed, because G0 changes no panel.** (1) Constructing
the Research tab reaches `treasury_calendar_service`, which attempted an HTTPS call on
every run and failed on certificate verification - the bench is not hermetic and must not
be described as such. (2) The baseline is OFFSCREEN, which has no DPI and no real paint;
gate #81 is the windowed run that says whether it is the right yardstick for G7.

**The fix round (same evening, after the reviewer's NO-GO).** One blocker, and it was the
packet's own invariant: `main()` ran `_prepare_environment` FIRST, which mkdirs `--data-dir`
and writes `<data_dir>\_localappdata\TradingBotV3\local_settings.json` into it, and only
then called `_abort_if_live`. So `--data-dir C:\TradingBotData` exited 2 with three
directories and a file already sitting in the live home folder - the refusal printed after
the damage. Both guards now run on the ARGUMENT at the top of `main()` through
`refuse_live_destination`, for `--data-dir` and for `--out`, and again on the resolved out
path (the default included) before its `mkdir`; `_abort_if_live` still runs afterwards
because it answers the different question of what `project_paths` actually resolved to. The
second guard's literals moved into `WRITE_REFUSAL_PREFIXES` so a test can point BOTH guards
at a FAKE live root under `tmp_path` and assert nothing was created there - **no test points
a sabotaged guard at the real live paths as a destination**, which is exactly how round 1
created `C:\TradingBotData\scratch`. Proven twice: all six new tests fail on `2bee86db`, and
with the fixed module in place and ONLY the guard loop removed from `main()` the two
ordering tests fail on "`_prepare_environment` ran before the live-store guards".

Three advisories taken in the same round. (a) The `local_settings.json` allowlist entry
named the home-folder root, where that file does not live, so it reported `absent` on every
staging run and the baseline was measured against a synthetic one-key settings file; the
entry is dropped and `machine_settings_seed` now carries an allowlist of keys out of the
real `%LOCALAPPDATA%\TradingBotV3\local_settings.json` at run time - the display keys that
change what is measured (`qt_ui_scale` scales every `theme.px`) plus the trader's
`daily_bars_source: yahoo` pin, and never one of the five credentials or three live-store
path keys that file also holds. (b) The gate's clause 3 is restated as the two SLOWEST ops
rather than "the two ops over 250 ms": three ops crossed that line at the target size here
and four in the reviewer's re-run, so the line is noisy and the ORDER is the check. (c)
`settle()`'s docstring claimed nothing was slept inside a timed region; the 2 ms yield IS
inside `settle_ms` (and outside `longest_iteration_ms`), and every settle carries the 120 ms
`QUIET_MS` floor, so a reading near 120-135 ms measured nothing. Advisories 5 (the
`_widget_workers_running` poll cost) and 7 are G7's, not this round's. `main` `68762909`
(ST1-ST5) was merged into the branch and this packet's gate renumbered #75 -> **#81**.

**Nothing was not measurable.** Every op the packet named was driven through a real widget:
Research's tab strip is found by identity against `setup_tracker_panel` (it is not on an
attribute), and the child tab strips are measured only after their owning Research tab is
brought to the front, because a tab switch inside a hidden page costs almost nothing and
would have reported a fast desk nobody has.
### 2026-09-07 - ST7 BUILT: the trader's three decisions become the tracker's defaults (branch `claude/st7-decisions-default`)

**The decisions, verbatim** (trader, 2026-09-06 ~21:15 PT): *"Yes a trade not yet completed
should say pending. A second entry after a first close is its own trade yes. 3. This one is
up to your discretion."* The lead settled (3) - both `gap_aware_v2` and `prior_session_v2`
become the default - because each is a pure correctness fix and keeping a known look-ahead
because the honest number is worse is the failure decision 0016 goal 8 names. All three are
recorded as `docs/decisions/0019-tracker-selection-and-execution-defaults.md`, which also
carries the survivorship caveat on the 463 re-entries and the one-switch rollback.

**What moved.** `DEFAULT_SELECTION_POLICY` -> `first_actionable_v2`,
`DEFAULT_EXECUTION_CONVENTION` -> `gap_aware_v2`, `DEFAULT_LEVEL_KNOWLEDGE` ->
`prior_session_v2`. `closed_first_v1` / `literal_level_v1` / `same_session_v1` keep their
values and stay selectable by keyword. `recompute_tracker_setup_record` now stamps
`execution_convention` and `level_knowledge` on EVERY record instead of popping them on the
default path - after a flip an absent stamp is ambiguous - and the tracker-write call site
gained a log line (`Setup tracker policies: selection=... execution=... levels=...`) because
there was NO success line at that seam to extend.

**One rule the packet did not name and the code needed.** ST7.1 says a compact scoring
projection written under the old default is read AS-IS. It is - `_summarize_tracker_setup_outcome`
returns the cache verbatim for a default read - but the AGGREGATE then has to grade it, and
v2's "pending stays pending" keys on `representative_status`, a column a PRE-ST4 cache does
not carry at all, so `_row_is_graded` falls back to the row's own `closed_setups` when
the key is ABSENT and `no_representative_in_population` counts those rows.

**Measured, not asserted** (reviewer, on a COPY of the 2026-09-06 snapshot, with the bridge
reverted): the 32 recent family rows still appear - the total collapse the first ST4 build
produced does not repeat - but their graded population goes **1,688 closed -> 0**, all 2,195
episodes reading pending, and the nonzero `recent_tracker_score_delta` count goes **14 -> 7**.
`setup_type` deltas are **74 either way**, because `build_tracker_setup_type_rows` takes no
policy at all. So the failure is partial rather than total, and it is still the champion's
recent-family score input going half blank for one scan.

**The bridge is keyed on the key being genuinely ABSENT, not on an empty string.** 48 records
on that snapshot carry the column with an EMPTY value because ST4+ measured them and found no
representative scenario; they stay UNMEASURED (13 episodes move pending -> unmeasured, so
ST4's quoted 915 pending reads 902 + 13 on this branch). A row cannot tell the two apart from
the coerced string, so it now carries `representative_status_known`. A status that is PRESENT
and reads `pending` is still never graded. Gate #84 clause (5) is the check.

**The goldens.** `st3_replay_golden.json` and `st4_family_rows_golden.csv` are untouched and
are now reproduced with the v1 policies NAMED; the tester's `st7_v2_default_golden.json` and
`st7_family_rows_v2_default_golden.csv` were pinned on `main` at `68762909` through the
explicit v2 keywords, before the flip, and the default now reproduces them. Neither pin is a
self-portrait, and `ST7_REGEN_GOLDEN` was never set.

**Re-pinned, none weakened (37 pre-existing failures, the tester's own count, all closed).**
`test_st3_execution_convention` 9, `test_st3_execution_convention_builder` 7,
`test_st4_first_actionable` 5, `test_st4_compact_projection` 3, `test_master_avwap_setups` 4
(3 of which the `_row_is_graded` fallback fixed by itself), `test_tracker_methodology` 3,
`test_tracker_band_variant_parity` 2, and one each in `test_st4_compare_guards`,
`test_first_dev_breakout`, `test_p4_half_b_evidence`, `test_regime_conditioning`. Every v1
leg gained its policy by name; every "and the default agrees" leg now asserts the v2 answer
beside it. Three test names were changed because the NAME asserted something the flip made
false (the two "the default reproduces the golden" ones, and "the default event dict has
exactly the keys it always had" - the default dict legitimately adds `fill_basis` and
`execution_convention`, and the test now pins that it adds exactly those two).
`build_tracker_band_variant_parity_fixture.measure` gained three policy keywords so the
Phase 0.10 B-2 fixture is read under the policies it was FROZEN on, plus a new test that
re-runs the same shadow FENCE under the CURRENT defaults - the fixture's real subject is
that a band-variant scenario never enters a champion aggregate, and that must not become
unverified just because the replay policies moved.

**Tests whose subject is ONE axis name the other axis** rather than letting it drift with
the default (the partial/target-gap cases, the deferral-flag cases, the post-earnings
...
of the bridge. Fixed here: gates **#77 and #78** rewritten as SUPERSEDED by #84 with their
surviving clauses named (both demanded the opposite of the truth and would have recorded a
false FAIL on Tuesday); the two compare CLIs' notes now say which arm is the default, with
`README_TEXT` asserted by a test; the bridge's blast radius restated everywhere with the
reviewer's MEASURED figures (32 rows survive, 1,688 closed -> 0, recent deltas 14 -> 7,
`setup_type` deltas 74 either way) instead of the first ST4 build's total collapse; the
bridge NARROWED from "falsy status" to "key ABSENT" via a new non-exported
`representative_status_known`, because the wide form also caught 48 live records whose ST4+
summary carries the column present and EMPTY and turned 13 unmeasured episodes into graded
ones - gate #84 now expects `n_pending` 902, not ST4's 915. Narrowing surfaced one real
consequence, now pinned rather than discovered on Tuesday: a study family whose stop is not
the protective band has no representative scenario and reads UNMEASURED under the default
(the 1st-dev-breakout study is stopped at the VWAP). Plus five stale "v1 is the default"
comments in `legacy.py`, a test for the log line, and the 25 entries from 2026-09-05 back to
2026-09-03 archived so `CHANGELOG.md`'s `Recent changes` holds the two days it promises.

**Live gate #84** is owed at the first persisted tracker write after merge. Nothing here
promotes a setup or reaches a detector, a score, an alert, a watchlist, Focus, the review
queue or `review_policy.json`; `calc_anchored_vwap_bands` is untouched (decision 0008).
...
`main` at `84ee24d6`) was still held by the tester's worktree, so the build went on
`claude/st5-personal-evidence-build` branched from that tip - the same convention ST3 used.
...
imports it by string name.

The first full run had ONE failure and it was mine, not pre-existing:
`test_one_unreadable_store_still_leaves_a_card` asserts the Weekend Prep verdict card is
5-8 lines, and the packet's coverage sentence made it 9. Fixed by placing the sentence in
its own label under the card (see the deviation below), and re-run green.

**What was built.** Every item of the packet:

- **ST5.1** `preference_trade_outcomes.TRADE_WINDOW_SESSIONS` = 10, walked through
  `market_calendar` by the new `statement_window_end`. The constant said DAYS and every
  docstring in the module said SESSIONS; over Labor Day that is five real sessions
  discarded - a statement on 2026-09-04 reached 2026-09-14 instead of 2026-09-21, so a
  trade opened on the 18th read as "never taken". `TRADE_WINDOW_DAYS` stays one release as
  an alias (grep: nothing in `scripts/` imported it). A calendar that refuses falls back to
  the OLD, strictly NARROWER arithmetic - uncertainty may never widen a window into a match
  nobody made. Confidence labels untouched.
- **ST5.2** `trade_level_summary(rows)` and `summary_note(rows)`. The file stays one row per
  STATEMENT; the money is summed ONCE per `trade_id`. Live: 538 rows, 13 `traded=yes`, **10
  distinct trade ids**, so the statement-grain total was three trades' P&L too large.
  `run_preference_trade_outcomes` now returns `n_statements_matched` / `n_trades_matched`
  (live gate #79), and Weekend Prep's note prints both.
- **ST5.3** new pure `scripts/journal_exposure.py`. `classify_exposure(trade)` ->
  `Exposure(instrument, ownership_direction, market_bias, structure, certainty)`. A LONG
  option is never a bullish setup; a bought put is `bearish`, a sold put
  `bullish_or_neutral`, a sold call `bearish_or_neutral`; `UNKNOWN`, `BAG` and `CASH` are
  `unknown` and uncertain. `multi_leg` = more than one distinct option CONTRACT among the
...
legs of a spread are two `trades` rows keyed by their own OCC symbols and nothing links
them. What is OBSERVABLE is a second option trade on the same underlying and expiry opened
in the same session on a different contract - which is also what two independent ideas on
one name look like. So the label is `partial_of_spread_candidate`, it puts the row in the
UNCERTAIN population, and no claim is made that it is half a spread.

**DEVIATION from the packet.** The coverage line was asked for "on Weekend Prep's verdict
card". The card is five to eight lines by the trader's own request (V2 item 2b) and
`test_the_card_is_five_to_eight_lines` + `test_one_unreadable_store_still_leaves_a_card` pin
it. The sentence sits in its own label directly UNDER the card, filled from the tag page's
existing worker through a new `coverageChanged` signal - same screen, same worker seam, no
second read of the journal, card contract intact. A test records the choice.

**Tests.** The tester's seven were re-proven red at `f6a28138` and are green. The builder
added twelve in `tests/test_st5_review_flow.py`; ten of them were proven red by checking the
branch base's `scripts/` over the working tree, and the two that passed there only exercise
the already-committed `journal_exposure` module. Nothing in the tester's file was weakened.

**Read-only, verified.** No broker call, no auto-confirm, nothing reconstructs a
`planned_risk`. Two tests spy `JournalStore.save_risk_fields` into a raise and assert every
read path on both surfaces leaves it uncalled, and check the `trade_annotations` table
afterwards. `journal_feed.suggest_planned_risk` was inspected in passing and is clean: it
prefills from an ARMED ALERT's entry and stop - the trader's own plan at decision time - and
returns `None` on anything but a unique match. Nothing new reaches a detector, score, gate,
alert, watchlist, Focus, review queue or `review_policy.json`.
...
machine cache (633 symbols). **The denominator is `n_setups` 794, the artifact's own**:
800 records were OFFERED and 6 carry no tradeable scenario, 0 were missing bars, 0 failed
to replay. `--limit` is what was offered and is never the denominator - the CLI now prints
the whole split (`population: n_setups 794 compared (offered 800, untradeable skipped 6,
no cached bars skipped 0, replay failed 0)`) and the JSON carries a `population_note`
saying the same thing, because the first write-up quoted 800 and invited exactly this
confusion. **The three JSON/CSV pairs are copied where the lead can read them**, machine-local
and outside the repo: `%LOCALAPPDATA%\TradingBotV3\diagnostics\st3_execution_compare\` -
`both__comparison_20260906T110731.*`, `exec_only__comparison_20260906T110814.*`,
`levels_only__comparison_20260906T110855.*`. The first draft reported `min R -4.0 -> -4.0`,
which is `TRACKER_SCORING_R_CLIP` (4.0) inside `_summarize_tracker_setup_outcome` and not a
tail; the artifact carries the CLIPPED R (what scoring reads) and the RAW R (where the tail
lives) side by side, never blended.

| run | changed | expectancy (raw) | win rate (Wilson) | R < -2 | rank moves |
|---|---|---|---|---|---|
| both repairs | 472 of 794 | -0.0981 → **-0.1192** | 0.576 → 0.596 (0.541 → 0.561) | 47 → 48 | 43 of 50 groups, max 14 |
| execution only | 89 of 794 | -0.0981 → **-0.0811** | 0.576 → 0.597 (0.541 → 0.562) | 47 → 47 | 33 of 50, max 9 |
| level knowledge only | 458 of 794 | -0.0981 → **-0.1491** | 0.576 → 0.548 (0.541 → 0.513) | 47 → 48 | 40 of 50, max 17 |

`min R` is **-65.6011 under every policy** - the worst trade in the sample was already
filled inside its bar, so the repair does not touch it. Of the 89 setups the execution
repair moved, **84 got BETTER and 5 got worse**: the convention is symmetric by design, a
resting limit that opens through its price fills better than the level, and in this sample
target gaps (166 setups touched `gap_open`) outnumber stop gaps. The largest single rank
move is `LONG|favorite_zone_watch|favorite_setup` 26 → 40 under both repairs.

**The two skip counters, and what one of them found.** `no_prior_session_level` fired 4
bar-tests over 1 setup - real but rare, as expected, since the band history almost always
covers the previous session. `invalid_bar` fired **380 scenario-bars over 26 setups (22
symbols)**, and the cause is specific: each of those 22 cached daily-bar files carries
**exactly ONE invalid candle, all of them dated 2026-09-04**, every one with `low > open`
or `high < open` - AEE `O=105.81 H=106.96 L=106.11`, TWLO `O=239.52 H=239.29 L=231.21`,
GPGI `O=14.195 H=13.925`. That is the signature of a FORMING bar written into
`%LOCALAPPDATA%\TradingBotV3\machine_cache\daily_bars` mid-session, where the "low" is
the low since the snapshot rather than the day's. **Under `literal_level_v1` - which is
...
### 2026-09-06 (Sunday) - ST1: each outcome gets its true meaning and its true clock (branch `claude/st1-outcome-clock-build`)
...
**Fix round after the reviewer's NO-GO (same day).** Three blockers, all of them the SAME defect
surviving somewhere the packet did not name. (1) **The Summary card crowned max-R-on-three three
lines ABOVE the fixed banner**: `research_explanations.build_plain_english_whats_working` picked
`max(avg_closed_r)` over `closed_setups >= 3` across BOTH namespaces, so live it read *"LONG
top_pattern leads at +0.99R ... 3 closes"* while the banner underneath said *"SHORT general"*, and
10 of its 17 candidates were studies. Both its leader bullets now read the same
`select_leader` verdict. (2) The **Best Type Edge** tile read `setup_type_rows[0]`, so ST2.2's
bound-first sort silently moved it from `SHORT +23` to `LONG +14`; it now picks max `score_delta`
explicitly - a tile that borrows another surface's ordering has no meaning of its own. (3) The
Summary's **Setup types working** block showed `rows[:8]` of a side-first list, which on the live
export was eight LONG rows and no SHORT one (first SHORT at index 68 of 117); the CARD now takes
its eight by the bound across both books while the TAB keeps its side-first order.

Eight advisories with it: the count columns moved to the END of the SHIPPED header
(`_move_keys_to_end`, golden `st2_shipped_headers_golden.json` pinned from `84ee24d6`); the panel
remembers its last FRESH leader per horizon so `last_reliable_reading` is reachable (in-memory,
ST6 persists it); `min_n` binds the stale and undated discovery pools; the population sentence
names its window (ALL HISTORY, pointing at Last 30 Days) and `working_lately.FRESHNESS_SENTENCE`
states the rule in words; `counted_pair` is the one reader so an exported integer 0 is a count;
`headline_from_counts` gained an optional `flats`; and `0 expired unmeasured` renders. **The
ask was ANSWERED yes**, so `build_tracker_short_horizon_rows` gained its own
`n_wins`/`n_losses`/`n_flats`/`n_unmeasured`/`outcome_kind`/`horizon_basis`/`latest_measured_session`
(golden `st2_short_horizon_golden.csv` pinned BEFORE the edit; `win_rate_2d` byte-identical), so
the 2-session block is freshness-checked rather than discovery by construction. **One ruling
along the way**: `select_leader` judges the FLOOR BEFORE freshness - a family with three samples
is under the floor whatever the clock says, and telling a reader "not fresh" about three samples
answers a question they did not ask.

**Re-review round (same day).** The three original blockers reproduced as fixed; two NEW ones,
both the same shape as the first round's - a surface computing for itself what the page had
already decided. (1) **The card and the banner disagreed on the STALE path**: the banner went
through `_remembered_verdict` (which carries a `previous`) while `research_explanations` called
`select_leader` directly without one, so after a stale refresh the card printed *"no clear leader.
Leading on thin evidence, SHORT general on n=88 - discovery only"* three lines above the banner's
*"SHORT general [last reliable reading, as of 2026-09-04]"*. `panel_verdicts(panel)` now computes
both horizons ONCE in `_summary_html` and hands the same objects to both renderers. (2) **The
2-session label contradicted its own verdict**: it hardcoded "2-session discovery" over a real
`leader` and printed "the export carries no session" beside "its newest measured session is 58
sessions behind"; the label is now the horizon plus `verdict_label_suffix(verdict)`, and the
no-session sentence renders only when `discovery_reason` is `no_session`.

Five advisories with it: `discovery_basis_phrase` says **"older evidence"** for a stale row and
keeps "thin" for the floor (both renderers, one helper); the two tests that could not fail on the
old code are rebuilt (one row is now genuinely undated - the `_evidence_row` helper had been
dating it - and the other drives the REAL export end to end into the banner);
`research_explanations` falls back to MARKET-local like the panel; and **the 2-session rows now
date themselves by the MEASURED bar** (`_short_horizon_measured_session` reads the trade date of
`post_marks[horizon - 1]`, the same index the summary reads the close from), so a family entered
eight weeks ago and measured two sessions later stops reading as 58 sessions stale on a current
file - unknown stays EMPTY, never back-filled. The RECENT rows keep entry dating and the constant
says why: they carry no exit date until ST4 adds `representative_exit_date`. The export identity
is stated and tested: `n_wins + n_losses + n_flats == samples_2d` and
`samples_2d + n_unmeasured == tracked_setups`.

**Re-check round (same day), one blocker.** The banner guarded its short-term block on *"a
discovery row OR a leader"* and otherwise printed the hardcoded *"not enough 2-session samples yet
(accrues automatically each scan)"* - so `no_clear_leader`, which is TODAY'S LIVE STATE for that
horizon (12 eligible families, the top two 0.001 of bound apart), rendered as "no samples" under a
card three lines above saying "no clear leader". False, and self-contradicting on one screen. The
conditional is gone: `_verdict_block_html` renders all four states. Two advisories with it:
`freshness_sentence(kind)` replaces the single `FRESHNESS_SENTENCE`, because only the 2-session
rows are measured-dated and the swing line was claiming a clock it does not have - it now reads
**entry-dated** (`DATING_BASIS_BY_KIND`, defaulting to the conservative "entry" for an unknown
kind); and the dead `discovery_note` parameter moved into `working_lately` beside
`discovery_basis_phrase`, so the sentence and the gate it belongs to cannot drift apart.

**Owed at integration:** ST1's `Headline.outcome_kind` wire, and ST4's `representative_exit_date`.
**The ST4 switch is a one-line change** and the lead can make it at merge: ST4
(`origin/claude/st4-first-actionable` at `9d75a27f`) stamps `representative_exit_date` onto the
per-episode row inside `build_recent_tracker_setup_family_rows` - the SAME dict ST2's counting
loop already reads as `closed_row`. In that loop, `session = str(closed_row.get("scan_date") or "")`
becomes `str(closed_row.get("representative_exit_date") or closed_row.get("scan_date") or "")`,
and `DATING_BASIS_BY_KIND["swing"]` flips from `"entry"` to `"measured"` so the sentence follows.
Nothing else moves; the fallback keeps a still-running episode dated by its entry. ST1's tests-only branch was still at
...
`Headline`. **The ask-first question was answered** in the fix round (lead, same day):
`legacy.build_tracker_short_horizon_rows` may gain the additive counts and a session under the
trader's *"Export true integer wins/losses/flats/unmeasured at each table's actual episode and
outcome grain"*, golden-pinned from the code before the edit. Done.
...
**Active item:** packet ST1 (Setup Tracker review repairs), plan.md Phase 0.14 V3 item 1
and Phase 2 P2.3. Built on the tester's ten RED tests (`tests/test_st1_outcome_clock.py`,
committed red at `29e36350` on `main` @ `84ee24d6`); all ten were re-run RED on this
branch before the first edit and are green now.

**What was wrong.** `master_avwap_tier_outcomes.csv`'s `win` column is
`side_return_pct > 0` - the sign of a close-to-close percent move between two of a
symbol's OWN scan rows, chosen as `future_idx = idx + horizon`. It is not a stop-rule
verdict and it is not R, and `swing_headline.headline_from_tracker_rows` told four
trader-facing surfaces that it was "the stop-at-a-level, two-closes rule". Separately,
three readers of that one file applied three different rules: `setup_docs` and
`autopilot_core` drop `stale_horizon` rows and `build_bot_tier_performance_rows` did not.

**What landed.**

- `outcome_kind` appended to both writers (`favorable_direction_scanrow_v1`), with
  `swing_evidence.outcome_kind_of` reading an absent or empty cell as v1. Every v1 value
  and column is byte-identical against `tests/fixtures/st1_tier_outcomes_golden.csv`,
  which the tester generated from the pre-fix code.
- `swing_headline.Headline.outcome_kind` decides the wording. `62% favorable
  (>=52%, n=90)`; the sentence names its basis ("a price-direction rate measured close to
  close, not a stop-rule verdict"); only a `trade_r` headline says "win rate". The setups
  table header is **Family favorable %** (the column KEY is unchanged).
- `scripts/master_avwap_lib/session_horizon_outcomes.py` (v2, beside v1): entry-session
  close against the close ON the N-th session after it, holidays skipped, missing target
  bar left unmeasured with a reason, an unarrived horizon `immature`, duplicates counted.
  Exported to `master_avwap_session_horizon_outcomes.csv` in the tier tracker's own pass
  from the frames the scan already holds - it never fetches, and the write is guarded so
  it can never cost a v1 export or the tracker save.
- `scripts/swing_evidence.py`: `SwingOutcomePolicy` + `read_eligible_rows`, used by all
  three readers. `build_bot_tier_performance_rows` now drops explicit `stale_horizon`
  rows, gained `n_assigned_tier` / `n_derived_tier` and an `assigned_only` parameter.
...
sessions. That would change `sessions_spanned` and `stale_horizon` on the v1 rows -
June-to-July 2026 alone contains Juneteenth and the observed July 4 - which the same
packet forbids ("every v1 column stays byte-identical") and which the trader's prompt
excludes ("does not approve ... historical restatement"). `horizon_drift` gained
`calendar=` and reports "exchange sessions" when given one; the v1 export deliberately
