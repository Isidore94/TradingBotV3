...
| Last verified baseline | **`claude/g5-research-results` at the G5 fix round (`origin/main` `b231323e` merged in), 2026-09-07 ~13:00 PT in the builder's worktree, nightly AI lock FREE (probed): `pytest tests/ -q` with NOTHING DESELECTED: **7255 passed, 3 skipped, 72 subtests passed, ZERO failures and ZERO errors, pytest's own exit code 0** in 9 min 32 s; ruff `All checks passed`; smoke 7/7; `launch_gui.py --selftest` 74/74.** That is `main`'s 7215 plus G5's forty (nine tester, thirty-one builder). The page was also RENDERED at 3456x2160 on the real `windows` platform from an UNSHOWN widget (`WA_DontShowOnScreen` + `show()`, no window on the desk, pid 9036 untouched) against a fixture snapshot carrying the LIVE sources shape - the fix round entry below lists what that render proved. Previous: `lead/merge-g` at `496bd37a` (the G2b merge), 7215 passed, exit 0. |
