...
| 10C H1/H4 retester | `WS-10C` / `claude/ws-10c-h1-retester` | TESTS RED a9790697 (24 red + 2 guards); BUILDING on `claude/ws-10c-h1-retester-build` (builder restarted 15:48 PT after the cap; lead rulings: pure copy of the H1 aggregation - importing bounce_bot_lib.legacy drags 1,056 modules; `armed_at` stays naive in the chart-watch store, the fired/expiry rows aware; the `h1_ema_bounce` kind exempt from load_chart_watches' date reset; ChartWatch gains watch_id + reason; `PriceAlertService.notify_armed_watch` de-duplicated by watch id in every mode) | Step 1 only (H1 15-EMA bounce watch, opt-in arm button, versioned rule sheet). Wave 3. |
...
| 10G one Watchlist tab | `WS-WL` / `claude/ws-wl-watchlist-tab` | TESTS RED 591ae08c (52+2 red, fixture saves/restores the home, timers stopped); BUILDING on `claude/ws-wl-watchlist-tab-build` (item 1 pushed f6efd7c2; builder killed by the 15:40 PT cap, resumed 15:48 PT) | Wave 3. |
...
| 10I thesis/context/setup join | `WS-10I` / `claude/ws-10i-context-join` | TESTS RED 6e3854aa (23 red + 1 guard); BUILDING on `claude/ws-10i-context-join-build` (builder restarted 15:48 PT after the cap; tester findings: the By environment SECTION already exists from ENV, 10I adds the page-level control; day cut is the D1 label never `context_json.market_environment`; attach_context before link_theses; the Daily Recap widget wiring waits for DR) | Deterministic join contract + module after WS-ENV; wave 3. |
