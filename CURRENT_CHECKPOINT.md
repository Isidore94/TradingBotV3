...
| 2 FC1 forming candles in the daily-bar cache | `WS-FC1` / `claude/ws-fc1-forming-candles` | BUILT 801820c8 on origin/claude/ws-fc1-forming-candles; fix round (frozen-clock repair tests) running; two ask-first seams owed (Parquet mirror unfiltered; fetch_daily_bars return carries the forming bar) | Build the writer guard + manifest counts + `python -m master_avwap_lib.daily_bar_cache repair` (dry run default). `--apply` on the live cache stays the TRADER's action. legacy.py writer seam only - the trader's FC1 prompt is the recorded yes. |
...
| 6 Theta pick tracker | `WS-TH` / `claude/ws-th-theta-tracker` | PACKET | Decisions: one row per (symbol, scan_date); graded at the sold put's expiry AND marked at 5/10/20 sessions; the readout's cohort is FIRST appearance with a repeat count. Written from the runner after the scan (never legacy.py's save pass). |
| 7 D1 market environments | `WS-ENV` / `claude/ws-env-d1-environment` | TESTS RED 59789967, builder running | One pure rule (`scripts/indicators/d1_environment.py`, versioned), append-only daily store, runner hook, reader join by SCAN DATE, a "By environment" block on Research > Results. The legacy.py stamp on the outcome row is NOT built (ask owed) - the dated store gives the same point-in-time label. |
| 8 Setups star / X reflect the day's decisions | `WS-SX` / `claude/ws-sx-star-x` | TESTS RED 95093cb1, builder running | Both marks when both facts hold. Presentation only. |
...
| 10B TC2000 picks reach the M5 watchlist | `WS-10B` / `claude/ws-10b-board-to-scan` | PACKET | Verify/repair + a visible "watching / staged / not adopted because" surface. |
