...
| Last verified baseline | **`lead/merge-g` at `496bd37a` (the G2b merge on top of G4b's `d46034f1`), 2026-09-07 ~12:25 PT, in the scratch merge worktree, nightly AI lock FREE (probed): `pytest tests/ -q` with NOTHING DESELECTED: 7215 passed, 3 skipped, 72 subtests passed, ZERO failures, exit 0, 9 min 38 s; smoke 7/7; selftest 74/74; ruff clean.** Previous: `main` `817ff699`, 7199 passed on the rerun (the first run's order-dependent `test_qt_alert_center` flake is in the open list). |
