...
| Latest work | **TJ-10 (`57b44ca9`, fix `3e52d94f`) and TJ-14B (`161e905c`) are LIVE since 2026-09-20 11:48 PDT** (`main` at `cdad9533`), on top of TJ-13B and TJ-9Q (live since 07:14). TJ-9Q's import switch ships OFF, so nothing in the live journal has moved; the `--apply` is the trader's own act and is owed, and the 27B has still never run. TJ-4 and TJ-16 are with their builders (red tests at `60bccf06` / `7cf949b1`); whatever merges after 15:35 PDT waits on `lead/p033-integration2` for the TRADER's next restart. TJ-10: a read is graded with no model anywhere - a CLICK is the read, an extracted stance is always labelled as one, an `unmeasured` never supersedes a `pending`, no grade is stored without its context, and the congruence lines are printed, never pushed. TJ-14B: the Mentor asks only what something will READ - every question kind names the reader of its answer and four kinds ship DORMANT until that reader exists, a card asks at most three beyond the forced rows and carries the rest, and one card starts at most one Questrade import. |
| Working branch | **`lead/p033-integration2` on top of `main` `739ce5a4`**, ahead by the TJ-10 merge `57b44ca9`, the integration fix `3e52d94f` and the TJ-14B merge `161e905c`. Nothing is pushed. |
| Unmerged / open | **TJ-4 and TJ-16 are in TEST off the integration tip** - both unblocked by TJ-10 (TJ-16's context snapshot shipped with it). Not started: TJ-12, TJ-5, TJ-6, TJ-7, TJ-8, plus **TJ-14C** - give `quick_like_followup` a reader, then wake it (plan.md 12.4). Free now: TJ-14B's `grader_gap` kind can be woken, TJ-10's field being on this branch. Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` is ~53.7 KB against its ~45 KB rule, `CHANGELOG.md` Recent changes holds 09-07..09-20, `memory/projects/tradingbotv3.md` is over its 15,000-char line. Two known unowned flakes, each green standalone: `test_ws_10a_scan_freshness`, `test_g7_speed_pass`. |
| Next action | **Run the full suite on the integration tip `161e905c`, then a desk-down fast-forward of `main` to it and a restart** - never while a `run_ai_jobs` process is running. A full suite is only a baseline with the nightly AI lock FREE: inside 22:00-06:00 PDT about 42 tests calling the real `runner.run_slots` FAIL rather than skip (`docs/AGENT_TEAM.md`, "The nightly AI lock"). Gates owed, one line each: **#155 (TJ-10)**, unjudgeable until a `day_review/bars/<date>.parquet` tape EXISTS - the live home has no `day_review\bars\` folder yet; **#158**'s TJ-13B clause (the TJ-13A half was read 09-20); **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**, after #146/#152's bars back-fill; **#159** (TJ-14A's first clause, plus TJ-14B's rest); **#160** (TJ-15's first nightly `miss_contrast`); **#163** (TJ-14B: the pull tally survives a restart, the three pre-card pulls land only on the 09:00 / middle / last cards, the overnight question is one click with the old line gone). |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one, then run it again with `--apply --i-am-the-trader` and read those same two lines; keep the printed backup a week; start the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means rows moved, the import rule stayed off and nothing is broken - run it again. To undo: copy the backup back. Both commands: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait a few minutes and type it again. (3) **Open Day Review once and pick Friday 2026-09-18** so its 5-minute tape back-fills: Friday's desk predated the bars writer, so `C:\TradingBotData\day_review\bars\` does not exist yet and every rest-of-day grade reads unmeasured until it does. After 2026-09-20 15:35 PDT restarts are yours again, and the click-gates (#154, #155, #159, #161, #163 and every `staged-pass`) are read on real sessions. From Monday the 09:00 Mentor card will insist you label Friday's trades, and it may add up to three small questions. (4) **The "flat" band is my number, not yours.** A day that moves less than a quarter of SPY's normal daily range counts as flat, so calling "up" on a day that went nowhere is not wrong. I picked a quarter because the packet gave none. Change it whenever you like - but never from one day's result. |
| Last verified baseline | **9,622 passed, 14 skipped, 72 subtests, 0 failed on the integration tip `161e905c`** (everything live + TJ-10 + TJ-14B), nightly AI lock free, 2026-09-20 11:44 PDT; ruff clean, smoke 7/7, source selftest **93/93**. Before it: 9,396 passed + the one known flake (`test_ws_10a_scan_freshness`) on `86c64f96`. |
