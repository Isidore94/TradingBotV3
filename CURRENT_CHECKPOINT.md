...
| Latest work | **2026-09-19: WAVE 1 of Phase 0.33 is MERGED to `main` - TJ-9 (forced 09:00 trade labels, one-click setup, `label_provenance`, planned vs unplanned), TJ-11 (Day Review walk-away v2: a D1 ruler, `REAL_MISS_V1`, the skill line against a base rate, an ADDITIVE `decision_session`) and TJ-13A (local inference night-only seven days a week, each night picks its slate, four overnight repairs).** Every packet went tester -> builder -> reviewer by reproduction. **TJ-11F has been LIVE on the desk since 20:57 PDT (restart log below).** Then, 2026-09-19 evening: **TJ-3 is MERGED into `lead/p033-integration2` (`72647104`) at 21:14 PDT** - the trader's own words drawn on the session tape, and a mark sits on a bar only when it happened DURING that bar. |
| Working branch | **`lead/p033-integration2` (merge `72647104` of `claude/tj3-note-markers`, tip `58ee11f4`) on top of `main` `996b79b7`.** It also carries merge `f00ec302` of TJ-11F. `main` holds wave 1 plus the docs commits and is NOT pushed. |
| Unmerged / open | **WAVE 2 IS IN PROGRESS on `lead/p033-integration2`: TJ-11F and TJ-3 are MERGED.** **TJ-14A is in a fix round after a NO-GO on one blocker** - `read_unchanged` wrote a D1 row with a rest-of-day horizon. **TJ-15 is in a fix round after a NO-GO on what the pack SAYS** - a leader named on 4 values against 1, and M5 decisions, about half the reviewed population, pooled under the D1 ruler. **TJ-9Q has a tester running on `claude/tj9q-questrade-instrument`.** Remaining packets written and unbuilt: `TJ-14B`, `TJ-10`, `TJ-4`, `TJ-12`, `TJ-16`, `TJ-13B`, `TJ-5`, `TJ-6`, `TJ-7`. Machine rule, still binding: ONE full-suite run at a time (the suite is 8-10 GB; three at once gave a MemoryError and a deadlocked run) - builders and reviewers run targeted tests, the lead runs the suite at integration. Doc debts, none blocking: `CLAUDE.md` is ~52 KB against its ~45 KB rule (held flat across TJ-3), `CHANGELOG.md` Recent changes holds 09-07..09-19, `memory/projects/tradingbotv3.md` is over its 15,000-char consolidation line. Two known suite flakes, unowned: `test_ws_10a_scan_freshness` and `test_g7_speed_pass`, each green standalone. |
| Next action | **Land TJ-14A's and TJ-15's fix rounds into `lead/p033-integration2`, then wave 3 = TJ-14B + TJ-10.** Live gate **#147 (TJ-3)** is owed and needs gate #146/#152's bars back-fill first - the live home has no `day_review/bars/*.parquet`, so with no file the SPY chart draws from the Qt hand-off, the caption says the marks were not drawn, and no name chart can open; the gate reads that a leg filled after the last completed bar is COUNTED, never drawn. Full-suite baselines run ONLY while the nightly AI lock is FREE - the `TradingBotV3 AI Jobs` task runs 22:00-06:00 PDT. Read gate #158 on Sunday morning 2026-09-20 from the AI job ledger (tonight is the first SATURDAY slate). Until 2026-09-20 15:35 PDT the lead may restart the desk and read weekend-provable gates itself under `plan.md` 12.6a; the live click-gates stay the trader's. |
...
| Last verified baseline | **TJ-3 merge (`72647104`): 9,166 passed, 14 skipped, 72 subtests, 2 failed + 1 teardown error - exactly the two known unowned flakes `test_ws_10a_scan_freshness` and `test_g7_speed_pass` (41/41 green standalone) - on `72647104` (wave 1 + TJ-11F + TJ-3), nightly AI lock free, 2026-09-19 21:29 PDT; ruff clean, smoke 7/7, source selftest 92/92** (the lead's full suite is running). Before it, TJ-11F merge (`f00ec302`): 9,089 passed, 14 skipped, 72 subtests, 1 failed - the known unowned flake `test_ws_10a_scan_freshness` (green standalone) - nightly AI lock free, 2026-09-19 20:55 PDT; ruff clean, smoke 7/7, source selftest 92/92. Before it, wave-1 integration `8077a758`: 9,062 passed, 14 skipped, 72 subtests, same one flake. |
...
| Desk | **Running from `main` with wave 1 since the lead's 2026-09-19 restart (restart log below); Auto Pilot ON from saved state, idle for the weekend; IB not logged in (TWS is the trader's).** |
