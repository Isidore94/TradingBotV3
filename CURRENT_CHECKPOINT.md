...
| Unmerged / open | **WAVE 2 IS PARKED MID-TESTER (usage limit, 2026-09-19 ~13:15 PDT): three testers died with their work UNCOMMITTED in their worktrees - `claude/tj3-note-markers` (worktree `.claude/worktrees/agent-a21e50edd8e88d1f6`: three untracked `tests/test_tj3_*.py` written, fail-before-fix not yet proven, plus a scratch `tests/_tj3_probe_delete_me.py` to delete), `claude/tj15-miss-contrast` (`agent-acde0fec552f987ac`: premises verified, two untracked `tests/test_tj15_*.py` begun), `claude/tj14a-mentor-card` (`agent-ae55930c6b3f68cca`: nothing written). TJ-3 and TJ-15 branch from `a89ec7d5`, TJ-14A from `8077a758`; all three are ancestors of `main`. To resume: start a FRESH tester per packet on the existing branch and worktree, tell it to read the untracked test files first, finish, prove red, commit.** Packet `TJ-9Q` (Questrade instrument + sold-put side map, split out of TJ-9 item 7) is WRITTEN in `.claude/packets/TJ-9Q.md` and NOT STARTED; its parked tests are on `claude/tj9q-questrade-instrument`. All other packets are written and unbuilt (`TJ-14B`, `TJ-10`, `TJ-4`, `TJ-12`, `TJ-15-16`, `TJ-5-6-7-13B`). Machine rule learned today: ONE full-suite run at a time (the suite is 8-10 GB; three at once gave a MemoryError and a deadlocked run) - builders and reviewers run targeted tests, the lead runs the suite at integration. Doc debts, none blocking: `CLAUDE.md` is ~52 KB against its ~45 KB rule (it was 48 KB before this program), `CHANGELOG.md` Recent changes holds 09-07..09-19, `memory/projects/tradingbotv3.md` is over its 15,000-char consolidation line. Two known suite flakes, unowned: `test_ws_10a_scan_freshness` and `test_g7_speed_pass`, each green standalone. |
