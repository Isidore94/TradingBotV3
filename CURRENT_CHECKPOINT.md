...
| 5B symmetric actual-trade comparison | `WS-5B` / `claude/ws-5b-preference-symmetric` | TESTS RED 2754b122, builder running (lead ruling: P6's opposite-side matching stays; verdicts separate, money once) | Extend `preference_trade_outcomes` with rejections + `like_mode`; wave 2. |
...
| 10E journal tags from setup notes | `WS-10E` / `claude/ws-10e-note-tags` | TESTS RED fd361259, builder running | After WS-AI1. Wave 2. |
...
| 10J Trade Mentor | `WS-TM` / `claude/ws-tm-trade-mentor` | IN BUILD on origin/claude/ws-tm-trade-mentor-build 247a8e43 (schedule/service/card partly built + a WIP commit; a fresh builder reviews the WIP and finishes items 3-6) | Steps 1-2 (scheduler/presence + raw reads; missing-field questionnaire). AI form filling (step 3) deferred. Wave 2. |
