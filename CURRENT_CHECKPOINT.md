...
record behind it, and the newest dated entry wins if they disagree. Refresh it on every
handoff. One sentence per cell, one line per gate; the clause behind a gate lives in the
...
| Latest work | **TJ-10 (`57b44ca9`, fix `3e52d94f`) and TJ-14B (`161e905c`) are LIVE since 2026-09-20 11:48 PDT**, on TJ-13B + TJ-9Q (07:14); `main` `16ec284e` is what the desk runs. TJ-9Q's import switch ships OFF, so nothing in the live journal has moved; its `--apply` and the 27B probe are owed and are the trader's acts (below). **MERGED on the integration branch and NOT live** until the next fast-forward: TJ-16 (`c4a760e5`), TJ-4 (`d929e34f`), the NaN-row bar fix (`ed62bd0a`) and **TJ-12 (`843a3f02`)** - the six-line report card heading Day Review, which computes no statistic of its own, SAYS what it could not see, and wakes the two Mentor kinds it reads. |
| Working branch | **`lead/p033-integration2` tip `843a3f02`, on top of `main` `16ec284e`**, ahead by TJ-16 `c4a760e5`, the seventh slot-order pin `4f131b4c`, TJ-4 `d929e34f`, the NaN fix `ed62bd0a` and TJ-12 `843a3f02`. Nothing is pushed. |
| Unmerged / open | Not started: **TJ-5** (UNBLOCKED by TJ-12; tester starting now) → TJ-6 → TJ-7, then TJ-8; plus two unscheduled follow-ups - **TJ-14C** (give `quick_like_followup` a reader, then wake it) and **TJ-12F** (bounded readers for the Focus-add and armed-alert lanes, then both names in `DESK_ORIGIN_LANES_READ`; until then the Process line and the `trade_origin` prompt NAME them unread). Free now: TJ-14B's `grader_gap` kind can be woken. **Slot-order pins: SEVEN files** (plan.md 12.3); only `day_review_narration` and `observation_tags` may sit between `ai_summary` and `ticker_briefs`. Machine rule: ONE full suite at a time. Doc debts, none blocking: `CLAUDE.md` ~57 KB vs its ~45 KB rule; `CHANGELOG.md` Recent changes holds 09-07..09-20; `memory/projects/tradingbotv3.md` over its 15,000-char line. Two known unowned flakes, green standalone: `test_ws_10a_scan_freshness`, `test_g7_speed_pass`. |
| Next action | **Run the full suite on the integration tip `843a3f02`.** `main` moves only on the TRADER's word, desk down - the lead's restart grant ended 15:35. Never while a `run_ai_jobs` process runs; a baseline needs the nightly AI lock FREE (inside 22:00-06:00 PDT ~42 real-`run_slots` tests FAIL rather than skip - `docs/AGENT_TEAM.md`). Gates owed, one line each: **#155 (TJ-10)**, now judgeable - the `day_review/bars/` tape exists; **#158**'s TJ-13B clause; **#162** (TJ-9Q's `--apply`); **#147 (TJ-3)**; **#148 (TJ-4)** - the morning-after story, verdicts equal to the ledger's by `read_id`, a DAY Redo narrated and cleared next night; **#159** (TJ-14A + TJ-14B); **#160** (TJ-15's first `miss_contrast`); **#163** (TJ-14B's pulls and the one-click overnight question); **#164-#167 (TJ-16)** - the contrast tables off real clicks, the tagger's first grounded night, `tag:` features the night after, `entries_written_after` present and 0; **#157 re-worded + #168-#170 (TJ-12)** - six lines matching the tables and clicking through; `How fresh` naming ONLY slots whose LAST deciding row was not `ok` (`failed`/`degraded` apart), `unknown` past the ledger tail, no `ai_store/logs/` created; the wake asking an origin at most once with its unread-lane caveat, never a date-only fill, AWAY silent; TJ-5's week strip pooling to the day cards' sum. |
| Trader actions owed | **Four things.** (1) **Fix the options in your journal.** In the DAY with the market closed, never 22:00-06:00 Pacific: close the desk, run the LOOK-first command, read page one, run it again with `--apply --i-am-the-trader`, keep the backup a week, restart the desk and check the three puts read SHORT and CLOSED (AAOI +241.03). Exit 4 means rows moved, the rule stayed off, nothing is broken - run it again; to undo, copy the backup back. Commands: DESK_INTERNALS "TJ-9Q". (2) **Measure the big model once**, on a SATURDAY night after 10pm and before 5am: type `.venv\Scripts\python.exe scripts\run_ai_jobs.py --probe-model large`. If a job is running it refuses and says so - wait and type it again. (3) **Click a prediction on the Mentor card** - What I expect, a direction and How sure. No journal row carries one yet, so the were-you-right list stays empty until you do. From Monday the 09:00 card also insists you label Friday's trades, and may add up to three small questions. (4) **The "flat" band is my number, not yours** - a day moving less than a quarter of SPY's normal daily range counts as flat; change it whenever you like, but never from one day's result. Done 16:06 PDT: the `day_review\bars\` tape back-filled 19 sessions through 2026-09-18, so #155 and #147 can now be read - but **after the next restart, re-fetch those 19 bar files**: they were written on 2026-09-20 by the old code and are missing the names it dropped. |
| Last verified baseline | **9,807 passed, 14 skipped, 72 subtests, 0 failed on `d929e34f`** (live + TJ-16 + TJ-4), AI lock free, 16:52 PDT; ruff clean, smoke 7/7, source selftest **95/95**. **TJ-12 on top (`843a3f02`): 9,913 passed, 14 skipped, 72 subtests, 0 failed, 18:25 PDT; ruff clean, smoke 7/7, selftest 96/96** (the pack-wiring follow-up `ccecf320`: 81 targeted green, full suite rides with TJ-5's) on `843a3f02`; on that tip the reviewer measured 102 `test_tj12_*` + 766 neighbours passed, ruff clean, source selftest **96/96**. |
