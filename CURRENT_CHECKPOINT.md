...
| Unmerged / open | **Nothing from the ST lane.** G lane: G2a merged; **G2b BUILT and unmerged on `claude/g2b-tracker-desk-away-columns`, awaiting review** (it edits `setup_tracker_panel.py`'s `_make_table` and its construction lines only, so it merges cleanly with `claude/g4b-setup-tracker-detail`'s detail-pane work on the same file); `claude/g4b-setup-tracker-detail` and `claude/g5-research-results` have testers writing red tests on `7e018c99`; queued after them G7 speed pass, then the Journal packet the G2a tester surfaced (the Trades splitter opens 39/61 against its declared 3:2; the blank space above the table). G5 Research > Results (consumes ST2's counts and ST6's snapshot), G7 - ST6 has LANDED, so the Phase 0.22 hold on `setup_tracker_panel.py` / `master_avwap_panel.py` / `away_recap_panel.py` / `alert_center_panel.py` / `research_panel.py` is lifted (`legacy.py` stays ask-first). Open from before: `claude/s1-quick-verbs`, the 2026-09-03 lake incident call, the lock-file sweep packet, the `test_chart_snapshot` cooldown flake. **Two new packets owed:** (1) 100 of 1,980 cached daily-bar files end in a FORMING candle (open outside [low, high]) on seven sessions and the replay reads them (ST3's counter); (2) CLAUDE.md is 50.1 KB against its ~45 KB rule. |
...
| Last verified baseline | **`claude/g2b-tracker-desk-away-columns` at `8155feeb` (with `origin/main` `6d9f4b43` merged in), 2026-09-07, in the builder's worktree, nightly AI lock FREE (probed): `pytest tests/ -q` with NOTHING DESELECTED: 7208 passed, 3 skipped, 72 subtests passed, ZERO failures, process exit 0, 11 min 34 s; ruff clean; smoke 7/7; source selftest 74/74; CLAUDE.md == AGENTS.md (51,229 bytes).** The +16 over the merge baseline is exactly G2b's own tests (the tester's nine, the builder's four, and `test_table_width_rule.py`'s three). No packaging trigger: no new dependency, no non-`.py` asset, no new top-level `scripts/` package, no dynamic import. Previous: `lead/merge-g` at the G2a merge on top of `7e018c99`, 7192 passed, exit 0 (the G2a branch alone on `6de5aced` ran 7141); `main` `7e018c99`, 7187 passed. |
