...
| 6 Theta pick tracker | `WS-TH` / `claude/ws-th-theta-tracker` | IN BUILD on origin/claude/ws-th-theta-tracker-build ee676ae5 (items 1-4 + slot pins pushed; suite/handoff not received - a fresh builder finishes from the tip) | Decisions: one row per (symbol, scan_date); graded at the sold put's expiry AND marked at 5/10/20 sessions; the readout's cohort is FIRST appearance with a repeat count. Written from the runner after the scan (never legacy.py's save pass). |
...
| 10H more chart history | `WS-CH` / `claude/ws-ch-chart-history` | IN BUILD on origin/claude/ws-ch-chart-history-build dd00c21b (items 1-2 pushed + a WIP commit of uncommitted edits; a fresh builder reviews the WIP, finishes item 4 docs, runs the gates) | Wave 2. |
