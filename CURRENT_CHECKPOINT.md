...
| **Active packet** | **R3 SWING QUALITY AND FEEDBACK — SHADOW CLASSIFIER + POST-CLOSE SCHEDULING BUILT; STABLE/PREVIEW NEXT** (`docs/SWING_QUALITY_AND_FEEDBACK_PLAN.md`). The classifier emits additive `would_demote` evidence without changing membership. The schedule now adds a 12:45 PT forming-bar preview and gives the actual close slot sole tracker-write ownership; later manual runs/catch-up retain recovery. Completed-bar STABLE output, explicit row stamps, and the dislike loop remain. The full shadow/comparison weeks remain UNKNOWN/owed; no row moves, reorders, hides or demotes. R4–R6 follow only after R3's deterministic build and governance close-out |
...
| State | **R3 post-close scheduling milestone green: 3363 passed / 19 subtests, exit 0**; focused gate **303 passed**, exit 0. Production journal code, live stores and brokers remain untouched. Prior smoke 7/7 and clean-cache frozen selftest 49/49 `(frozen)` remain the last applicable non-pytest gates |
| Next action | Run the full gate and push the one-writer scheduling milestone, then build the completed-bar STABLE output + forming/completed stamps from the frozen replay seam, followed by the structured dislike-reason loop. Monday's one-merge sequence remains owed and unchanged |
...
|---|---|---|
| Full suite | `.venv\Scripts\python.exe -m pytest tests/ -q` | **3363 passed / 19 subtests**, exit 0 — check pytest's own exit code, not a piped tail |
