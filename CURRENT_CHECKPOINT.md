...
owed. No frozen rebuild. Recorded as **BD-80 … BD-84**; 78/79 belong to the unmerged
...
**1. Episodes beside rows (BD-80). THE MEASUREMENT CHANGED THE CONCLUSION.** Every cell
...
BD-80 records that the follow-up must be a CROSS-CELL floor - not the per-cell swap,
which on today's data would change nothing at all.

**2. The eligible block leads (BD-81).** Eligible whole and sorted as before, then a
...
**3. Non-trade roles excluded and named (BD-82).** An explicit map; everything unnamed
...
**4. Coverage published (BD-83).** New `research_warehouse/outcome_coverage.py`,
...
**5. The readout is not hard-filtered (BD-84).** `slice_readout(setups=...)` - omitted
