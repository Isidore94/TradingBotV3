...
| | |
|---|---|
| Latest work | **2026-09-15, DR-REPAIR built on `codex/daily-recap-repair`: Daily Recap streams and reduces M5 states, shows one whole best event per stock/side, separates M5 from D1 decision outcomes, renders pending swings and a factual summary, and refreshes at noon plus once after the exchange close (gate #125 owed).** Before it, two trader asks built by the lead on the combined desk branch `claude/desk-combined-2026-09-14`: (SC) a chart opened from the Master AVWAP setups table cycles to the table's next row on veto / claim / Next (`chart_symbol(next_pick=)`, consumed by `_advance_review_queue`), and a row vetoed for the day is HIDDEN and counted (`Show vetoed (N)`, per symbol, `pick_feedback.HIDDEN_REJECT_KINDS`) while the tracker keeps tracking it; (DTR) the day-trade watchlists are wiped after the close (decision 0020).** New `scripts/daytrade_watchlist_reset.py` (stateless: a `longs.txt` / `shorts.txt` written at or before the last completed session's close is emptied, one `session_reset` intent row per name, dry-run CLI), `AutopilotService._maybe_reset_daytrade_watchlists` on every tick before the weekend short-circuit and the open scan, switch `daytrade_watchlists_reset` default ON; plan.md sec 5's watchlist invariant amended for these two lists only. Before it, 2026-09-14: the Daily Recap fills itself in at 12:00 Pacific (trader-directed WS-DR follow-up), built by the lead on the combined desk branch `claude/desk-combined-2026-09-14`.** New pure `scripts/daily_recap_schedule.py`, one `QTimer` on `DailyRecapPanel` started in `showEvent`, setting `daily_recap_auto_time` (default 12:00), once per session, provisional until the 13:00 Pacific close, the picker relabelled after the close; the WS-DR wall-clock flake is fixed. Before it: the two 2026-09-14 streams (Codex's Mentor pop-up/context, M5 left-grade and chart-symbol isolation; Claude's claimed D1 picks) combined on this one branch at 84d4ca10 with every other branch deleted after a backup bundle. Live checks remain open. |
...
| # | Gate | Owed by |
|---|---|---|
| 125 | **Daily Recap is compact and closes itself (DR-REPAIR, Phase 0.29, `codex/daily-recap-repair`)** - after the next restart, open Daily Recap after noon: each M5 stock/side appears once with its best measured alert, pending swings stay visible as `Pending` with dashes, the summary states update/event/decision/swing counts, and a D1 decision shows its D1 result or pending/unavailable state rather than `Day best`. After the exchange close (including a half day), the page refreshes once without a click and its Today entry is no longer provisional; a later timer tick does not start another read. | trader, next session |
| 124 | **The setups table cycles and a veto hides the row (SC, Phase 0.28, trader 2026-09-15, `claude/desk-combined-2026-09-14`)** - next restart, on the Trading Desk: click a symbol in the Master AVWAP setups table so it charts in the centre; veto it on the capture rail - within a second the centre chart shows the NEXT visible setups row (the table's selection moves with it), the vetoed row is gone from the table and the strip's box reads `Show vetoed (1)`; tick the box and the row is back in its old place with a red ✕. Alt+K plus a double-clicked setup on a setups chart also moves to the next row and the claimed row STAYS (`My liked trade`). Reach the last row and veto it: the status row reads `End of the setups list - nothing after X` and the pane shows the waiting list's next chart or "waiting". A chart from the lookup box or a board still advances to the waiting list as before. PASS on day two: Research > Setup Tracker still carries the vetoed symbol's outcome rows (the hide is presentation only). | trader, next restart |
...
**Second ask the same morning (SC - the setups table cycles and hides; commit after ea2dc7bb).** Trader: *"when i
click on master avwap setups tab and then I click the veto or like and claim buttons it should cycle it to
the next pick. additionally vetoing it for the day SHOULD remove it from the list (but the stock should
still be tracked for setup tracker purposes)"*. Recon (Sonnet, read-only) verified the premises: a
setups-table chart is a `MANUAL_CHART_TAG` look with no place in the waiting list, both retire paths end
in `_advance_review_queue` over the ordinary queue, and WS-SX's ✕ hid nothing. Built by the lead:
`AlertCenterPanel.chart_symbol(next_pick=)` -> `_manual_next_pick`, consumed by `_advance_review_queue`
before the waiting list and dropped by any non-manual chart; `reviewDecisionRecorded` after a rail veto
and a placed claim, connected on the desk to `MasterAvwapPanel.refresh_decisions`;
`MasterAvwapPanel._chart_row_on_desk` / `_chart_next_pick` (visible order, identity-first lookup, same
symbol and today's rejects skipped, selection follows, `End of the setups list` when out);
`pick_feedback.HIDDEN_REJECT_KINDS` + `DayDecisions.rejected_symbols()`;
`SetupFilterProxyModel.set_filters(rejected_symbols=, show_rejected=)` + `hidden_rejected()`; the
`Show vetoed (N)` box (`qt_setups_show_vetoed`, default off). Presentation only; no ask-first file
touched; the tracker never reads the filter. Lead decisions the trader may overrule: the table's own ✕
hides too, the hide is per symbol, a day-trade pass and an M5 click-away hide nothing. WS-SX's
moves-nothing test rewritten for the new contract; three WS-SX paint tests found order-dependent
(pre-existing, pass after the desk-build test, not fixed). Tests: `tests/test_setups_cycle_and_veto_hide.py`
(16). Full suite 8221 passed, 1 failed, 6 skipped, 72 subtests in 754 s (exit 1: the one failure is tests/test_ws_10c_h1_retester.py::test_nothing_new_reaches_the_retired_h1_emitter_seam, which globs the whole repo folder and found the copy of legacy.py inside ANOTHER session's worktree `.claude/worktrees/pullback-lead` (branch `claude/pullback-compression-2026-09-15`, created 09:13 PT today, not this build's); it fails alone for the same reason and passes once that worktree is gone); ruff clean. Docs: CLAUDE.md/AGENTS.md (SC rule), plan.md (Phase 0.28 widened),
CHANGELOG (inventory + Recent changes), DESK_INTERNALS "SC", gate #99 amended, memory. Gate #124.
