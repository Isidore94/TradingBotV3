...
| 67 | **The control and study populations are shown, and the April framework is read (M5)** - after the next persisted tracker write: `master_avwap_control_discovery.csv` and `master_avwap_study_discovery.csv` exist with rows (the control file carrying its three cohort rows plus one family row per (side, family), in an `all` block AND a `lately` block); the Setup Tracker's **Controls** and **Studies** tabs render them with the population sentence above each table (`N control setups the scan REJECTED ...`), sorted by the Wilson lower bound; and the **Exit frameworks** tab shows `comparison_apr2026` rows beside `baseline` rows with the same `n` and the Experimental column reading True. **Read one control row and one baseline/comparison pair aloud before believing the tab**: the comparison's whole value is that the two share a setup population, and an n that differs between them means the pairing is wrong, not that the framework lost | 2026-09-05 M5 entry |
