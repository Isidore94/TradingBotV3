...
| 10C H1/H4 retester | `WS-10C` / `claude/ws-10c-h1-retester` | PACKET (after CH) | Step 1 only (H1 15-EMA bounce watch, opt-in arm button, versioned rule sheet). Wave 3. |
...
| 10F Daily Recap | `WS-DR` / `claude/ws-dr-daily-recap` | TESTS RED 3e1c1973 (45 red + 2 guards); builder NOT started before the 12:15 pause - next session spawns it (tester findings: pick_feedback/alert_review_events `ts` are NAIVE, attach a zone; the M5 outcome side column is `direction` and `mfe_pct`/`eod_move_pct` are already side-adjusted; replacing the PAGE_SPECS entry breaks four tests that address 'AWAY Recap' by title - keep away_recap_panel.py on disk) | Session reader over durable stores first, then the four views. Wave 2/3. |
| 10G one Watchlist tab | `WS-WL` / `claude/ws-wl-watchlist-tab` | TESTS RED d9f44da2 (tester's handoff pending its suite run); builder NOT started before the 12:15 pause - next session spawns it | Wave 3. |
...
| 10I thesis/context/setup join | `WS-10I` / `claude/ws-10i-context-join` | PACKET (after ENV, 10D, DR) | Deterministic join contract + module after WS-ENV; wave 3. |
