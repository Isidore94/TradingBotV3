...
**Q3.2** A statement matching `POSITION_CLAIM_PATTERNS` is DROPPED unless a
`journal`-kind source survives among its refs
(`detail: "position claim without a journal source"`); a statement stating a
...
**Fail-before-fix.** `tests/test_q3_ai_grounding.py` (21 tests) was committed RED at
`1f1f5d1` with 19 failing on `6b74165`; the two that passed are deliberate controls (a
journal-cited position claim must still survive; an all-valid payload must still
validate unchanged). Proven again after the fix by restoring all five source files to
`6b74165` - the same 19 failed - and restoring.

**Verification** (builder worktree, nightly AI lock probed FREE first): full
`pytest tests/ -q` with nothing deselected - **6629 passed, 1 skipped, 72 subtests,
exit 0, 6 min 29 s**, which is +21 on the 6608 baseline and exactly the 21 new tests.
...
**Known residual, named rather than silently accepted:** the four Market Journal ids
(`journal.evidence_report`, `journal.day_context`, `journal.chart_digests`,
`journal.entries`) read as kind `journal` because the packet specified a family-keyed
table, so a position claim citing the trader's free-text market journal survives. Only
`journal.trades_and_reviews` carries per-trade `status`. Narrowing the position rule to
a `POSITION_SOURCE_IDS` set would close it; it was not authorized here.
