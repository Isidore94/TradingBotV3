...
not carry at all. Reading an absent column as "not closed" would have zeroed the live
scoring population exactly the way the first ST4 build did (32 recent family rows -> 0, 74
nonzero `setup_type` deltas -> 0), so `_row_is_graded` falls back to the row's own
`closed_setups` when the status is ABSENT, and `no_representative_in_population` counts
those rows. A status that is PRESENT and `pending` is still never graded. Without this the
first D1 scan after merge writes zero score deltas; gate #84 clause (5) is the check.
