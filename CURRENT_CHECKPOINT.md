...
| Latest work | **2026-09-19 (night): TJ-15 and TJ-14A are MERGED into `lead/p033-integration2`, reaching `main` after the night's AI run - wave 2 is complete.** TJ-15 is what the misses had in common (the pure `evidence_contrast` with TWO floors, a deterministic `miss_contrast` slot inside stage 1 above the pair that closes it, D1 decisions only, the point-in-time join reaching one session back; the lead's `1f260ffa` moved the slot); TJ-14A splits the Mentor card into What I see / What I expect with a forced prediction click, makes a row's timeframe and its horizon always agree at the WRITER, and gives the card `trade_mentor_context_v2` and its internals strip. **TJ-3 has been LIVE on the desk since 21:30 PDT** (restart log below). Earlier the same day: WAVE 1 (TJ-9, TJ-11, TJ-13A) merged to `main`, then TJ-11F and TJ-3 into `lead/p033-integration2`. |
| Working branch | **`lead/p033-integration2` on top of `main` `6f8113d7` (the TJ-3 restart-log docs commit; the desk runs its code tip `72647104`).** The branch is ahead of `main` by TJ-15 (`fb3f55e9`), TJ-14A (`e8c04f88`) and the lead's slot-position fix `1f260ffa`, over the earlier merges of TJ-11F (`f00ec302`) and TJ-3 (`72647104`). Nothing is pushed. |
| Unmerged / open | **WAVE 2 IS COMPLETE on `lead/p033-integration2`: TJ-11F, TJ-3, TJ-15 and TJ-14A are MERGED** and reach `main` after the night's AI run. Running now: **TJ-9Q's builder** on `claude/tj9q-questrade-instrument` (tests red at `55fbf731`; the live `--apply` is the TRADER's act), **TJ-13B's builder** (red at `43124c2c`), and **TJ-14B's and TJ-10's testers** off `e8c04f88`. Written and unbuilt: `TJ-4`, `TJ-16`, `TJ-12`, `TJ-5`, `TJ-6`, `TJ-7`, `TJ-8`. Machine rule, still binding: ONE full-suite run at a time (the suite is 8-10 GB; three at once gave a MemoryError and a deadlocked run) - builders and reviewers run targeted tests, the lead runs the suite at integration. Doc debts, none blocking: `CLAUDE.md` is ~52 KB against its ~45 KB rule (TJ-15 and TJ-14A each paid for most of their bullet by trimming their own section; +590 bytes net across the two), `CHANGELOG.md` Recent changes holds 09-07..09-19, `memory/projects/tradingbotv3.md` is over its 15,000-char consolidation line. Two known suite flakes, unowned: `test_ws_10a_scan_freshness` and `test_g7_speed_pass`, each green standalone. |
| Next action | **A CLEAN full-suite baseline on the integration tip once the nightly AI lock is free** (the 21:50 run straddles the lock and is a regression pass only, so both merged entries below read `9,264 passed, 14 skipped, 72 subtests, 2 failed - exactly the two known unowned flakes `test_ws_10a_scan_freshness` and `test_g7_speed_pass` - on `1f260ffa` (wave 1 + TJ-11F + TJ-3 + TJ-15 + TJ-14A), 2026-09-19 22:03 PDT; the run began at 21:49 so every AI-lock test ran before the 22:00 task start and none stood down (the usual 14 skips); ruff clean, smoke 7/7`), **then a desk-down fast-forward of `main` and a restart - never while a `run_ai_jobs` process is running** (the `TradingBotV3 AI Jobs` task runs 22:00-06:00 PDT from the main checkout). Then wave 3 = TJ-14B + TJ-10. Gates owed, one line each: **#158** read Sunday morning 2026-09-20 from the AI job ledger (tonight is the first SATURDAY slate); **#147 (TJ-3)**, which needs gate #146/#152's bars back-fill first; **#159** (TJ-14A's first clause) and **#160** (TJ-15's first nightly `miss_contrast` run) - each clause is in its dated entry below. Until 2026-09-20 15:35 PDT the lead may restart the desk and read weekend-provable gates itself under `plan.md` 12.6a; the live click-gates stay the trader's. |
| Trader actions owed | **Nothing today. Two things to know: (1) your SOLD PUTS are recorded backwards in the journal (they read LONG, three look stuck open - it was like that before today); packet TJ-9Q will give you a dry run to read before anything in the live journal changes, and the `--apply` is yours. (2) your Friday-evening rule is BUILT (TJ-11F): a call made after Friday's close or on the weekend now reads on FRIDAY's Day Review, measured from Friday's close; nothing on the live desk changed.** After 2026-09-20 15:35 PDT restarts are yours again; then read the click-gates (#154, #155, #159, #161 and every `staged-pass`) on real sessions. From Monday the 09:00 Mentor card will insist you label Friday's trades. |
| Last verified baseline | **The wave-2 tip (`1f260ffa`, TJ-15 + TJ-14A + the lead's slot fix) has NO clean full-suite count yet: 9,264 passed, 14 skipped, 72 subtests, 2 failed - exactly the two known unowned flakes `test_ws_10a_scan_freshness` and `test_g7_speed_pass` - on `1f260ffa` (wave 1 + TJ-11F + TJ-3 + TJ-15 + TJ-14A), 2026-09-19 22:03 PDT; the run began at 21:49 so every AI-lock test ran before the 22:00 task start and none stood down (the usual 14 skips); ruff clean, smoke 7/7** - that run waits for the nightly AI lock to be free. Before it, TJ-3 merge (`72647104`): 9,166 passed, 14 skipped, 72 subtests, 2 failed + 1 teardown error - exactly the two known unowned flakes `test_ws_10a_scan_freshness` and `test_g7_speed_pass` (41/41 green standalone) - nightly AI lock free, 2026-09-19 21:29 PDT; ruff clean, smoke 7/7, source selftest 92/92. Before that, TJ-11F (`f00ec302`) 9,089 passed and wave-1 `8077a758` 9,062 passed, the same flakes. |
...
| Desk | **Running from `main` since the lead's 21:30 PDT restart - wave 1 + TJ-11F + TJ-3, code tip `72647104` (restart log below); Auto Pilot ON from saved state, idle for the weekend; IB not logged in (TWS is the trader's).** |
...
| 158 | **The night serves the trader first (TJ-13A MERGED 2026-09-19; TJ-13B planned)** - the morning brief lists no membership-only name, `ai_summary` is synthesized or fails fast and runs Saturday night only, no ledger row starts between 06:00 and 22:00 Pacific on any day, Sunday night's ledger shows only the deterministic stage plus whatever the weekend still owed; then (TJ-4) the day story before 23:30 Pacific and (TJ-13B) a week story whose ledger row names the large local model. FIRST READ owed after tonight's Saturday slate (2026-09-19 22:00 PDT). | lead, first weekend after TJ-13A |
