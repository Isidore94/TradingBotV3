...
| 2 FC1 forming candles in the daily-bar cache | `WS-FC1` / `claude/ws-fc1-forming-candles` | TESTS RED 98cad100, builder running | Build the writer guard + manifest counts + `python -m master_avwap_lib.daily_bar_cache repair` (dry run default). `--apply` on the live cache stays the TRADER's action. legacy.py writer seam only - the trader's FC1 prompt is the recorded yes. |
| 3 EF1 exit frameworks by family | `WS-EF1` / `claude/ws-ef1-exit-by-family` | TESTS RED 3ef97b3d, builder running | Second export + family picker on the Exit frameworks tab. legacy.py export seam only - the EF1 prompt is the recorded yes. |
...
| 4 (points) block 4 - AWAY digest ranks swing picks by points | `WS-PT4` / `claude/ws-pt4-digest-points` | PACKET | Decision: the digest ranks by points ONLY when the existing Points switch is ON; OFF keeps today's Wilson order. Weight tuning stays the trader's. |
| 5A weekend verdict (confirmed defect) | `WS-5A` / `claude/ws-5a-weekend-verdict` | PACKET | Typed numeric contract, horizon/side explicit, percent never labelled R, "weakest veto reason" becomes "rejected names with the highest side-adjusted return". |
...
| 5D watchlist intent events | `WS-5D` / `claude/ws-5d-watchlist-intent` | PACKET, builder running | Append-only dated add/remove event at the trader-edit seam; machine writers labelled; an external diff labelled `observed_external`. |
