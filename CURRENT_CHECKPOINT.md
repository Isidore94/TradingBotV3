...
| Last verified baseline | `pytest tests/ -q` **6053 passed, 72 subtests** on the merged tree (2026-09-02) plus the one real break it found and fixed. **THE 32 `ai_jobs` FAILURES ARE NOT A BASELINE AND WERE NOT TREATED AS ONE**: they stand down while the machine-local `ai_jobs_runner` writer lock is held, and it was held from 22:00 through this whole round by `scripts/run_ai_jobs.py` (pid 24336, started 2026-09-01 22:00:04) - a real, working nightly run, not an orphan; the 2026-08-31 run took until 02:52 for comparison. **THE FULL SUITE WITH THE LOCK FREE IS THE ONE THING THIS ROUND STILL OWES**, and the command to check the lock is in the R1 entry below. `ruff` **clean** · smoke **7/7** · source `--selftest` **74/74** |
...
| Desk restart | **OWED, AND IT MATTERS MORE THAN USUAL.** The running desk (pid 17132) loaded its code on 2026-09-01 11:09 and does not know `tag_status`, so **the 24 provisional tags P6a wrote into the live journal at 21:50 that night render as the trader's own** until it restarts - the one thing P6a exists to prevent. Verified on the live database: `trade_annotations.tag_status` and `auto_tag_candidates.context_row_id` both exist (1 confirmed / 24 provisional / 132 needs_review), and the MERGED migration is a proven no-op on a copy of it. The restart is the trader's call and was not made here |
