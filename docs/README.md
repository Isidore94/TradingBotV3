...
| [`SWING_SIMULATOR_INVESTIGATION_2026-09-04.md`](SWING_SIMULATOR_INVESTIGATION_2026-09-04.md) | Follow-up to the lake assessment's 0/257 swing wins finding. Root cause: `feature_snapshot_daily` AVWAP bands are 99.15% null (234/27,579 rows), so 942/947 occurrences run a no-target fallback path where wins are structurally impossible. The simulator logic is correct; the band data pipeline is starved. The control recipe proves the occurrences themselves have a 45.3% WR. Recommendations: fix band pipeline, then recompute outcomes. |
