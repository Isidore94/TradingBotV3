...
| [`AGENT_TEAM.md`](AGENT_TEAM.md) | How a Claude Code session plans, builds, reviews and integrates through the project's sub-agents (`.claude/agents/`) |
...
| [`ROADMAP_ARCHIVE_PHASES_0.8-0.13.md`](ROADMAP_ARCHIVE_PHASES_0.8-0.13.md) | The verbatim build narrative for `plan.md` Section 12's Phases 0.8, 0.9, 0.11, 0.12 and the Phase 0.13 packets and review rounds, moved out 2026-09-03 (F1 docs packet). Each phase leaves a stub in `plan.md` with its status; every live gate still owed is a row in `CURRENT_CHECKPOINT.md`'s gates table. Evidence, not authority. |
| [`CHANGELOG_ARCHIVE_2026-08-26_2026-09-01.md`](CHANGELOG_ARCHIVE_2026-08-26_2026-09-01.md) | `CHANGELOG.md`'s revision history from **2026-09-01 back to 2026-08-26** (56 entries), split out 2026-09-03 so `Recent changes` holds the last two build days again. What currently exists is the `Current implemented inventory`. |
| [`CHECKPOINT_ARCHIVE_2026-08.md`](CHECKPOINT_ARCHIVE_2026-08.md) | The dated working record from `CURRENT_CHECKPOINT.md` for **2026-08-31 and earlier**: the 2026-08-26 to 2026-08-31 entries moved 2026-09-03 (F1 docs packet; their open gates stay in the active table), the 2026-08-25-and-earlier entries (95, 379 KB) on 2026-08-27. The active checkpoint had reached 7,901 lines and made the mandatory documentation read ~260k tokens, so it was no longer followable. Evidence for one specific past decision — **never load it as context**, and never treat an entry here as an open gate. Open gates live in the active checkpoint's "Active state at a glance" block |
...
| [`analysis/LAKE_ASSESSMENT_2026-09-04.md`](analysis/LAKE_ASSESSMENT_2026-09-04.md) | Read-only assessment of the repaired research lake: Q1-Q8 on integrity, swing/day-trade/HTF LRSI evidence, likes, coverage. Verdict: integrity passes; swing_house_v1 0/257 wins (simulator investigation needed); all M5-close recipes negative mean net_r (MFE confirms the opportunity; exit research is the path); like-link basis field broken (all 74 links carry `basis=unknown`). Scripts at `analysis/scripts/lake_assessment.py` et al. |
| [`SWING_SIMULATOR_INVESTIGATION_2026-09-04.md`](SWING_SIMULATOR_INVESTIGATION_2026-09-04.md) | Follow-up to the lake assessment's 0/257 swing wins finding. Root cause: `feature_snapshot_daily` AVWAP bands are 99.15% null (234/27,579 rows), so 942/947 occurrences run a no-target fallback path where wins are structurally impossible. The simulator logic is correct; the band data pipeline is starved. The control recipe proves the occurrences themselves have a 45.3% WR. Recommendations: fix band pipeline, then recompute outcomes. **Resolved 2026-09-04**: the scan now feeds `earnings_avwap_anchors.csv` (earnings-anchor bridge, gate #59); its Resolution section records this. |
