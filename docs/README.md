...
| [`M5_SIGNAL_ENGINES_PLAN.md`](M5_SIGNAL_ENGINES_PLAN.md) | Phase 0.5 R5 — **IN PROGRESS**: §2's pure SMI/efficiency-LRSI/Heikin-Ashi modules are **BUILT 2026-08-16** with 42 hand-computed tests and no packaging trigger yet. All wiring (new M5 alert types, AnyBounceWatch, first-candle ORB, the shared completed-bars helper) is unbuilt and **blocked on §8's Alert Center lane question**; §9 carries the build state, the two answered §8 questions, and the packaging/reconciliation rules for the wiring commit |
