...
| [`AVWAP_BAND_VARIANT_STUDY.md`](AVWAP_BAND_VARIANT_STUDY.md) | **PROPOSAL 2026-08-26, nothing built, not in `plan.md`**: replicate the second charting program's anchored-VWAP σ (non-zero on the anchor bar, ≈1.5× the champion after 13 bars) by a kill-the-candidates fit against hover readings, freeze it as a pure `indicators/` module, then grade it against the frozen champion in three shadow harnesses (level-quality backfill, playbook re-run, setup-tracker shadow stops) under pre-declared criteria. Never a swap of `calc_anchored_vwap_bands` (decision 0008) |
