...
| [`M5_SIGNAL_ENGINES_PLAN.md`](M5_SIGNAL_ENGINES_PLAN.md) | Phase 0.5 R5 — **IN PROGRESS**: §2's pure SMI/efficiency-LRSI/Heikin-Ashi modules and §5's shared completed-bars rule are **BUILT 2026-08-16**; no packaging trigger has fired yet. §3/§4 wiring (new M5 alert types, AnyBounceWatch, first-candle ORB, prior-anchor AVWAP line) is next, LRSI cross first. **All three §8 questions are answered** — §8.1 records the `M5_SIGNAL_TAG` lane decision and its consequences; §9 carries build state and the packaging rules the wiring commit must honour |
