# Roadmap archive — Phases 0.5, 0.6 and 0.7, long form

Verbatim build narrative for the three completed refinement programs in
[`plan.md`](../plan.md) Section 12, moved here on 2026-08-28.

**`plan.md` keeps the roadmap; this keeps the story.** Those three phases were 97,886 B
of Section 12's 135,762 B — 72% of the roadmap — describing work that is BUILT. What
`plan.md` must carry is each item's status and the gate still owed, and it still does:
all 89 numbered items and every gate clause survive there, verbatim and unabridged
(including multi-part gate lists such as R1's drain-on-return / EVENING-day /
SPY-alarm trio). What moved here is the implementation description behind them, which is
also in `CHANGELOG.md` and in each packet's own spec under `docs/`.

**This is evidence, not authority.** An owed gate is owed because `plan.md` says so, not
because a paragraph here describes it. If the two disagree, `plan.md` wins.
