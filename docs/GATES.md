...
- #200 Rule loop: after a recap sets a rule, the next morning the status bar shows `Rule: ...` (tooltip = streak) and Research > Market Prep shows `Today's rule: ... (streak N)`; a trade that breaks a checkable rule gets ONE "kept it or broke it?" on a later Mentor card, never more than 3 a day [trader]
