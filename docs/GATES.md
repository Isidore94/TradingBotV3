...
- #204 Movers board: in market hours the MOVERS board (top of the Alert Center's right column) refreshes each 5-minute bar (header = SPY bar time, banner "N of M fresh") and a row click charts it; during an up-day SPY pullback the Dip-strong chip gets its dot, the board jumps to it once, and its top names held up vs SPY on the chart; Review ▾ opens Focus/Faded review; Deep read shows the old Strength page; `ui_stalls.jsonl` shows no stalls from it; ER, Lvl (HOD brk/ext), ▲/▼/new and "Groups:" match the charts; +F on a row lands it in M5 Focus (or names the gate's refusal); after a pullback episode `movers_dip_outcomes.jsonl` has flag and outcome rows and `python scripts/movers_outcomes.py --summary` prints them [trader + lead]
