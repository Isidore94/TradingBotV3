...
- #223 Daily-bar batch (P0-2 2e): the next scan's trading_bot.log has one `[daily-bar cache] lookups=... hits=... misses=... refreshes=... served_from_batch=... batch_calls=...` line, no `Daily-bar batch ... failed` warning, and `[run_master timing] prep+fetch+priority` is shorter than the 450 s baseline [lead]
