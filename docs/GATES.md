...
- #264 Night telemetry: the next weeknight ledger has `tokens` on every model-calling slot row, the digest facts carry 'slots per goal' and 'unread 14+ days' lines, and Health shows the Night AI and Broker import rows [lead]
...
- #266 Missing-inputs chip (B4): with an open trade lacking a stop the status bar shows 'Inputs: N ...', one click opens the Mentor on the oldest such trade at the stop question, and the chip disappears once every trade in the last 30 days has stop and confirmed setup [trader]
...
- #257 Night AI fixes: the next weeknight ledger has `improvement_ideas` ok (not a "which tonight does not carry" rejection); `setup_research` either says "narrated K of N" with numbers in its statements or "narration absent" with "quoted no number"; enrichment rows with empty evidence carry no tags; no low-memory event 2004 in the System log that night [lead]
