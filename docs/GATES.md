...
- #204 Journal P&L: with the desk closed run `journal_pnl_repair.py` (dry run) then `--apply --i-am-the-trader`; the two 2026-06-12 CVNA put legs are CLOSED and net -65 together, no open CVNA BAG, IBKR fill times match TWS, and Analytics (range All) shows 'N trades need missing fills - not counted' [trader]
