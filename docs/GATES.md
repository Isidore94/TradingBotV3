...
- #226 Permutation stamp (P1-4 4a): after the next scan, the newest `d1_features_history.csv` rows carry `dist_*_atr`, `permutation_key` and `permutation_rule_version` = setup_permutations.v1, no "Setup permutation stamp skipped" warning in the log, and the scan's run time is within a few seconds of the day before [lead]
