...
- #284 Regime label + conditional long search (S15 items 2, 4, 8): after the next scan the newest `d1_features_history.csv` rows carry `perm_regime_as_of`, `perm_spy_vs_sma20_pct`, `perm_breadth_above_sma20_pct` and `perm_regime_working` / `_rule` (the trader's label in `perm_regime_trader` once S16's regime is typed); after the next universe rebuild the ledger's `universe_rebuild` row has a `journal_traded` stage count and DRAM is in `universe_all.txt`; the Saturday permutation report (backfill run with `--structural-regime <journal copy>`) has a `swing_long_working_raw` population with its definition, named horizons and `working_rules` counts [lead]
...
- #282 Grades per regime (S16 item 3): once the trader has typed the regimes (#279), after the next Working-lately build the Setup Tracker's 'By regime' tab and the Daytrade Tracker's 'This regime' / 'By regime' columns show every grade per regime with the current one first, 'untested in this regime' where it has no rows, and 'all regimes' = the unchanged badge grade [trader]
...
- #280 Regime table (S17): after Monday's close and that night, `C:\TradingBotData\data\runtime\market_regime_table.jsonl` has Monday rows for SPY, QQQ and IWM with six timeframes (M5, M30, H1, H4, D1, W; QQQ/IWM intraday may read unknown until the lake holds their M5) plus the structure facts, and the desk's regime strip under Entry assist shows SPY/QQQ/IWM with six cells each [lead]
- #279 Regime journal (S16 item 1): the trader answers the Mentor's weekly regime question once (or uses the card's Regime button) and confirms the three past regimes; `trade_journal.sqlite3` table `structural_regime` then holds bull_run 2026-03-01, weekly_hh_then_compression 2026-06-01 and bear_channel_lower_highs 2026-08-01 (or the dates the trader edited), all `source='trader'` [trader]
...
- #276 Points challenger SP4 (S12), shadow only: the first night after merge writes `family_side_evidence.json` (ledger row `family_side_evidence` ok, "N family/side cells over M sessions"); on the next desk start the Setup Tracker's Current Picks shows 'SP4 (shadow)' after Score with the live order unchanged, and Setup keys shows one SP4 line per side. Rules fixed 2026-09-26: 20 entry sessions from that first night + 5 to mature; success = SP4 top quartile beats the champion top quartile by >= 0.5% excess vs SPY at 5 sessions AND >= 0.10R tracker R; downside stop = SP4 trails by > 0.5% after 10 matured entry sessions; rollback = delete the column; promotion to live points is ask-first (`legacy.py`, golden fixtures, the trader's quoted yes) [lead]
- #275 Sunday ritual card (B3): on Sunday, Weekend Prep's Tag week card says how many setup tags wait, its button lists them before anything is written and confirms only the ticked ones (the count drops by that many), and each family line's won/lost/exit early/held losers counts match the journal's closed trades for that confirmed setup [trader]
- #274 Alert feed speed (B8): on the next session with 500+ alerts the Alert Center feed scrolls smoothly, a row click opens the chart, row tooltips show, and the status line's loud / hidden-by-Show / first30 counts match what the feed shows [trader]
- #273 Tape line and slot narrations (S5 + B11): on the next desk start Day Review shows 'Last N sessions, tape-relative: longs beat SPY X% (excess Y%), shorts Z% (W%)' under the glance strip and the Night digest section shows that session's digest narration (or 'no narration for <date>'); the Setup Tracker shows the same tape line above its summary; after Refresh everything, Weekend Prep's Week Ahead step shows the setup-research narration, marked STALE when the newest pack has none [trader]
