...
| PCT-2 | Trendline break: `trendline_break` family tag + D1 event kind + feed alert | **REVIEW ROUND 1 NO-GO at c4f50344; fix round active.** Saved-report golden and close-only firing pass, but three blockers remain: the persisted watch does not require/freeze a line ID, type, endpoint prices and candidate break date; it records arm-click time instead of the report's `generated_at`; duplicate watches can both fire instead of once per `(symbol, side, break_date)`. The fixture must also carry endpoint prices. |
...
3. **PCT-2 - trendline break (section 7).** Review round 1 is NO-GO at c4f50344; fix the three section-1 blockers, then reviewer round 2. The `TRENDLINE_BREAK` tag already exists
