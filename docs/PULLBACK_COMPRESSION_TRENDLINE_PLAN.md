...
| PCT-2 | Trendline break: `trendline_break` family tag + D1 event kind + feed alert | **REVIEW ROUND 2 NO-GO at 4a825acf; one fix active.** Strict identity/endpoints and duplicate-fire collapse now pass, as do the nonempty golden and completed-close rules. One blocker remains: a parseable but timezone-less report `generated_at` arms a watch; missing-zone knowledge time must refuse. |
...
3. **PCT-2 - trendline break (section 7).** Review round 2 found one blocker at 4a825acf: refuse timezone-less report knowledge time, then reviewer round 3. The `TRENDLINE_BREAK` tag already exists
