...
| PCT-2 | Trendline break: `trendline_break` family tag + D1 event kind + feed alert | **ROUND-1 BLOCKERS FIXED at 4a825acf; reviewer round 2 next.** The watch now requires stable line ID, type, endpoint dates/prices, slope/anchor inputs and break date; it stores the saved report's parseable `generated_at` or refuses; duplicate persisted watches collapse to one fire per `(symbol, side, break_date)` with one save. The nonempty champion golden stays byte-pinned. Builder verification: 8362 passed / 0 failed, ruff clean, smoke 7/7, selftest 83/83. |
...
3. **PCT-2 - trendline break (section 7).** Round-1 blockers are fixed at 4a825acf; reviewer round 2 next. The `TRENDLINE_BREAK` tag already exists
