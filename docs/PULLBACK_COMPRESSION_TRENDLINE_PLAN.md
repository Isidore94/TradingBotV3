...
| PCT-2 | Trendline break: `trendline_break` family tag + D1 event kind + feed alert | **BUILT at c4f50344; independent review next.** The existing tag stays pinned. A frozen explicit extension watch and one saved-report row per `(symbol, side, break_date)` are implemented; the separate partial `Trendline breakthrough` Focus event stays unchanged. Builder verification: 8360 passed / 0 failed, ruff clean, smoke 7/7, selftest 83/83; no packaging trigger. Reviewer must reproduce golden preservation, line identity/endpoints/knowledge time, completed-close-only firing, redraw immunity and once-only identity. |
...
3. **PCT-2 - trendline break (section 7).** BUILT at c4f50344; independent reviewer next. The `TRENDLINE_BREAK` tag already exists
