...
| Packet | What | Status |
|---|---|---|
| PCT-1 | Pullback alert (M15 150-SMA / M30 75-SMA reclaim + LRSI, retest; the H1 retester folded in) + the three new claim names | RED TESTS COMMITTED 2026-09-15 (`claude/pct-1-pullback` 18806a83, 40 failing / 3 fixture checks) - builder spawned; lead rulings: `companion_bars` for the M15 leg of `reclaim_then_lrsi`, `IntradayHistoryCache(interval_minutes=60, ...)`, a declined auto watch is a kept row with `declined=True`, "2-4 bars before" counts from the cross bar, a retest bar is after the reclaim bar, the registry is `scripts/setup_registry_v1.json` |
