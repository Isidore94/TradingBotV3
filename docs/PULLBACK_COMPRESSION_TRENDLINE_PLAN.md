...
| PCT-2 | Trendline break: `trendline_break` family tag + D1 event kind + feed alert | **TESTS RED** on `claude/pct-2-trendline` at 764701fb: the existing tag pin passes; five new behavior tests fail for the missing extension kind, frozen watch fields/evaluator and real-runner feed row. The pre-change D1 alert JSON is pinned. Tester found that the scan candidate has endpoint dates but not pivot prices, and that `master_avwap_shared` has a separate partial `Trendline breakthrough` Focus event; the builder must preserve that path and add the requested scan-level `Trendline break` row through the saved D1 report. Builder next. |
...
3. **PCT-2 - trendline break (section 7).** Tests are RED at 764701fb; builder next. The `TRENDLINE_BREAK` tag already exists
