...
## BD-85 — The setup crosswalk is frozen data, and it resolves nothing it was not told

**Date:** 2026-09-01 (Phase 0.13 packet P7).

**Numbering note.** BD-78 through BD-84 are claimed on unmerged branches -
`claude/focus-declutter-lrsi-htf` uses 78-80 and `claude/p3-fact-pack-truth`
uses 80-84, so BD-80 is already double-claimed by those two and one of them will
have to renumber on merge. P7 starts at 85 to avoid being a third claimant.
...
## BD-86 — A trial is declared before the numbers, and never rewritten after them
