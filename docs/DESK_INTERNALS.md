...
### S1 fix round 1 (2026-09-03) - a rail veto is ONE row, and the decision is dated by the click

Review by reproduction found three things S1 got wrong. All three are about the
same seam: **the capture rail and the Alert Center both thought they owned the
"Not today" verdict**, and deferring the retire made the overlap visible.

#### A RAIL VETO WROTE TWO ROWS, AND AFTER S1.2 IT WROTE THEM MINUTES APART

`AlertChartReview.removeTodayRequested` is emitted by TWO things: the rail's veto
(deferred, since S1.2) and the pane's own "✕ Not today" button. The Alert Center
answered both the same way - `_remove_review_alert_for_today` wrote its own
UNCODED veto row through `_record_not_today_annotation` and then opened a note
box. For the button that is right and is what the trader asked for in R4 A6. For
a rail veto the row was a duplicate of one already on disk, and after S1.2 the
box opened **after the trader had already typed their note into the rail**.

Measured on the live stream: one KKR veto of 2026-09-03 is **three rows** -
`veto KKR rail`, `veto KKR chart_review`, and `veto KKR chart_review` carrying
"rejecting a trendline". On the base commit the same click gave two rows plus the
box at the moment of the click.

**The rule: a retire the capture rail earned costs the Alert Center nothing.**
`AlertChartReview.retiring_from_capture_verb()` is true only for the duration of
that emit - the same idiom, for the same reason, as `CaptureRail.veto_keeps_chart`
- and `_remove_review_alert_for_today` reads it once, up front, and then writes
neither the annotation nor the review event. The pane's own button is untouched:
it is not in the capture tab, the packet does not name it, and it keeps its row
and its box.

**THE VETO COHORT'S PER-CLICK ROW COUNT CHANGES AT THIS COMMIT (2026-09-03).**
Before it, one rail veto produced two `veto` rows (three when the trader used the
box); after it, exactly one, plus the follow-up `note` if they typed one. Any
count over `trader_annotations.jsonl` that spans this date is counting two
different things either side of it. The rows already written are evidence and are
never rewritten - the file is append-only - so the boundary is a date to know,
not a migration to run.

#### `like_advance` WAS DATED BY THE TYPING, NOT BY THE DECISION

`_advance_after_like` is reachable only from the deferred `likeAdvanceRequested`,
so after S1.2 the review event was written when the trader pressed Enter. Two
consequences, both measured: `dwell_ms` on the take row included however long
they spent typing, so `review_learning`'s median take dwell stopped being
comparable with the rest of the window; and a desk closed between the click and
Enter lost the decision entirely, because the annotation row survives a restart
and the review event had not been written yet.

**The rule: the host records the decision when the verb LANDS and moves the chart
when the trader is finished.** They are two moments, so they are two connections:
`AlertChartReview.captureVerbRecorded(alert, event_type)` fires from
`_await_follow_up`, and `_record_capture_verb_review_event` writes `like_advance`
or `remove_today` there with the click-time dwell. The deferred signals only
move. The ✕'s special branches - an auto pick declined, a Focus or a faded
walkthrough - record their own events when the retire runs and are deliberately
NOT duplicated at the click; only the plain path's `remove_today` moved. One
thing is genuinely lost with it: that row's `auto_pick_dropped` detail, which is
only knowable once the removal runs. The direct button keeps it.

#### TWO VERDICTS ON ONE CHART: BOTH TAKE EFFECT

S1 kept only the LAST retiring verb, so a veto followed by a like dropped
`removeTodayRequested` and the vetoed symbol was never parked for the day - a
judgement the trader made and the desk then ignored. Before S1 both fired.
`_pending_advance` now carries the alert and a SET of verb kinds, and
`_release_pending_advance` fires every route the set earned, in the host's
existing order: the veto's retire first, then the like's advance-only route. The
second one's queue advance is a no-op because of the current-alert guard.

#### AND THE SETUPS ✕ DOES NOT CHART

Three sentences shipped in S1 said "S1.4 charts this ticker in the review pane on
the same click", which is not what was built: the ✕ writes the rejection, and it
is a click on the row's TICKER - a different cell - that charts it. The words are
corrected rather than the code, and **no link is written between a note typed on
that chart and the rejection**: the link would need a second identity for the
opportunity, and plan.md P5.3/P5.4 own that one.
