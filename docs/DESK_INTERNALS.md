...
**AI-R3 outcome recovery.** The night uses the scanner's canonical finalizer after the close+35 gate. Its CSV final-row read has three states: present, absent, and unreadable. An unreadable file is UNKNOWN. Crash recovery then leaves every `finalizing` write-ahead mark and its checkpoint bytes intact and reports unresolved; the one-trade finalizer returns `commit_failed` after its durable intent, and a batch sweep reports failure before processing rows. A later readable pass reconciles the mark against the CSV. The independent review reproduced the former defect with one pending+finalizing event and an unreadable outcomes directory; the fix was red-before-green and passed the same scratch reproduction. One active finalizing intent was observed read-only in live data on 2026-09-22; no live store was written by the repair.
