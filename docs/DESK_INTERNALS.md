...
- **Entry comparisons keep the opportunity denominator and the fill denominator apart** (Phase 0.32 Packet 2, 2026-09-15). `entry_comparison` is a caller-supplied-row reader: its P8 adapter emits every declared entry variant for every base opportunity, so a no-trigger or unavailable measurement is visible rather than silently disappearing; its old-M5 adapter does not turn exit recipes into entry samples. A shared-triggered gross-MFE delta and all-opportunity coverage answer different questions and are printed separately. Scanner, like, veto, actual-trade and unreviewed rows never pool; repeated scans collapse at the attempt and `dependency_cluster_id` seams; a review comparison refuses when knowledge basis/timing/window differ. It reads the existing trial ledger for recipe ownership and lifetime looks, but cannot write, register, amend or launch a trial. Under floor or before maturity it says `not_evaluated`, never winner. This remains pure research with no store or Qt work and no live influence.
