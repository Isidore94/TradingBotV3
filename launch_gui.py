...
# The open crash-log handle (the same object as crash_log._HANDLE once armed).
...
def _enable_crash_log() -> None:
    ...
    `crash_log.arm` stamps the times faulthandler cannot.
    """
    ...
    try:
        ...
        _CRASH_LOG_HANDLE = crash_log.arm(
            log_dir, enable=lambda handle: faulthandler.enable(file=handle, all_threads=True)
        )
    except Exception:
