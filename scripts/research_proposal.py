...
def validate_proposal(
    ...
    _require(bool(str(copied.get("proposal_id") or "")), "proposal id")
    ...
    recipe_id = str(control.get("recipe_id") or "")
...
def build_compact_input(facts: Mapping[str, Any], *, limit: int = 12) -> dict[str, Any]:
    ...
    return {
        ...
        "entry_quality_cells": [
            {key: cell.get(key) for key in ("cell_id", "state", "coverage", "window")}
            for cell in _cells(report).values()
        ],
    }
...
def _render_memo(proposal: Mapping[str, Any], report: Mapping[str, Any], now: datetime) -> str:
    """A deliberately repetitive, bounded reader view from validated JSON only."""
...
Keep collecting the active, registered test. The next action is **{proposal.get('primary_action')}**: {proposal.get('question')}

## What we know

This memo is a rendered view of validated proposal JSON, not a new instruction source. It is measured as of {report.get('as_of')}; report ID {report.get('report_id')} and report hash {report.get('report_hash')} are the publication identity. The research narration states {((report.get('narrated') or {}).get('label') or 'narrated 0 of 0')}. Observed cells: {'; '.join(observed) or 'none'}. These are opportunity movements, not booked profit, and they do not identify a best trade or change a live rule.

## Active test and limits

The related trial is {progress.get('trial_id', 'not registered')}. Its progress is eligible {counts.get('eligible', 0)}, no-trigger {counts.get('no_trigger', 0)}, missing-data {counts.get('missing_data', 0)}, and sessions {counts.get('sessions', 0)}. No-trigger attempts remain in the all-opportunity denominator. Missing data remains missing, not zero. The unresolved limit is: {proposal.get('unknown')}. Failed or inconclusive findings stay visible; immature evidence is not confirmation.

## One next test

Control: {json.dumps(proposal.get('control', {}), sort_keys=True)}. One proposed change: {json.dumps(proposal.get('changed_condition', {}), sort_keys=True)}. The stated primary metric is {proposal.get('primary_metric')} over {', '.join(map(str, proposal.get('measurement_windows') or ()))}, with meaningful effect and evidence floors marked proposed choices. Support: {proposal.get('support')}. Reject: {proposal.get('reject')}. Inconclusive: {proposal.get('inconclusive')}. Comparison: {proposal.get('comparison_plan')}.

## What happens next

{proposal.get('data_needs')} Collection effort: {proposal.get('collection_effort')}. {proposal.get('similar_trial')}. No code needed. The code must keep collecting deterministic facts, and a trader may copy this brief to Sol for any separately approved implementation. Generated {now.isoformat(timespec='seconds')}."""
    words = body.split()
    # The UI requirement is a small but useful memo.  This neutral provenance
    # sentence is repeated only when terse model text would otherwise underfill it.
    filler = "The report keeps coverage, exclusions, source cells, and proposed status separate from conclusions."
    while len(words) < 400:
...
def publish_proposal_bundle(root: Path, *, proposal: Mapping[str, Any], report: Mapping[str, Any], now: datetime | None = None) -> dict[str, str]:
    ...
    # History is immutable.  A later current-file failure leaves the existing
    # good memo untouched; the orphan history is a truthful audit of validation.
    ...
    _atomic_write(_current_json(target), json.dumps(current_payload, indent=2, sort_keys=True) + "\n")
    memo = _render_memo(proposal, report, moment)
    _atomic_write(_memo_path(target), memo)
    display = build_display_payload(report, proposal, status={"worker": "complete"})
    return {"history_path": str(history), "current_path": str(_current_json(target)), "memo_path": str(_memo_path(target)), "copy_brief": copy_test_brief(display)}
...
def run_next_test_job(*, facts: Mapping[str, Any], root: Path, session_date: str, model_call: Callable[[Mapping[str, Any]], Mapping[str, Any]] | None, now: datetime | None = None, allowed_recipe_ids: set[str] | None = None) -> dict[str, Any]:
    ...
    if last:
        try:
            already = str(last.get("generated_at") or "")[:10] == str(session_date)[:10]
    ...
    if not material or same_evidence or already or model_call is None:
        return {"status": "deterministic_facts_available", "model_called": False, "progress_refreshed": True, "last_valid_proposal_id": (last or {}).get("proposal_id"), "proposal_age": bool(last)}
    try:
        candidate = model_call(build_compact_input(facts))
        allowed = allowed_recipe_ids or {str((candidate.get("control") or {}).get("recipe_id") or "")}
