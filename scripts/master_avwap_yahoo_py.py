...
def pick_previous_earnings_anchor(dates):
    """dates: ISO strings desc (most recent first). Previous anchor: second most recent past earnings."""
...
def calc_anchored_vwap_bands(df: pd.DataFrame, anchor_idx: int):
    ...
    cumsum_vol = (df["volume"].iloc[anchor_idx:] * df["close"].iloc[anchor_idx:]).cumsum()
    cumsum_q = df["volume"].iloc[anchor_idx:].cumsum()
    vwap_series = cumsum_vol / cumsum_q

    vwap_value = vwap_series.iloc[-1]
    if pd.isna(vwap_value):
        return None, None, None

    prices = df["close"].iloc[anchor_idx:]
    stdev = prices.std()

    bands = {
        "VWAP": vwap_value,
        "UPPER_1": vwap_value + stdev,
        "UPPER_2": vwap_value + 2 * stdev,
        "UPPER_3": vwap_value + 3 * stdev,
        "LOWER_1": vwap_value - stdev,
        "LOWER_2": vwap_value - 2 * stdev,
        "LOWER_3": vwap_value - 3 * stdev,
    }

    return vwap_value, stdev, bands
...
def cross_up_through_level(df: pd.DataFrame, level: float) -> bool:
    ...
    B, C = df.iloc[-2], df.iloc[-1]
    below_then_above = (
        B["close"] <= level - eps and C["close"] >= level + push and C["close"] > B["close"]
    )
...
def trim_history(history):
    ...
    for sym, entries in list(history.items()):
        new_entries = [e for e in entries if datetime.fromisoformat(e["date"]).date() >= cutoff]
...
def run_master():
    ...
    if missing_anchors:
        logging.warning("No earnings data found for: " + ", ".join(sorted(missing_anchors)))
    logging.info(
    ...
    for sym in symbols:
        ...
        df = fetch_daily_bars(sym, days_needed)
        if df.empty:
        ...
        logging.info(
            f"→ Processing {sym} ({side}) with {len(df)} daily bars; last date {last_trade_date}"
        )
    ...
    if csv_rows:
        ...
        write_header = not AVWAP_SIGNALS_FILE.exists() or AVWAP_SIGNALS_FILE.stat().st_size == 0
        df_signals.to_csv(
            AVWAP_SIGNALS_FILE,
            mode="a",
            index=False,
        ...
        logging.info(f"Appended {len(df_signals)} AVWAP signals to {AVWAP_SIGNALS_FILE}")
    else:
        logging.info(
