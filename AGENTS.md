...
- **Win rate leads every trader-facing SWING surface** (first, with `n` and a Wilson lower bound from `swing_headline`, sorting by the bound, mean R beside it); **MFE-after-a-held-level leads every DAY-TRADE surface**. ONE WILSON: `swing_headline`'s z (1.96); `master_avwap_lib/expected_r.py`'s 1.28 is a parameter inside a fenced file. **The Setup Types tab counts its own wins at its own grain; the recent rows export integer counts (`n_wins`/`n_losses`/`n_flats`/`n_unmeasured`/`n_pending`) and the weighted rate under its own name** (`win_rate_closed_basis`) - a count is never rebuilt from a rate, and a row without them says `counts not exported yet`. **ONE leader through `working_lately.select_leader`** with a declared margin (`LEADER_MARGIN_LB` 0.05) and freshness on the ENTRY session (`LEADER_FRESHNESS_SESSIONS` 2, stated by `FRESHNESS_SENTENCE`), four states, `min_n` an argument, the FLOOR judged before the clock; a study never leads and a `discovery_leader` is never called one. **Every leader surface on a page reads that one function, and the verdict is computed ONCE per page and handed to each renderer** (`panel_verdicts`) - the banner and the Summary card each computed their own and disagreed the moment one gained a `previous`. A LABEL follows its verdict (`verdict_label_suffix`), **a renderer that has a verdict renders all four states** (a hardcoded fallback sentence is a state the machine does not know about), and a tile or card that borrows another surface's sort has no meaning of its own. `latest_measured_session` is the MEASURED bar wherever the export can answer (the 2-session rows), the entry only where it cannot (the recent rows, until ST4), and `freshness_sentence(kind)` says which so neither line claims the other's clock.
