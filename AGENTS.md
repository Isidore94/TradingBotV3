...
- **The tracker replay has a versioned execution convention and level knowledge** (ST3, `scripts/master_avwap_lib/execution_convention.py`), default `literal_level_v1` / `same_session_v1` and byte-identical, a default run writing neither key. `gap_aware_v2` (gapped level fills at the OPEN, no open clamps into `[low, high]`, an invalid candle books nothing - no fill, no excursion, no mark - counts `skipped_bar_reasons["invalid_bar"]` and DEFERS the max-hold `TIME_STOP`, which is the only exit that label may reach) and `prior_session_v2` (only the INTRABAR target tests move; close-based checks keep day D) are shadow evidence through `scripts/tracker_execution_compare.py` and reach no score until the trader decides.
