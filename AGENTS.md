...
- **A like and a veto merge into their cohorts on the same click, through one helper** (`capture_rail._merge_cohort_safely`). They are read side by side, so a difference between them must come from the data. Failure degrades to a status suffix — the annotation row is already on disk and both merges are idempotent. The nightly slot stays.
