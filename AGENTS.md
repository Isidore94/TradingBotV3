...
- Auto-mode matrix (trader rules 2026-08-14, packet R1 — `docs/AUTO_MODES_AND_QUIET_HOURS_PLAN.md`): discovery is identical in every mode; what changes is who is present. **DESK** adopts staged auto picks into M5 Focus immediately. **AWAY** stages and never adopts, and queues alerts silently — feed/history/D1 badge keep filling, only the sound is suppressed; the queue drains on the flip back to DESK. **EVENING** runs the open+30 early slot, the 07:00/07:15/07:30 strength checks and the briefing, then stops — no ordinary hourly slot, no open watchlist self-build — and stages picks for the wake-up flip. **OFF** is the only mode where auto picks still self-apply.
