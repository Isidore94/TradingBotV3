...
- **The tracker replay has a versioned execution convention and level knowledge** (ST3, `master_avwap_lib/execution_convention.py`): default `literal_level_v1` / `same_session_v1`, byte-identical, a default run writing neither key. `gap_aware_v2` (gapped fills at the OPEN, no open clamps into the bar, an invalid candle books nothing and defers the max-hold `TIME_STOP`) and `prior_session_v2` (only INTRABAR target tests move) are shadow evidence through `tracker_execution_compare.py` until the trader decides.
- **Which observation of a thesis gets graded is a NAMED policy, and today's is unchanged** (ST4): family rows carry `selection_policy`; `closed_first_v1` (`master_avwap_lib/selection_policy.py`) is the default and `first_actionable_v2` (earliest row per attempt, declared re-entry rule, declared `full_band2` representative, pending stays pending) is shadow evidence through `tracker_selection_compare` until the trader decides. A scenario's exit date is its last `events` entry, read only when CLOSED; a COMPACTED record is `undatable_exit`, never silently pending. **A compact scoring projection's `_scoring_outcome_summary` IS the record**: a default read takes it unconditionally - recomputing dropped every setup and zeroed both scan score deltas.
