...
- **A real miss is a RULE, and the session a decision belongs to is an ADDITIVE field** (TJ-11): `REAL_MISS_V1` (`scripts/real_miss.py`) is 1.0 ATR in the decision's favour BEFORE 0.5 ATR against, completed bars only, the adverse extreme first in a bar, point-in-time ATR(14), anything missing `unmeasured` never `no_run`, reported never acted on, ONE function shared with TJ-15. **`session_date` is UNCHANGED** - the join `pick_feedback`, the cohort graders, `review_learning` and `daily_recap_reader` use. **A decision belongs to the session it JUDGED** (TJ-11F, trader 2026-09-19: a Friday-evening veto is FRIDAY's): `market_calendar.decision_session` is the stamp's New York date if a session day, else the PRIOR one; a NEW row carries `decision_session` plus `decision_session_rule: "judged_session_v2"`; an unmarked value came from the struck forward rule and every reader recomputes it, never rewriting a row. Long form: DESK_INTERNALS "Day Review - walk-away v2".
- **A D1 call gets a D1 ruler, and a miss is read against a base rate** (TJ-11): `walkaway_day` measures a D1 decision on DAILY bars from its session's close over 1/3/5 exchange sessions (`pending <date>`, never zero) and an M5 one on the session tape; the skill line partitions the SAME scan into liked/claimed, rejected and untouched with `n`, `measured`, `pending` and the ONE Wilson interval, says when intervals overlap, and no R statistic selects what it shows. **A pooled rate counts a name only when its horizon has CLOSED**, runs and no-runs alike - an early run inside an open horizon shows on its ROW and never in a fraction, never censored by its outcome. An option or a hold past five sessions reads `not judged here`.
