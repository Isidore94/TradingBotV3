...
- **`preference_trade_outcomes` matches inside 10 SESSIONS** (`statement_window_end`; a calendar refusal falls back NARROWER) **and `trade_level_summary` sums P&L once per `trade_id`** over a file that stays one row per statement. **`journal_exposure` reads bias from the legs and a LONG option is never a bullish setup**; a `trade_legs` row is a FILL, so `multi_leg` means more than one option CONTRACT, and a spread look-alike is `partial_of_spread_candidate` (no sibling seam exists). **`personal_evidence_summary` partitions by STATUS — complete / partly_closed / open_exposure (net AND winners `None`, never zero) — and `uncertain` is a CROSS-CUTTING label that pools no money**; checking uncertainty first made it a fourth bucket that ate 84 CLOSED, all 7 CLOSED_PARTIAL and 29 of 32 OPEN rows and counted open marks as winners. **Both tag lanes share ONE denominator (closed or partly closed)** — the journal's one confirmed tag is on a CLOSED_PARTIAL trade — and **no personal setup is called best below `MIN_REPORTABLE_N` confirmed tags**, with "No confirmed setup tags" reserved for a true zero. Weekend Prep lists the WHOLE provisional backlog and REFERS a missing planned risk to the Trades tab; nothing computes `planned_risk` from an outcome.
