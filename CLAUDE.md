...
- Today's swing picks (`ui/widgets/swing_favorites_bar.py`) get two writes - swing Focus FIRST and must not fail, then the append-only `swing_favorites.jsonl` row whose failure is swallowed; never an auto-adoption marker; like-origin `vetted`; a removal appends a RETRACTION. The strip sits UNDER the setups in the RIGHT (D1) column since 2026-09-14 (`d1_column`, D1C-L); the left column is M5 only.
...
- **The Trade Mentor prompts only a PRESENT trader** (WS-TM + Phase 0.31): whole-hour reads from 07:00 Pacific in DESK / EVENING / OFF, never in AWAY, never a push; an answer is one dated Market Journal row, a missed hour is recorded as skipped, and the Settings checkbox ships OFF. **Since TJ-9 the 09:00 slot is the FORCED trade check** (trader 2026-09-19, `trade_mentor_schedule.TRADES_HOUR`): it lists every trade of the previous session, Save stays grey until each material field holds a value or an answer state, and it RIDES on any later delivered slot of that session while still owed - so an AWAY, idle or expired 09:00 no longer takes the day's questions with it - a section carrying ANSWER WIDGETS never rebuilt, a one-line state always rebuilt so its date is re-read. **A `provisional` tag is not an answer and a rejection tag is never offered or confirmed as a setup** (`journal_analytics.is_rejection_tag` is the ONE definition of `vetoed:` / `passed:`, refused again at `confirm_setup`); the guess is a confirm button that writes nothing until the trader presses it, and a confirmed tag carries `label_provenance` from the pure `scripts/trade_origin.py`, whose session is the FIRST FILL's and never `trade_date`. A journal that is not ready earns ONE Questrade retry through `JournalImportService`. The 10:00 reply is saved RAW before an off-Qt local-AI draft fills only missing fields; exact spans and units are validated, existing values win, and Save/Correct is still the trader's act. It is the one narrow fixed-time exception to quiet hours. Long form: DESK_INTERNALS "TM - a prompt is a slot, an answer is a dated row".
...
- The Market Journal is what the trader thought; the Journal is what they traded - two stores, never merged, both through `shared_journal_service()`; `written_after_the_session` is COMPUTED, never backdated; a capture joins by `entry_id`. The STORE keeps that name and that rule; since TJ-1 the PAGE that reads it is **Day Review**.
- **One page reads the day and no machine row is on it** (TJ-1): `Day Review` reads ONE payload on ONE worker and builds ONE chart on first need; a mode flip is an Auto Pilot log line, `is_machine_entry` is the ONE filter (`entries_about`, `build_daily_story`, `_stories_from_journal`), and a per-session index under `DAY_REVIEW_DIR` replaces the 476 MB stream. **Since TJ-1L it reads in TWO COLUMNS** (trader, 2026-09-18): the `DayReviewColumns` splitter at 55/45 saved per machine, story/theses/chart left and the trader's words right, Walk-away a 2 x 2 grid, and every table on this page stretches its last section instead of taking the shared width rule's 260 px ceiling. Long form: DESK_INTERNALS "Day Review - one page, no machine rows, a per-session index".
- **A real miss is a RULE, and the session a decision belongs to is an ADDITIVE field** (TJ-11): `REAL_MISS_V1` (`scripts/real_miss.py`) is 1.0 ATR in the decision's favour BEFORE 0.5 ATR against, completed bars only, the adverse extreme taken first inside a bar, point-in-time ATR(14) from `chart_snapshot.load_d1_bars`, anything missing `unmeasured` never `no_run` - reported, never acted on, and ONE function shared with TJ-15's future slot. **`session_date` is UNCHANGED** - it is what `pick_feedback`, the three cohort graders, `review_learning` and `daily_recap_reader` join on; `market_calendar.decision_session` answers which exchange session a stamp belongs to, a NEW annotation row carries it as the extra key `decision_session` (empty when unanswerable, absent and never backfilled on old rows), and only TJ-11's readers use it (`load_annotations(..., by_decision_session=True)` is the opt-in). Long form: DESK_INTERNALS "Day Review - walk-away v2".
- **A D1 call gets a D1 ruler, and a miss is read against a base rate** (TJ-11): `walkaway_day` measures a D1 decision on DAILY bars from its session's close over 1/3/5 exchange sessions (`pending <date>`, never zero) and an M5 one on the session tape; the skill line partitions the SAME scan into liked/claimed, rejected and untouched with `n`, `measured`, `pending` and the ONE Wilson interval, says when intervals overlap, and no R statistic selects what it shows. **A pooled rate counts a name only when its horizon has CLOSED**, runs and no-runs alike - an early run inside an open horizon shows on its ROW and never in a fraction, or the rate is censored by its own outcome. An option or a hold past five sessions reads `not judged here`.
...
- **Local inference is night-only, seven days a week, and the NIGHT picks the slate** (TJ-13A, trader 2026-09-19): `window.in_offhours_window` applies the configured ET window every day (the market-session block's own weekend short-circuit is untouched) and `--force` buys the attempt caps and the already-done check, never the clock for a `JobSlot.uses_model` slot; `runner.night_kind()` names a night on the exchange calendar from the evening it started and `runner.slots_for()` builds its slate - weeknight without `ai_summary`, Saturday with `ai_summary` and `weekly_synthesis`, Sunday the deterministic stage plus what the weekend still owes - while `EXPECTED_SLOT_ORDER` stays the order WITHIN a night. `JobSlot.model_free_kwargs` is how a forced daytime run still gets a slot's deterministic half (`daily_digest`'s `narrate=False`, superseding and never overwriting last night's narrated digest), and a per-ticker projection's `truncated` flag describes that projection and is never inherited from the session package. Long form: DESK_INTERNALS "Night kinds".
...
- **A chart opened FROM the setups table cycles through that table, and a veto for the day HIDES the row** (SC, trader 2026-09-15): `MasterAvwapPanel._chart_row_on_desk` hands `chart_symbol` a `next_pick` callback that charts the table's next visible row (same symbol and today's rejects skipped); `_advance_review_queue` consumes it INSTEAD of the waiting list after a veto, a claim or the Next verb, and any other chart taking the pane drops it. The hide is a PRESENTATION filter over `pick_feedback.HIDDEN_REJECT_KINDS` (veto / dislike / not_today / remove_today - never a day-trade pass or an M5 click-away), counted on the `Show vetoed` box, never a deletion; the scan and the tracker never read it. Long form: DESK_INTERNALS "SC".
