...
- **The capture rail is a recorder that now moves the queue** (trader rules 2026-08-20): a VETO and a LIKE each retire the chart the way "Not today" does; a NOTE never does, because it is written ABOUT the chart in front of you. "Veto D1 - but M5 today" writes an ordinary veto row and emits a REQUEST - `AlertCenterPanel` performs the M5 Focus placement, so the Focus store keeps one writer; place first, retire second, and a failed placement still retires. Known limitation, deliberately not papered over: the veto cohort therefore counts a day-traded name as vetoed. The hypothetical-stop CONTROL was removed; `EVENT_HYPO_STOP` stays in the annotation schema so existing rows remain readable.
- **Veto vocabulary is versioned, and codes are never reused** (`scripts/ui/annotations/vocabularies/`): v2 shipped 2026-08-20 replacing the "S/R cluttered" slot with "Compressed" as a NEW code. v1 stays on disk and stays loadable. Cohort identity is `(vocab_version, reason_code)` - `veto_cohort_source(code, version)` - so eight unchanged reasons now grade in two cohorts across the bump; that cost is recorded in `docs/CHART_REVIEW_WORKSPACE_PLAN.md`. Never assert a literal `vocab_version` in a test; assert against the loaded vocabulary.
