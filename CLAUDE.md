...
- **A VETO and a LIKE each retire the chart ON ENTER after the click - the chart stays up until the trader finishes typing** (S1.2, trader 2026-09-03: *"when I hit like or not today or anything, it should keep the chart up UNTIL I finish typing"*). The verdict row still reaches disk on the CLICK, and so do `like_advance` and "reviewed today"; what waits is only `removeTodayRequested` / `likeAdvanceRequested`. Enter with a typed line writes ONE `note` row carrying `supersedes` - the id the verdict row ALREADY has, never a second opportunity id - and then retires; Enter on an empty field or Escape retires and writes nothing; a failed note write still retires. Clicking away is still a SKIP and still retires the chart it left. A NOTE and a day-trade PASS never retire. "Veto D1 — but M5 today" writes a veto row and emits a REQUEST; the panel performs the Focus placement, so the Focus store keeps one writer. Place first, retire second; a failed placement still retires. What a LIKE offers is `MAIN_CLAIM_GROUP` + `EXTRA_CLAIM_IDS` in `ui/annotations/setup_claims.py`; keys run in list order so learned digits never move.
...
- **A LIKE has two modes and only one of them names a setup** (P9, 2026-09-02, trader: *"anytime I like and claim a setup or like a day trade setup I just want to let the bot and the future AI know 'something about this was good'"*). **Alt+L** writes a QUICK like - `like_mode: "quick"`, no claim, no why - and **Alt+K** the claimed one, which still requires both. **NO VERB OPENS A DIALOG** (S1.1, trader 2026-09-03: *"when I hit something in the capture tab such as veto, or like and claim etc that is sufficient reason enough - these are quick buttons to get a note in essentially and do NOT require a pop up note"*). This SUPERSEDES the 2026-09-02 "the BUTTON prompts" rule: both quick-like buttons now do exactly what Alt+L does, `prompt_quick_like` is an alias for `commit_quick_like`, and the optional sentence is typed into the rail's own note field while the chart waits (S1.2). **R9.2(a)'s required why is superseded for the CLAIMED path too** - the claim is the whole requirement, an empty why is recorded as nothing rather than refused, and `_prompt_for_why` is gone. The **Master AVWAP ✕** asks nothing either: it writes the rejection at once, UNCODED, with `reason` present and empty, and opens neither the picklist, the detail box nor the R4 A6 note box - a code the trader did not choose would be worse than none, because cohort identity is `(vocab_version, reason_code)` and rows are never rewritten. The ★ keeps its note box; it has no other surface to type on. A quick like retires the chart, records `like_advance` and marks the symbol reviewed exactly as a claimed one does, and **places nothing** - a like carries zero privileges (plan.md P3.1). It grades under `like_unclaimed` and contributes a **LINK** to the auto-tagger, never a tag, because it names no setup. `like_mode` is ADDITIVE (schema stays 1) and its ABSENCE reads as `claimed` - a claim was required until P9. **A capture sidecar is completed after the close** by the `sidecar_completion` slot into a NEW file (`m5_bars_completed_ref`); the original `m5_bars_ref` still means "what the desk held at the click" and is never rewritten.
...
- **The group RS/RW tape owns its own clock** (`scripts/group_rrs.py` pure formula + `ui/services/group_tape_service.py`): ONE batched `yfinance` download per 5-minute tick, no retry inside the tick, **zero IB traffic and no `legacy.py` change**. Session filter is completed M5 bars **plus a same-date filter**; a window without `length + 2` bars is `None` and draws NOTHING (0.0 would claim "in line with SPY"). The RS Window answers a different question (who led over the selected window at scan time) and is a SECTION of the Strength surface since S1.3, not a tab of the setups column. It has **never** read `rrsSnapshotChanged` — it re-ranks on its own 5-minute timer plus a `showEvent` tick, which now fires at startup rather than when a tab is opened. `focus_picks_panel` is the reader that does.
- **The Strength column is ONE surface and every section starts OPEN** (S1.3, trader 2026-09-03: *"lets revamp the strength tab to just include all of that great information into one tab. also make that tab resizable horizontally so I can compress the capture/journal tab a bit and see more of the strength tab"*). Four `CollapsibleSection`s in reading order - **Focus Strength**, the **RS/RW Board** (V1's rule survives: RS/RW above M5 Strength), the **M5 Strength Board**, and the **RS Window** moved out of the setups column's tabs - inside ONE column-wide `QScrollArea` that scrolls vertically and **never horizontally**. Every body goes through `_strength_section_body`, so no board's own minimum reaches the splitter (the RS Window's is 1,612 px, the RS/RW board's 452) and each gets its own horizontal scrollbar rather than being clipped: the alert column's floor is **unchanged at 932 px, measured**. The boundary is `tabs_row` (`ALERT_TABS_SPLIT_KEY`), whose handle was Qt's default **4 px** and is now **8** so it can be grabbed. **A split the trader saved counts as user-set from the first paint** (`desk_layout._PresetTracker`) - otherwise the first resize after launch scales the 60/40 preset over the top of it - and a failed `persist_sizes` save is LOGGED, never swallowed.
- **M5 Strength Board:** batched yfinance over `universe_all.txt` **PLUS the four trader watchlists** (V1, decision 0016 answer 9 - a name the trader follows may not clear the universe's liquidity spec, and the board it never appears on is the one they read), **zero IB traffic**. Its relative volume is **SESSION-RELATIVE** (R4 A7): bar k of today against bar k of each of the prior 15 sessions, and a session that never reached bar k **contributes nothing rather than a zero**. The D1 SMA floors read a **`2y`** daily download with **today's forming bar dropped** (R4 A8). `relative_volume` is deliberately NOT one of the seven fenced formula functions; those seven stay byte-identical to the R8 baseline. Every board add re-runs the adoption gate at click time. **A row click charts into the Visual Alert Review pane** via `chart_symbol` (the lookup box's door), never `_enqueue_review_alert` (the scanner's door, which drops in AWAY, drops parked symbols, diverts M5 to the alert bar and can hide a row behind movers-only). **The Master AVWAP setups table's ticker click takes the same door** (S1.4, trader 2026-09-03: *"when I hit a ticker on the master avwap setups tab, I dont want the chart to be a pop up, I want it to come up on the visual chart review instead"*) - `symbolActivated` out of `MasterAvwapPanel`, with the Alert Center tab raised in tabs mode and the snapshot dialog still one right-click away; ★ / ✕ are excluded, and Qt's click-then-double-click pair charts once.
