...
- **Which observation of a thesis gets graded is a NAMED policy, and today's is unchanged** (ST4, 2026-09-06). Family rows carry a `selection_policy`; `closed_first_v1` (`scripts/master_avwap_lib/selection_policy.py`, `DEFAULT_SELECTION_POLICY`) is the default and `first_actionable_v2` (earliest row per attempt, declared re-entry rule, declared `full_band2` representative, pending stays pending) is shadow evidence through `tracker_selection_compare` until the trader decides. A scenario has NO exit-date field: the recorded exit is the last `events` entry's `trade_date`, read only when the scenario is CLOSED, and a COMPACTED record (`events` emptied) is `undatable_exit`, never silently pending; in `excluded_reasons` a bare `reason=N` is a drop summed into `n_excluded` and an `_in_population` token is a KEPT decision, never summed. **`_scoring_outcome_summary` on a compact scoring projection IS the record** (no `scenarios` key, 11,372 in `master_avwap_tracker_scoring_snapshot.json`): a default read takes it unconditionally and a missing key is never a reason to recompute - recomputing dropped every setup and zeroed both scan score deltas.
