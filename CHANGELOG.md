...
- **Overnight daily AI repair (2026-09-22, main).** Day Review is
  ...
  strict publication checks. Two model-free night jobs sweep canonical pending
  outcomes after close and build the Day Review facts before narration. Theta
  pick evidence is recorded after deferred option quotes. The downstairs DAS
  was reachable in the audit; this change does not alter storage. Live model,
  scan and page gates remain owed.
...
September 21's two exhausted model jobs left no verified day story or observation tags. The repaired requests let code own read links, verdicts and exact quote spans while the local model supplies only bounded words or closed codes; invalid replies still preserve the last verified output. A.I. Summary now opens the last completed session's Day Review, where measured facts remain visible and a failed story is named. A model-free close+35 outcome sweep and a day-facts build run before narration, using the existing scanner finalizer and Day Review service. Deferred theta capture now stores observed option quotes after enrichment, or an honestly unmeasured pick when quotes fail. R1/R2 each passed independent reproduction; R3's tester established red failures, then the lead completed its protected seams after trader approval. Combined gates and live proof are recorded in `CURRENT_CHECKPOINT.md` and `plan.md`; this scratch branch is not on the running desk.
