...
- 2026-09-23 RVOL on chart review: D1 volume columns coloured by daily rvol (volume / 50-day avg; white <1, yellow 1-1.5, orange 1.5-2, green 2-3, blue 3+, grey = unmeasured or today's preview), and `RVOL 1.84×` on the D1 header row - today's volume vs the same time of day over 15 sessions (`rvol.session_rvol`), one yfinance 5m download per symbol per 2 min off the Qt thread (`ui/services/intraday_rvol_service.py`).
