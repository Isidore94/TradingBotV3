...
Initial inventory reconciliation: **2026-08-27** on `claude/gui-phase-0-9`, at the four trader
...
`Recent changes` below cover the last two build days; everything older is in the
archives indexed by `docs/README.md`, the newest being
...
- **AR Pullback timing and review (built and verified 2026-09-22, isolated `codex/alert-quality-2026-09-22`; activation owed).**
  M30's native/companion cross uses its own completion time and stable identity;
  a pre-arm cross cannot fire or exclude an eligible companion. Cache delivery
  can be judged on the next poll inside the same bucket. Existing fire evidence
  retains measured SMA/LRSI/EMA values and separate event/cross times. H1 remains
  EMA15, M30 SMA75 plus LRSI80; thresholds are unchanged. Feed and chart reason
  text share price/Focus/D1/M15/M30/H1 colours, including a folded latest reason.
  Ordinary D1 scan ideas wait behind the reversible Show all view; personal and
  Focus alerts retain their routes and all backing history remains. The exact
  saved extended-from-base veto requests an M30/H1 Pullback watch before normal
  retirement. Verification and activation status: CURRENT_CHECKPOINT.md; contract:
  DESK_INTERNALS "AR" and the Pullback / Chart Review Workspace amendments.

- **Manual-only Pullback alerts (2026-09-17; narrow AR exception 2026-09-22).** Claims and Focus names no longer
  pre-arm Pullback alerts; any old automatic row is removed. The chart button is the only arm path,
  apart from the trader-requested, successfully saved extended-from-base veto.
  The chart button skips the unnecessary M5-cache read that could stall the desk.
...
### 2026-09-22 — AR: correct pullback timing, colour the reason, and reduce review load

The trader's four-part request is built and verified on an isolated review branch. M30 was
reusing an M15 cross from before the watch was armed and stamping a new M30 identity
on later reports; delayed cache delivery could also miss the current evaluation
bucket. Both are corrected without changing indicator mathematics. The existing
fire row now preserves the measured source and event times; H1 names its source
so its cyan label works on the real producer path. A shared display classifier
colours price red, Focus green, D1 blue, M15 amber, M30 purple and H1 cyan, and a
folded reason follows the latest alert. The default personal review view holds
ordinary scan ideas behind Show all while keeping their feed and evidence. Only
the exact saved extended-from-base veto can request the new M30/H1 follow-up;
existing manual watches and ordinary veto retirement are preserved. The lead
selected both timeframes and the reversible view after offering optional choices;
no answer was treated as an explicit choice. Independent reviewers gave GO to
all packets. The final isolated full suite passed 10,320 tests with 14 skipped
and 72 subtests; Ruff, smoke 7/7 and source selftest 97/97 passed. Live gates
remain in CURRENT_CHECKPOINT.md. No desk checkout or live store changed.

Maintenance: moved twelve Sep 18–19 checkpoint entries and eighteen Sep 19–20
changelog entries verbatim into their dated archives; all existing gate rows were
verified identical. Corrected the checkpoint's stale branch-only journal claims
against main's actual ancestry. The existing instruction-file size debt remains.
